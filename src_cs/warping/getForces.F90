...
#include <petsc/finclude/petsc.h>
    ...
    complex(kind=realType), intent(inout) :: forces(3, npts)

    integer(kind=intType) :: mm, nn, i, j, ii, jj, iDim, ierr
    integer(kind=intType) :: iBeg, iEnd, jBeg, jEnd
    complex(kind=realType) :: sss(3), v2(3), v1(3), qa, sepSensor, Cavitation
    complex(kind=realType) :: sepSensorAvg(3)
    complex(kind=realType) :: Fp(3), Fv(3), Mp(3), Mv(3), yplusmax, qf(3)
    ...
    complex(kind=realType), dimension(nCostFunction) :: funcValues
...
#include <petsc/finclude/petsc.h>
    ...
    implicit none
    integer(kind=intType), intent(in) :: npts, sps
    complex(kind=realType), intent(out), dimension(3, npts) :: forces, forcesd
    integer(kind=intType) :: mm, nn, i, j, ii, jj, iDim, ierr
    integer(kind=intType) :: iBeg, iEnd, jBeg, jEnd, ind(4), ni, nj
    complex(kind=realType) :: qa, qad, qf, qfd
    complex(kind=realType), dimension(:), pointer :: localPtr, localPtrd
    type(zipperMesh), pointer :: zipper
    type(familyexchange), pointer :: exch
...
#include <petsc/finclude/petsc.h>
    ...
    implicit none
    integer(kind=intType), intent(in) :: npts, sps
    complex(kind=realType), intent(inout) :: forcesd(3, npts)
    integer(kind=intType) :: mm, nn, i, j, ii, iDim, ierr
    integer(kind=intType) :: iBeg, iEnd, jBeg, jEnd
    type(zipperMesh), pointer :: zipper
    type(familyexchange), pointer :: exch
    complex(kind=realType), dimension(:), pointer :: localPtr
    complex(kind=realType), dimension(3, npts) :: forces
...
#include <petsc/finclude/petsc.h>
    ...
    integer(kind=intType) :: mm, nn, i, j, ii, jj, iDim, ierr
    integer(kind=intType) :: iBeg, iEnd, jBeg, jEnd, ind(4), ni, nj
    complex(kind=realType) :: qf_b, qf, qa, qa_b
    complex(kind=realType), dimension(:), pointer :: localPtr, localPtr_b
    type(familyExchange), pointer :: exch
...
subroutine getHeatFlux(hflux, npts, sps)
    ...
    !
    integer(kind=intType), intent(in) :: npts, sps
    complex(kind=realType), intent(out) :: hflux(npts)
    ...
    do nn = 1, nDom
        ...
        do mm = 1, nBocos
            ...
            bocoType1: if (BCType(mm) .ceq. NSWallIsoThermal) then
                BCData(mm)%cellVal => BCData(mm)%area(:, :)
            else if ((BCType(mm) .ceq. EulerWall) .or. (BCType(mm) .ceq. NSWallAdiabatic)) then
    ...
    do nn = 1, nDom
        call setPointers(nn, 1_intType, sps)
        do mm = 1, nBocos
            bocoType2: if (BCType(mm) .ceq. NSWallIsoThermal) then
    ...
    do nn = 1, nDom
        ...
        do mm = 1, nBocos
            bocoType3: if (BCType(mm) .ceq. NSWallIsoThermal) then
                do j = BCData(mm)%jnBeg, BCData(mm)%jnEnd
                    do i = BCData(mm)%inBeg, BCData(mm)%inEnd
                        ii = ii + 1
            ...
            else if ((BCType(mm) .ceq. NSWallAdiabatic) .or. (BCType(mm) .ceq. EulerWall)) then
                do j = BCData(mm)%jnBeg, BCData(mm)%jnEnd
                    do i = BCData(mm)%inBeg, BCData(mm)%inEnd
                        ii = ii + 1
                        hflux(ii) = zero
...
subroutine heatFluxes
    ...
    complex(kind=realType) :: fact, scaleDim
    complex(kind=realType) :: qw, qA
    ...
    bocos: do mm = 1, nBocos
        ...
        if (BCType(mm) .ceq. NSWallIsoThermal) then
...
subroutine setTNSWall(tnsw, npts, sps)
    ...
    ! Input Variables
    integer(kind=intType), intent(in) :: npts, sps
    complex(kind=realType), intent(in) :: tnsw(npts)
    ...
    domains: do nn = 1, nDom
        ...
        ! Loop over the number of viscous boundary subfaces of this block.
        bocos: do mm = 1, nBocos
            isoWall: if (BCType(mm) .ceq. NSWallIsoThermal) then
                jBeg = BCdata(mm)%jnBeg; jEnd = BCData(mm)%jnEnd
                iBeg = BCData(mm)%inBeg; iEnd = BCData(mm)%inEnd
                do j = jBeg, jEnd
                    do i = iBeg, iEnd
...
subroutine getTNSWall(tnsw, npts, sps)
    ...
    ! Input Variables
    integer(kind=intType), intent(in) :: npts, sps
    complex(kind=realType), intent(out) :: tnsw(npts)

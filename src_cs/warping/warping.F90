 ...
 contains
    ...
    subroutine setGrid(grid, ndof)
       ...
       use complexify 
        ...
        if ((equationMode .ceq. steady) .or. (equationMode .ceq. TimeSpectral)) then
            timeUnsteady = zero
    ...
    subroutine setGridForOneInstance(grid, sps)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(:), intent(in) :: grid
    ...
    subroutine getGrid(grid, ndof)
       ...
       use complexify 
        implicit none
        integer(kind=intType), intent(in) :: ndof
        complex(kind=realType), intent(out) :: grid(ndof)
    ...
    subroutine getStatePerturbation(randVec, nRand, randState, nRandState)
       ...
       use complexify 
        ...
        complex(kind=realType), intent(in), dimension(nRand) :: randVec

        ! Ouput Parameters
        complex(kind=realType), intent(out), dimension(nRandState) :: randState
...
#include <petsc/finclude/petsc.h>
        ...
        complex(kind=realType), intent(in), dimension(nRand) :: xRand
        integer(kind=intType), intent(in) :: nFamList, famList(nFamList), sps
        ! Ouput Parameters
        complex(kind=realType), intent(inout), dimension(3*nRandSurface) :: randSurface

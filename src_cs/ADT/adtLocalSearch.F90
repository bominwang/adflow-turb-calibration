...
contains
    ...
    subroutine containmentTreeSearch(ADT, coor, &
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(:, :), intent(in) :: coor
        complex(kind=realType), dimension(:, :), intent(in) :: arrDonor

        integer(kind=intType), dimension(:, :), intent(out) :: intInfo
        complex(kind=realType), dimension(:, :), intent(out) :: uvw
        !
        !       Local variables.
        !
    ...
    subroutine containmentTreeSearchSinglePoint(ADT, coor, &
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(3), intent(in) :: coor
        complex(kind=realType), dimension(:, :), intent(in) :: arrDonor
        ...
        complex(kind=realType), parameter :: adtEps = 1.e-25_realType
        complex(kind=realType), parameter :: thresConv = 1.e-10_realType
        !
        !       Local variables.
        !
        integer :: ierr
        ...
        complex(kind=realType) :: u, v, w, uv, uw, vw, wvu, du, dv, dw
        complex(kind=realType) :: oneMinusU, oneMinusV, oneMinusW
        complex(kind=realType) :: oneMinusUMinusV
        complex(kind=realType) :: a11, a12, a13, a21, a22, a23
        complex(kind=realType) :: a31, a32, a33, val

        complex(kind=realType), dimension(3) :: x, f
        complex(kind=realType), dimension(8) :: weight
        complex(kind=realType), dimension(3, 2:8) :: xn
        ...
        treeTraversalLoop: do
                ...
                childrenLoop: do mm = 1, 2
                    ...
                    terminalTest: if (kk < 0) then
                        ...
                        if (coor(1) >= ADTree(kk)%xMin(1) .and. &
                            ...
                            if (nFrontLeavesNew .ceq. nAllocFront) then
                                i = nAllocFront
            ...
            if (nFrontLeavesNew .ceq. 0) exit treeTraversalLoop
    ...
    subroutine minDistanceTreeSearchSinglePoint(ADT, coor, intInfo, &
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(4), intent(in) :: coor
        complex(kind=realType), dimension(:, :), intent(in) :: arrDonor

        integer(kind=intType), dimension(3), intent(out) :: intInfo
        complex(kind=realType), dimension(5), intent(out) :: uvw
        integer(kind=intType), dimension(:), pointer :: frontLeaves
        integer(kind=intType), dimension(:), pointer :: frontLeavesNew
        type(adtBBoxTargetType), dimension(:), pointer :: BB
        !
        ...
        complex(kind=realType) :: dx, dy, dz, d1, d2, invLen, val
        complex(kind=realType) :: u, v, w, uv, uold, vold, vn, du, dv
        complex(kind=realType) :: uu, vv, ww

        complex(kind=realType), dimension(2) :: dd
        complex(kind=realType), dimension(3) :: x1, x21, x41, x3142, xf
        complex(kind=realType), dimension(3) :: vf, vt, a, b, norm, an, bn
        complex(kind=realType), dimension(3) :: chi
        complex(kind=realType), dimension(8) :: weight
        ...
        treeTraversal2: do
            ...
            if (nFrontLeavesNew .ceq. 0) exit treeTraversal2
    ...
    subroutine minD2Hexa(xP, x1, x2, x3, x4, x5, x6, x7, x8, d2, chi, iErr)
       ...
       use complexify 
        ...
        !     Subroutine arguments.
        !
        complex(kind=realType), dimension(3), intent(in) :: xP
        complex(kind=realType), dimension(3), intent(in) :: x1, x2, x3, x4
        complex(kind=realType), dimension(3), intent(in) :: x5, x6, x7, x8

        integer(kind=intType), intent(out) :: iErr

        complex(kind=realType), intent(out) :: d2
        complex(kind=realType), dimension(3), intent(out) :: chi
        !
        !     Local variables.
        !
        ...
        complex(kind=realType) :: inactGradNorm, normDeltaChi, x0, y0, z0
        complex(kind=realType), parameter :: gradTol = 1.0e-14_realType
        complex(kind=realType), parameter :: deltaChiTol = 1.0e-14_realType
        complex(kind=realType), dimension(3) :: deltaChi, actualDeltaChi
        complex(kind=realType), dimension(3) :: lwrBnd, uppBnd
        complex(kind=realType), dimension(3) :: grad, oldChi
        complex(kind=realType), dimension(3, 3) :: hess
        ...
        IterLoop: do while (itCount <= maxIt)
            ...
            do i = 1, 3
                ...
                if (actSet(i) .ceq. 0) inactGradNorm = inactGradNorm + grad(i)**2
            ...
            if (inactGradNorm < gradTol) then
                do i = 1, 3
                    if (actSet(i) .ceq. 0) chiGradConv(i) = 1
                end do
            end if

            if (sum(chiGradConv(1:3)) .ceq. 3) convGradD2 = .true.
    ...
    subroutine newtonStep(hess, grad, step, iErr)
       ...
       use complexify 
        ...
        !     Subroutine arguments.
        !
        complex(kind=realType), dimension(3), intent(in) :: grad
        complex(kind=realType), dimension(3), intent(out) :: step
        complex(kind=realType), dimension(3, 3), intent(in) :: hess
        ...
        !     Local variables.
        !
        complex(kind=realType) :: determinant
    ...
    subroutine hessD2Hexa(xP, x1, x2, x3, x4, x5, x6, x7, x8, chi, hess, iErr)
       ...
       use complexify 
        ...
        !     Subroutine arguments.
        !
        complex(kind=realType), dimension(3), intent(in) :: xP, chi
        complex(kind=realType), dimension(3), intent(in) :: x1, x2, x3, x4
        complex(kind=realType), dimension(3), intent(in) :: x5, x6, x7, x8
        complex(kind=realType), dimension(3, 3), intent(out) :: hess
        ...
        complex(kind=realType) :: ksi, eta, zeta, x0, y0, z0
        complex(kind=realType), dimension(8, 3) :: alpha
        complex(kind=realType) :: dxdksi, dxdeta, dxdzeta
        complex(kind=realType) :: dydksi, dydeta, dydzeta
        complex(kind=realType) :: dzdksi, dzdeta, dzdzeta
        complex(kind=realType) :: d2xdksideta, d2xdksidzeta, d2xdetadzeta
        complex(kind=realType) :: d2ydksideta, d2ydksidzeta, d2ydetadzeta
        complex(kind=realType) :: d2zdksideta, d2zdksidzeta, d2zdetadzeta
    ...
    subroutine gradD2Hexa(xP, x1, x2, x3, x4, x5, x6, x7, x8, chi, x0, y0, z0, grad, iErr)
       ...
       use complexify 
        ...
        !     Subroutine arguments.
        !
        complex(kind=realType), intent(out) :: x0, y0, z0
        complex(kind=realType), dimension(3), intent(in) :: xP, chi
        complex(kind=realType), dimension(3), intent(in) :: x1, x2, x3, x4
        complex(kind=realType), dimension(3), intent(in) :: x5, x6, x7, x8
        complex(kind=realType), dimension(3), intent(out) :: grad
        ...
        complex(kind=realType) :: ksi, eta, zeta
        complex(kind=realType), dimension(8, 3) :: alpha
        complex(kind=realType) :: dxdksi, dxdeta, dxdzeta
        complex(kind=realType) :: dydksi, dydeta, dydzeta
        complex(kind=realType) :: dzdksi, dzdeta, dzdzeta

...
contains
    ...
    subroutine buildADT(ADT)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(3, 2) :: rootLeafBBox
        complex(kind=realType), dimension(3, 2, 0:ADT%nProcs - 1) :: &
            rootLeavesBBox
        ...
        do i = 0, (ADT%nProcs - 1)
            if (tmpArr(i) > 0) then
                ...
                if (ADT%myID .ceq. i) ADT%myEntryInRootProcs = ii
            end if
        end do
    ...
    subroutine buildSurfaceADT(nTria, nQuads, nNodes, &
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(3, 2), intent(in) :: BBox

        complex(kind=realType), dimension(:, :), intent(in), &
            target :: coor
        !
        !       Local variables.
        !
        ...
        complex(kind=realType), dimension(3) :: xMin, xMax

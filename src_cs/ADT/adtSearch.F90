...
contains
    ...
    subroutine containmentSearch(nCoor, coor, &
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(:, :), intent(out) :: arrInterpol
        !
        !       Local variables.
        !
        integer :: ierr
        ...
        complex(kind=realType), dimension(1) :: dummy
        ...
        do jj = 1, nAlloc
            if (adtID .ceq. ADTs(jj)%adtID) exit
        end do
        ...
        if (ADT%adtType .cne. adtVolumeADT) then
            if (ADT%myID .ceq. 0) &
    ...
    subroutine failSafeSearch(nCoor, coor, &
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(:), intent(inout) :: dist2
        !
        !       Local variables.
        !
        integer :: ierr
        ...
        complex(kind=realType), dimension(:), allocatable :: tmpDist2
        complex(kind=realType), dimension(:, :), allocatable :: tmpCoor
        complex(kind=realType), dimension(:, :), allocatable :: tmpUVW
        complex(kind=realType), dimension(:, :), allocatable :: tmpArrInt
    ...
    subroutine initSearch(nCoor, coor, dist2, ADT, containmentSearch)
       ...
       use complexify 
        ...
        complex(kind=realType) :: d1, d2, dx, dy, dz
    ...
    subroutine search(nCoor, coor, procID, &
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(:, :), allocatable :: coorBuf
        complex(kind=realType), dimension(:, :), allocatable :: coorRecv
        complex(kind=realType), dimension(:, :), allocatable :: uvwRecv
        complex(kind=realType), dimension(:, :), allocatable :: uvwBuf
        ...
        outerLoop: do mm = 1, nRounds
            ...
            do i = startProcRecv, nProcRecv
                ...
                if (nCoorRecv .ceq. nCoorMax) exit
            ...
            sendCoorLoop: do i = 1, nProcSendCur
                ...
                test1Containment: if (containmentSearch) then
                    ...
                    do
                        ...
                        if ((j .ceq. nCoorPerProc(procCur)) .or. &
                            (nn .ceq. nCoorPerRootLeaf(procCur + 1))) exit
            ...
            test2Containment: if (containmentSearch) then
                ...
                do
                    ...
                    if ((j .ceq. nn) .or. (i .ceq. nCoorPerRootLeaf(myID + 1))) exit
            ...
            recvSendLoop: do i = 1, nProcRecvCur
                ...
                if (debug) then
                    if ((sizeMessage .ceq. mpi_undefined) .or. &
                        (mod(sizeMessage, nVarCoor) .cne. 0)) &
                        call adtTerminate(ADT, "search", &

module blockette
    ...
    ! Double halos
    complex(kind=realType), dimension(0:bbib, 0:bbjb, 0:bbkb, 1:6) :: w
    complex(kind=realType), dimension(0:bbib, 0:bbjb, 0:bbkb) :: P, gamma
    complex(kind=realType), dimension(0:bbib, 0:bbjb, 0:bbkb) :: ss ! Entropy

    ! Single halos
    complex(kind=realType), dimension(0:bbie, 0:bbje, 0:bbke, 3) :: x
    complex(kind=realType), dimension(1:bbie, 1:bbje, 1:bbke) :: rlv, rev, vol, aa
    complex(kind=realType), dimension(1:bbie, 1:bbje, 1:bbke) :: radI, radJ, radK, dtl
    complex(kind=realType), dimension(1:bbie, 1:bbje, 1:bbke, 3) :: dss ! Shock sensor

    ! No halos
    complex(kind=realType), dimension(2:bbil, 2:bbjl, 2:bbkl) :: volRef, d2wall
    ...
    complex(kind=realType), dimension(1:bbie, 1:bbje, 1:bbke, 1:5) :: fw
    complex(kind=realType), dimension(1:bbie, 1:bbje, 1:bbke, 1:6) :: dw

    ! Face projected areas
    complex(kind=realType), dimension(0:bbie, 1:bbje, 1:bbke, 3) :: sI
    complex(kind=realType), dimension(1:bbie, 0:bbje, 1:bbke, 3) :: sJ
    complex(kind=realType), dimension(1:bbie, 1:bbje, 0:bbke, 3) :: sK

    ! Face velocities
    complex(kind=realType), dimension(0:bbie, 1:bbje, 1:bbke) :: sFaceI
    complex(kind=realType), dimension(1:bbie, 0:bbje, 1:bbke) :: sFaceJ
    complex(kind=realType), dimension(1:bbie, 1:bbje, 0:bbke) :: sFaceK

    ! Nodal gradients
    complex(kind=realType), dimension(1:bbil, 1:bbjl, 1:bbkl) :: ux, uy, uz
    complex(kind=realType), dimension(1:bbil, 1:bbjl, 1:bbkl) :: vx, vy, vz
    complex(kind=realType), dimension(1:bbil, 1:bbjl, 1:bbkl) :: wx, wy, wz
    complex(kind=realType), dimension(1:bbil, 1:bbjl, 1:bbkl) :: qx, qy, qz
...
contains
    ...
    subroutine blocketteRes(useDissApprox, useViscApprox, useUpdateIntermed, useFlowRes, useTurbRes, useSpatial, &
       ...
       use complexify 
        ...
        complex(kind=realType), optional, dimension(:, :), intent(out) :: funcValues
        character, optional, dimension(:, :), intent(in) :: bcDataNames
        complex(kind=realType), optional, dimension(:), intent(in) :: bcDataValues
        integer(kind=intType), optional, dimension(:, :) :: bcDataFamLists
        complex(kind=realType), intent(out), optional, dimension(:, :, :) :: forces
        ...
        if (spatial) then
            ...
            do sps = 1, nTimeIntervalsSpectral
                ...
                if (oversetPresent .and. (oversetUpdateMode .ceq. updateFast)) then
        ...
        do sps = 1, nTimeIntervalsSpectral
            do nn = 1, nDom
                ...
                if (spatial) then
                    ...
                    if ((equations .ceq. RANSEquations) .and. useApproxWallDistance) then
                ...
                ! correct for K
                if ((equations .ceq. RANSEquations) .and. turbRes) then
        ...
        do kk = 2, bkl, BS
            do jj = 2, bjl, BS
                do ii = 2, bil, BS
                    ...
                    firstBlockette: if (ii .ceq. 2) then
                    ...
                    do k = 2, kl
                        do j = 2, jl
                            do i = 2, il
                                ...
                                if (equations .ceq. ransequations) &
    ...
    subroutine metrics
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(3) :: v1, v2
        complex(kind=realType) :: fact
    ...
    subroutine saSource
       ...
       use complexify 
        ...
        complex(kind=realType) :: fv1, fv2, ft2
        complex(kind=realType) :: sst, nu, dist2Inv, chi, chi2, chi3
        complex(kind=realType) :: rr, gg, gg6, termFw, fwSa, term1, term2
        complex(kind=realType) :: dfv1, dfv2, dft2, drr, dgg, dfw, sqrtProd
        complex(kind=realType) :: uux, uuy, uuz, vvx, vvy, vvz, wwx, wwy, wwz
        complex(kind=realType) :: div2, fact, sxx, syy, szz, sxy, sxz, syz
        complex(kind=realType) :: vortx, vorty, vortz
        complex(kind=realType) :: omegax, omegay, omegaz
        complex(kind=realType) :: strainMag2, prod
        complex(kind=realType), parameter :: xminn = 1.e-10_realType
        complex(kind=realType), parameter :: f23 = two * third
        integer(kind=intType) :: i, j, k
        complex(kind=realType) :: term1Fact
    ...
    subroutine saViscous
       ...
       use complexify 
        ...
        complex(kind=realType) :: voli, volmi, volpi, xm, ym, zm, xp, yp, zp
        complex(kind=realType) :: xa, ya, za, ttm, ttp, cnud, cam, cap
        complex(kind=realType) :: nutm, nutp, num, nup, cdm, cdp
        complex(kind=realType) :: c1m, c1p, c10, b1, c1, d1, qs, nu
        integer(Kind=intType) :: i, j, k
    ...
    subroutine saAdvection
       ...
       use complexify 
        ...
        complex(kind=realType) :: uu, dwt, dwtm1, dwtp1, dwti, dwtj, dwtk, qs
        complex(kind=realType) :: voli, xa, ya, za
        ...
        secondOrd = .false.
        if ((groundLevel .ceq. 1_intType) .and. &
            (orderTurb .ceq. secondOrder)) secondOrd = .true.
    ...
    subroutine timeStep(updateDtl)
       ...
       use complexify 
        ...
        ! Local parameters.
        complex(kind=realType), parameter :: b = 2.0_realType

        ! Variables for spectral Radius
        complex(kind=realType) :: plim, rlim, clim2
        complex(kind=realType) :: cc2, qsi, qsj, qsk, sx, sy, sz, rmu
        complex(kind=realType) :: ri, rj, rk, rij, rjk, rki
        complex(kind=realType) :: vsi, vsj, vsk, rfl, dpi, dpj, dpk
        complex(kind=realType) :: sFace, tmp, uux, uuy, uuz
        ...
        if (updateDt) then
            ...
            if (equationMode .ceq. timeSpectral) then
    ...
    subroutine inviscidCentralFlux
       ...
       use complexify 
        ...
        complex(kind=realType) :: qsp, qsm, rqsp, rqsm, porVel, porFlux
        complex(kind=realType) :: pa, vnp, vnm, fs, sFace
        integer(kind=intType) :: i, j, k
        complex(kind=realType) :: wwx, wwy, wwz, rvol
        ...
        do k = 2, kl
            do j = 2, jl
                do i = 1, il
                    ...
                    porVel = one
                    porFlux = half
                    if (porI(i, j, k) .ceq. noFlux) porFlux = zero
                    if (porI(i, j, k) .ceq. boundFlux) then
                        porVel = zero
                        vnp = sFace
                        vnm = sFace
                    end if
        ...
        do k = 2, kl
            do j = 1, jl
                do i = 2, il
                    ...
                    porVel = one
                    porFlux = half
                    if (porJ(i, j, k) .ceq. noFlux) porFlux = zero
                    if (porJ(i, j, k) .ceq. boundFlux) then
                        porVel = zero
                        vnp = sFace
                        vnm = sFace
                    end if
        ...
        do k = 1, kl
            do j = 2, jl
                do i = 2, il
                    ...
                    if (porK(i, j, k) .ceq. noFlux) porFlux = zero
                    if (porK(i, j, k) .ceq. boundFlux) then
                        porVel = zero
                        vnp = sFace
                        vnm = sFace
                    end if
        ...
        rotation: if (blockIsMoving .and. (equationMode .ceq. steady)) then
    ...
    subroutine inviscidDissFluxMatrix
       ...
       use complexify 
        ...
        !      Local parameters.
        !
        complex(kind=realType), parameter :: dpMax = 0.25_realType
        complex(kind=realType), parameter :: epsAcoustic = 0.25_realType
        complex(kind=realType), parameter :: epsShear = 0.025_realType
        complex(kind=realType), parameter :: omega = 0.5_realType
        complex(kind=realType), parameter :: oneMinOmega = one - omega
        !
        !      Local variables.
        !
        integer(kind=intType) :: i, j, k, ind, ii

        complex(kind=realType) :: plim, sface
        complex(kind=realType) :: sfil, fis2, fis4
        complex(kind=realType) :: gammaAvg, gm1, ovgm1, gm53
        complex(kind=realType) :: ppor, rrad, dis2, dis4
        complex(kind=realType) :: dp1, dp2, tmp, fs
        complex(kind=realType) :: ddw1, ddw2, ddw3, ddw4, ddw5, ddw6
        complex(kind=realType) :: dr, dru, drv, drw, dre, drk, sx, sy, sz
        complex(kind=realType) :: uAvg, vAvg, wAvg, a2Avg, aAvg, hAvg
        complex(kind=realType) :: alphaAvg, unAvg, ovaAvg, ova2Avg
        complex(kind=realType) :: kAvg, lam1, lam2, lam3, area
        complex(kind=realType) :: abv1, abv2, abv3, abv4, abv5, abv6, abv7
        logical :: correctForK
        ...
        do k = 2, kl
            do j = 2, jl
                do i = 1, il
                    ...
                    ppor = zero
                    if (porI(i, j, k) .ceq. normalFlux) ppor = one
        ...
        do k = 2, kl
            do j = 1, jl
                do i = 2, il
                    ...
                    ppor = zero
                    if (porJ(i, j, k) .ceq. normalFlux) ppor = one
        ...
        do k = 1, kl
            do j = 2, jl
                do i = 2, il
                    ...
                    ppor = zero
                    if (porK(i, j, k) .ceq. normalFlux) ppor = one
    ...
    subroutine inviscidDissFluxScalar
       ...
       use complexify 
        ...
        complex(kind=realType), parameter :: dssMax = 0.25_realType
        complex(kind=realType) :: sslim, rhoi
        complex(kind=realType) :: sfil, fis2, fis4
        complex(kind=realType) :: ppor, rrad, dis2, dis4, fs
        complex(kind=realType) :: ddw1, ddw2, ddw3, ddw4, ddw5
        integer(kind=intType) :: i, j, k
        ...
        if (useDissContinuation) then
            if ((totalR .ceq. zero) .or. (totalR0 .ceq. zero)) then
        ...
        do k = 2, kl
            do j = 2, jl
                do i = 1, il
                    ...
                    ppor = zero
                    if (porI(i, j, k) .ceq. normalFlux) ppor = half
        ...
        do k = 2, kl
            do j = 1, jl
                do i = 2, il
                    ...
                    ppor = zero
                    if (porJ(i, j, k) .ceq. normalFlux) ppor = half
        ...
        do k = 1, kl
            do j = 2, jl
                do i = 2, il
                    ...
                    ppor = zero
                    if (porK(i, j, k) .ceq. normalFlux) ppor = half
    ...
    subroutine inviscidUpwindFlux(fineGrid)
       ...
       use complexify 
        ...
        complex(kind=realType) :: sx, sy, sz, omk, opk, sFil, gammaFace
        complex(kind=realType) :: factMinmod, sFace

        complex(kind=realType), dimension(nw) :: left, right
        complex(kind=realType), dimension(nw) :: du1, du2, du3
        complex(kind=realType), dimension(nwf) :: flux
        ...
        if (correctForK) then
            if (orderTurb .ceq. firstOrder) then
                nwInt = nwf
                firstOrderK = .true.
            else
                nwInt = itu1
        ...
        !       scheme.
        !
        orderTest: if (limUsed .ceq. firstOrder) then
            !
    ...
    contains
       ...
       use complexify 
            ...
            !        Local parameter.
            !
            complex(kind=realType), parameter :: epsLim = 1.e-10_realType
            !
            !        Subroutine arguments.
            !
            complex(kind=realType), dimension(:), intent(inout) :: du1, du2, du3
            complex(kind=realType), dimension(:), intent(out) :: left, right

            complex(kind=realType), dimension(:, :, :, :, :), pointer :: rotMatrix
            !
            !        Local variables.
            !
            integer(kind=intType) :: l

            complex(kind=realType) :: rl1, rl2, rr1, rr2, tmp, dvx, dvy, dvz

            complex(kind=realType), dimension(3, 3) :: rot
       ...
       use complexify 
            ...
            !        Subroutine arguments.
            !
            complex(kind=realType), dimension(*), intent(in) :: left, right
            complex(kind=realType), dimension(*), intent(out) :: flux
            !
            !        Local variables.
            !
            complex(kind=realType) :: porFlux, rFace
            complex(kind=realType) :: Etl, Etr, z1l, z1r, tmp
            complex(kind=realType) :: dr, dru, drv, drw, drE, drk
            complex(kind=realType) :: rAvg, uAvg, vAvg, wAvg, hAvg, kAvg
            complex(kind=realType) :: alphaAvg, a2Avg, aAvg, unAvg
            complex(kind=realType) :: ovaAvg, ova2Avg, area, Eta
            complex(kind=realType) :: gm1, gm53
            complex(kind=realType) :: lam1, lam2, lam3
            complex(kind=realType) :: abv1, abv2, abv3, abv4, abv5, abv6, abv7
            complex(kind=realType), dimension(2) :: ktmp

            ! Set the porosity for the flux. The default value, 0.5*rFil, is
            ! a scaling factor where an rFil != 1 is taken into account.

            porFlux = half * rFil
            if ((por .ceq. noFlux) .or. (por .ceq. boundFlux)) porFlux = zero
            ...
            case (Roe)
                ...
                case (noPrecond)
                    ...
                    if (por .ceq. boundFlux) unAvg = rFace
    ...
    subroutine inviscidDissFluxMatrixApprox
       ...
       use complexify 
        ...
        complex(kind=realType) :: ppor, rrad, dis2, dis4
        complex(kind=realType) :: dp1, dp2, tmp, fs
        complex(kind=realType) :: ddw, ddw6
        complex(kind=realType) :: dr, dru, drv, drw, dre, drk, sx, sy, sz
        complex(kind=realType) :: uAvg, vAvg, wAvg, a2Avg, aAvg, hAvg
        complex(kind=realType) :: alphaAvg, unAvg, ovaAvg, ova2Avg
        complex(kind=realType) :: kAvg, lam1, lam2, lam3, area
    ...
    subroutine computeSpeedOfSoundSquared
       ...
       use complexify 
        ...
        logical :: correctForK
        complex(kind=realType) :: pp
        complex(kind=realType), parameter :: twoThird = two * third
        integer(kind=intType) :: i, j, k
    ...
    subroutine allNodalGradients
       ...
       use complexify 
        ...
        complex(kind=realType) :: a2, oVol, uBar, vBar, wBar, sx, sy, sz
        integer(kind=intType) :: i, j, k
    ...
    subroutine viscousFlux(storeWallTensor)
       ...
       use complexify 
        ...
        complex(kind=realType) :: rFilv, por, mul, mue, mut, heatCoef
        complex(kind=realType) :: gm1, factLamHeat, factTurbHeat
        complex(kind=realType) :: u_x, u_y, u_z, v_x, v_y, v_z, w_x, w_y, w_z
        complex(kind=realType) :: q_x, q_y, q_z
        complex(kind=realType) :: corr, ssx, ssy, ssz, fracDiv, snrm
        complex(kind=realType) :: tauxx, tauyy, tauzz
        complex(kind=realType) :: tauxy, tauxz, tauyz
        complex(kind=realType) :: tauxxS, tauyyS, tauzzS
        complex(kind=realType) :: tauxyS, tauxzS, tauyzS
        complex(kind=realType) :: ubar, vbar, wbar
        complex(kind=realType) :: exx, eyy, ezz
        complex(kind=realType) :: exy, exz, eyz
        complex(kind=realType) :: Wxx, Wyy, Wzz
        complex(kind=realType) :: Wxy, Wxz, Wyz, Wyx, Wzx, Wzy
        complex(kind=realType) :: den, Ccr1
        complex(kind=realType) :: fmx, fmy, fmz, frhoE, fact
        integer(kind=intType) :: i, j, k, io, jo, ko
        complex(kind=realType), parameter :: xminn = 1.e-10_realType
        complex(kind=realType), parameter :: twoThird = two * third
        complex(kind=realType), dimension(9, 2:max(il, jl), 2:max(jl, kl), 2) :: tmpStore
        ...
        do k = 1, kl
            do j = 2, jl
                do i = 2, il
                    ...
                    por = half * rFilv
                    if (porK(i, j, k) .ceq. noFlux) por = zero
        ...
        if (storeWall) then
            ...
            origKMax: if (kk + nz - 1 .ceq. bkl) then
                do j = 2, jl
                    do i = 2, il
                        io = i + ii - 2
                        jo = j + jj - 2
        ...
        do k = 2, kl
            do j = 1, jl
                do i = 2, il
                    ...
                    por = half * rFilv
                    if (porJ(i, j, k) .ceq. noFlux) por = zero
        ...
        if (storeWall) then
            ...
            origJMax: if (jj + ny - 1 .ceq. bjl) then
                do k = 2, kl
                    do i = 2, il
                        io = i + ii - 2
                        ko = k + kk - 2
        ...
        do k = 2, kl
            do j = 2, jl
                do i = 1, il
                    ...
                    por = half * rFilv
                    if (porI(i, j, k) .ceq. noFlux) por = zero
        ...
        if (storeWall) then
            ...
            origIMax: if (ii + nx - 1 .ceq. bil) then
                do k = 2, kl
                    do j = 2, jl
                        jo = j + jj - 2
                        ko = k + kk - 2
    ...
    subroutine viscousFluxApprox
       ...
       use complexify 
        ...
        complex(kind=realType) :: gm1, factLamHeat, factTurbHeat
        complex(kind=realType) :: u_x, u_y, u_z, v_x, v_y, v_z, w_x, w_y, w_z
        complex(kind=realType) :: q_x, q_y, q_z, ubar, vbar, wbar
        complex(kind=realType) :: corr, ssx, ssy, ssz, ss, fracDiv
        complex(kind=realType) :: tauxx, tauyy, tauzz
        complex(kind=realType) :: tauxy, tauxz, tauyz
        complex(kind=realType) :: fmx, fmy, fmz, frhoE
        complex(kind=realType) :: dd
        logical :: correctForK
    ...
    subroutine sumDwandFw
       ...
       use complexify 
        ...
        complex(kind=realType) :: oVol, rBlank

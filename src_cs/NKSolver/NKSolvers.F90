...
#include <petsc/finclude/petsc.h>
    ...
    logical :: NK_ADPC
    logical :: NK_viscPC
    complex(kind=realType) :: NK_CFL0
    complex(kind=realType) :: NK_switchTol
    complex(kind=realType) :: NK_rtolInit
    complex(kind=realType) :: NK_divTol = 10
    complex(kind=realType) :: NK_fixedStep
    ...
    complex(kind=realType) :: ew_rtol_0
    complex(kind=realType) :: ew_rtol_max
    complex(kind=realType) :: ew_gamma
    complex(kind=realType) :: ew_alpha
    complex(kind=realType) :: ew_alpha2
    complex(kind=realType) :: ew_threshold
    real(kind=alwaysRealType) :: rtolLast, oldNorm

    ! Misc Parameters
    logical :: freeStreamResSet = .False.
    complex(kind=realType) :: NK_CFL

    ! Variables for non-monotone line search
    complex(kind=realType), dimension(:), allocatable :: NKLSFuncEvals
...
contains
    ...
    subroutine NKMatMult(A, vecX, vecY, ierr)
       ...
       use complexify 
        ...
        complex(kind=realType) :: dt
        complex(kind=realType), pointer :: yPtr(:), xPtr(:)
    ...
    subroutine getFreeStreamResidual(rhoRes, totalRRes)
       ...
       use complexify 
        ...
        complex(kind=realType), intent(out) :: rhoRes, totalRRes
        complex(kind=realType), dimension(:), allocatable :: tmp
    ...
    subroutine FormJacobianNK
       ...
       use complexify 
        ...
        if (NK_precondType .ceq. 'asm') then
    ...
    subroutine NKStep(firstCall)
       ...
       use complexify 
        ...
        if (mod(NK_iter, NK_jacobianLag) .ceq. 0) then
        ...
        if ((NK_iter .ceq. 0) .or. .not. NK_useEW) then
            rtol = NK_rtolInit
        else
    ...
    subroutine LSCubic(x, f, g, y, w, fnorm, ynorm, gnorm, nfevals, flag, lambda)
       ...
       use complexify 
        ...
        complex(kind=realType) :: initslope, lambdaprev, gnormprev, a, b, d, t1, t2
        ...
        if (initslope .ceq. 0.0_realType) then
...
100     continue
    ...
    subroutine LSNM(x, f, g, y, w, fnorm, ynorm, gnorm, nfevals, flag, step)
       ...
       use complexify 
        ...
        !   where z(x) = .5 * fnorm*fnorm, and fnorm = || f ||_2.
        !
        complex(kind=realType) :: initslope, gamma, sigma, max_val
        ...
        backtrack: do iter = 1, 10
            ...
            call VecNorm(g, NORM_2, gnorm, ierr)
            if (ierr .ceq. PETSC_ERR_FP) then ! Error code 72 floating point error
    ...
    subroutine applyPC(in_vec, out_vec, ndof)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(ndof), intent(in) :: in_vec
        complex(kind=realTYpe), dimension(ndof), intent(inout) :: out_vec
    ...
    subroutine setWVec(wVec)
       ...
       use complexify 
        ...
        complex(kind=realType), pointer :: wvec_pointer(:)
    ...
    subroutine setRVec(rVec, flowRes, turbRes, totalRes)
       ...
       use complexify 
        ...
        Vec rVec
        integer(kind=intType) :: ierr, nn, sps, i, j, k, l, ii
        complex(kind=realType), pointer :: rvec_pointer(:)
        complex(Kind=realType) :: ovv
        real(kind=alwaysRealType), intent(out), optional :: flowRes, turbRes, totalRes
        complex(kind=realType) :: tmp, tmp2(2), flowResLocal, turbResLocal
    ...
    subroutine getStates(states, ndimw)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(ndimw), intent(out) :: states(ndimw)
    ...
    subroutine getRes(res, ndimw)
       ...
       use complexify 
        ...
        integer(kind=intType), intent(in) :: ndimw
        complex(kind=realType), dimension(ndimw), intent(inout) :: res(ndimw)
    ...
    subroutine setStates(states, ndimw)
       ...
       use complexify 
        ...
        integer(kind=intType), intent(in) :: ndimw
        complex(kind=realType), dimension(ndimw), intent(in) :: states(ndimw)
    ...
    subroutine setInfo(info, iSize)
       ...
       use complexify 
        ...
        complex(kind=realType), intent(in), dimension(iSize) :: info
    ...
    subroutine getInfo(info, iSize)
       ...
       use complexify 
        ...
        integer(kind=intType), intent(in) :: iSize
        complex(kind=realType), intent(out), dimension(iSize) :: info
        integer(kind=intType) :: nn, counter, i, j, k, l, sps
        ! Determine the size of a flat array needed to store w, P, ( and
        ! rlv, rev if necessary) with full double halos.
        counter = 0
...
#include <petsc/finclude/petsc.h>
    ...
    complex(kind=realType) :: ANK_rtol
    complex(kind=realType) :: ANK_atol_buffer
    complex(kind=realType) :: ANK_linResMax
    complex(kind=realType) :: ANK_switchTol
    complex(kind=realType) :: ANK_divTol = 10
    logical :: ANK_useTurbDADI
    logical :: ANK_useApproxSA
    complex(kind=realType) :: ANK_turbcflscale
    ...
    ! Misc variables
    complex(kind=realType) :: ANK_CFL, ANK_CFL0, ANK_CFLLimit, ANK_CFLFactor, ANK_CFLCutback
    complex(kind=realType) :: ANK_CFLMin0, ANK_CFLMin, ANK_CFLMinBase, ANK_CFLExponent
    complex(kind=realType) :: ANK_stepMin, ANK_StepFactor, ANK_constCFLStep
    complex(kind=realType) :: ANK_secondOrdSwitchTol, ANK_coupledSwitchTol
    complex(kind=realType) :: ANK_physLSTol, ANK_unstdyLSTol
    complex(kind=realType) :: ANK_pcUpdateTol, ANK_pcUpdateTol2
    complex(kind=realType) :: ANK_pcUpdateCutoff
    complex(kind=realType) :: lambda
    ...
    complex(kind=realType) :: lambdaTurb, ANK_physLSTolTurb
...
contains
    ...
    subroutine setupANKsolver
       ...
       use complexify 
        ...
        if (.not. ANK_solverSetup) then
            ...
            if (ANK_precondType .ceq. 'mg') then
            ...
            if ((.not. ANK_coupled) .and. (.not. ANK_useTurbDADI) .and. (equations .ceq. RANSEquations)) then
    ...
    subroutine FormJacobianANK
       ...
       use complexify 
        ...
        complex(kind=realType) :: dtinv, rho
        ...
        ! Set up the KSP using the same code as used for the adjoint
        if (ANK_precondType .ceq. 'asm') then
        ...
        else if (ANK_precondType .ceq. 'mg') then
    ...
    subroutine computeTimeStepMat(usePC)
       ...
       use complexify 
        ...
        integer(kind=intType), dimension(2:10) :: coarseRows
        complex(kind=realType), dimension(nState, nState) :: timeStepBlock
        logical :: useCoarseMats
    ...
    subroutine computeTimeStepBlock(i, j, k, timeStepBlock)
       ...
       use complexify 
        ...
        ! Output variables
        complex(kind=realType), dimension(nState, nState), intent(out) :: timeStepBlock

        ! Local variables
        complex(kind=realType) :: blendFactor, dtInv, rho, velX, velY, velZ
        complex(kind=realType) :: speed, speedOfSound, mach, machSqr, machSqrTrunc, alpha, beta, tau, gammaMinusOne
        complex(kind=realType) :: speedXY, sinTheta, cosTheta, sinAlpha, cosAlpha
        complex(kind=realType), dimension(nState, nState) :: streamToCart, symmToCons, consToSymm, stateToCons
        ...
        if (ANK_charTimeStepType .ceq. 'None') then
            ...
            if (ANK_charTimeStepType .ceq. 'VLR') then
            ...
            else if (ANK_charTimeStepType .ceq. 'Turkel') then
    ...
    subroutine FormFunction_mf(ctx, inVec, rVec, ierr)
       ...
       use complexify 
        ...
        complex(kind=realType), pointer :: rvec_pointer(:)
        complex(kind=realType), pointer :: invec_pointer(:)
        complex(kind=realType), pointer :: wvec_pointer(:)
    ...
    subroutine computeUnsteadyResANK(omega)
       ...
       use complexify 
        ...
        complex(kind=realType), intent(in) :: omega

        complex(kind=realType) :: dtinv, rho, uu, vv, ww
        integer(kind=intType) :: ierr, nn, sps, i, j, k, l, ii, iiRho
        complex(kind=realType), pointer :: rvec_pointer(:)
        complex(kind=realType), pointer :: dvec_pointer(:)

        complex(kind=realType), dimension(nState, nState) :: timeStepMatrix
        complex(kind=realType), dimension(nState) :: wPrev
    ...
    subroutine physicalityCheckANK(lambdaP)
       ...
       use complexify 
        ...
        ! input variable
        complex(kind=realType), intent(inout) :: lambdaP
    ...
    subroutine ANKTurbSolveKSP
       ...
       use complexify 
        ...
        complex(kind=realType) :: atol, val, v2, factK, gm1
        ...
        do n = 1, ANK_nsubIterTurb
            ...
            if (mod(ANK_iterTurb, ANK_jacobianLag) .ceq. 0) then

                ! Actually form the preconditioner and factorize it.
                if ((myid .ceq. 0) .and. ANK_turbDebug) &
            ...
            if ((myid .ceq. 0) .and. ANK_turbDebug) then
       ...
       use complexify 
        ...
        if (firstCall .or. &
            ((totalR .le. ANK_coupledSwitchTol * totalR0) .and. (.not. ANK_coupled) &
             .and. ((equations .ceq. RANSEquations)))) then
            ...
            if (firstCall) then
                ...
                if (totalR .le. ANK_coupledSwitchTol * totalR0 .and. (equations .ceq. RANSEquations)) then
                    ANK_coupled = .True.
                else
                    ANK_coupled = .False.
                end if
        ...
        ! Determine if if we need to form the Preconditioner
        if ((mod(ANK_iter, ANK_jacobianLag) .ceq. 0) .or. totalR / totalR_pcUpdate < rel_pcUpdateTol) then
        ...
        ! ============== Turb Update =============
        if ((.not. ANK_coupled) .and. (equations .ceq. RANSEquations) .and. lambda > zero) then

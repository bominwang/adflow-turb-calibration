subroutine fringeSearch(oBlock, oFringe)
    ...
    complex(kind=realType) :: uu, vv, ww, err1, err2
    complex(kind=realType) :: uvw(5), uvw2(5), xx(4), pt(3), xcheck(3)
    complex(kind=realType), dimension(:, :), allocatable :: offset
    complex(kind=realType) :: oneMinusU, oneMinusV, oneMinusW, weight(8)
    ...
    do i = 1, n
        ...
        elemFound: if (intInfo(1) >= 0) then
            ...
            do kkk = 0, 1
                do jjj = 0, 1
                    do iii = 0, 1
                        if (oBlock%invalidDonor(ii + iii, jj + jjj, kk + kkk) .cne. 0) then
                            invalid = .True.
                        end if
                    end do
                end do
            ...
            if ((oFringe%isWall(i) > 0) .and. .not. ((oBlock%nearWall(ii, jj, kk) .ceq. 1))) then

subroutine surfaceCorrection(oBlock, oFringe, offset, n)
    ...
    complex(kind=realType), intent(out), dimension(3, n) :: offset
    ...
    complex(kind=realType), dimension(5) :: uvwF, uvwB
    complex(kind=realType), dimension(3) :: ptB, ptF, yy
    complex(kind=realType), dimension(4) :: weightsF, weightsB, xx
    complex(kind=realType) :: ratio, fact, distY, q1(3, 4), q2(3, 4), dB
    ...
    complex(kind=realType), dimension(:, :), allocatable :: uniqueWallPts, masterOffset
    ...
    if ((bWall%nNodes .ceq. 0) .or. (fWall%nNodes .ceq. 0)) then
...
contains
    subroutine getWeights(uv, weights)
       ...
       use complexify 
        ...
        complex(kind=realType), intent(in) :: uv(2)
        complex(kind=realType), intent(out) :: weights(4)

subroutine floodInteriorCells(level, sps)
    ...
    parallelSyncLoop: do
        ...
        do nn = 1, nDom
            ...
            if (loopIter .ceq. 1) then
                ...
                do k = 2, kl
                    do j = 2, jl
                        do i = 2, il
                            if ((iblank(i, j, k) .ceq. -3) .or. (iblank(i, j, k) .ceq. -2)) then
                ...
                do k = 2, kl
                    do j = 2, jl
                        if (changed(1 + 1, j + 1, k + 1) .ceq. 1) then
                            call addSeed(2, j, k)
                        end if
                        if (changed(ie + 1, j + 1, k + 1) .ceq. 1) then
                            call addSeed(il, j, k)
                        end if
                    end do
                end do
                ...
                do k = 2, kl
                    do i = 2, il
                        if (changed(i + 1, 1 + 1, k + 1) .ceq. 1) then
                            call addSeed(i, 2, k)
                        end if
                        if (changed(i + 1, je + 1, k + 1) .ceq. 1) then
                            call addSeed(i, jl, k)
                        end if
                    end do
                end do
                ...
                do j = 2, jl
                    do i = 2, il
                        if (changed(i + 1, j + 1, 1 + 1) .ceq. 1) then
                            call addSeed(i, j, 2)
                        end if
                        if (changed(i + 1, j + 1, ke + 1) .ceq. 1) then
                            call addSeed(i, j, kl)
                        end if
                    end do
                end do
        ...
        if (((nChanged .ceq. 0)) .or. ((loopIter .ceq. nFloodIter))) then

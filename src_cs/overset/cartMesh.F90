...
#include <petsc/finclude/petsc.h>
        ...
        complex(kind=realType), dimension(:), pointer :: cartPointer
        complex(kind=realType), dimension(:, :, :), pointer :: xx
        complex(kind=realType), dimension(:, :, :), pointer :: arrVals, changed
        complex(kind=realType), dimension(:), allocatable :: values
        complex(kind=realType), dimension(3) :: xMinLocal, xMaxLocal, xMin, xMax, sss
        complex(kind=realType), dimension(3) :: pt1, pt2, pt3, pt4, newPt, v1, v2
        complex(kind=realType) :: areaLocal, area, areaAvg, err1, err2, h
        complex(kind=realType) :: coorAvg, scaleSize, length, u, v
        ...
        do iDim = 1, 3
            if (symOnFace(2 * iDim - 1) .ceq. 0) then
                xMin(iDim) = xMin(iDim) - 2 * h
            end if

            if (symOnFace(2 * iDim) .ceq. 0) then
        ...
        parallelSyncLoop: do
            ...
            if (loopIter .ceq. 1) then
                ...
                do k = kBeg + 1, kEnd - 1
                    do j = jBeg + 1, jEnd - 1
                        if (int(changed(iBeg, j, k)) .ceq. 1) then
                            call addSeed(iBeg + 1, j, k)
                        end if
                        if (int(changed(iEnd, j, k)) .ceq. 1) then
                ...
                do k = kBeg + 1, kEnd - 1
                    do i = iBeg + 1, iEnd - 1
                        if (int(changed(i, jBeg, k)) .ceq. 1) then
                            call addSeed(i, jBeg + 1, k)
                        end if
                        if (int(changed(i, jEnd, k)) .ceq. 1) then
                ...
                do j = jBeg + 1, jEnd - 1
                    do i = iBeg + 1, iEnd - 1
                        if (int(changed(i, j, kBeg)) .ceq. 1) then
                            call addSeed(i, j, kBeg + 1)
                        end if
                        if (int(changed(i, j, kEnd)) .ceq. 1) then
            ...
            do iSeed = 1, nSeed
                ...
                do while (stackPointer > 0)
                    ...
                    if (int(arrVals(i, j, k)) .ceq. 1) then
                    ...
                    do i = 2, il
                        ...
                        if (xSeed(i, j, k, 1) < large) then
                            ...
                            if (length > 1.73205080 * h) then
                                if (int(cartPointer(globalInd + 1)) .ceq. -3) then
                                    iblank(i, j, k) = -3
                                end if
                            end if
                        else
...
#include <petsc/finclude/petsc.h>
        ...
        complex(kind=realType), intent(in), dimension(3) :: xMin
        complex(kind=realType), intent(in) :: h
        Vec blankVec
        ! Working

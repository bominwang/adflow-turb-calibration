 ...
 contains
    subroutine wallSearch(aSurf, bSurf)
       ...
       use complexify 
        ...
        complex(kind=realType) :: uvw(5), xx(4), dist, q1(3, 4), q2(3, 4), delta, radius1, radius2
        ...
        if ((aSurf%nCells .ceq. 0) .or. (bSurf%nCells .ceq. 0)) then
        ...
        if (clusterAreas(bSurf%cluster) < clusterAreas(aSurf%cluster)) then
            ...
            return
        else if (clusterAreas(bSurf%cluster) .ceq. clusterAreas(aSurf%cluster)) then
       ...
       use complexify 
        ...
        do i = 1, 4
            ...
            do iCell = 1, 4
                ...
                if (curElem .cne. 0) then
                    ! This is a real cell:

                    if ((mask(curElem) .cne. baseElemID) .and. (mask(curElem) .ceq. 0)) then
    ...
    subroutine quadOverlap(q1, q2, overlapped)
       ...
       use complexify 
        ...
        ! input/output
        complex(kind=realType), dimension(3, 4), intent(in) :: q1, q2
        logical, intent(out) :: overlapped

        ! Working
        integer(kind=intType) :: ii, jj
        complex(kind=realType), dimension(2, 4) :: qq1, qq2
        complex(kind=realType), dimension(3) :: axis1, axis2, n1, n2, normal, v1, v2, c1, c2
        complex(kind=realType) :: e1, e2
    ...
    subroutine quadOverlap2D(q1, q2, overlapped)
       ...
       use complexify 
        ...
        ! input/output
        complex(kind=realType), dimension(2, 4), intent(in) :: q1, q2
        logical, intent(out) :: overlapped

        ! Working
        complex(kind=realType), dimension(4) :: tmp1, tmp2
        integer(kind=intType) :: ii, jj, kk, jjp1
        complex(kind=realType), dimension(2) :: axis, p0
        complex(kind=realType) :: min1, max1, min2, max2
        overlapped = .True.
        tmp1 = zero
        tmp2 = zero
    ...
    function getCellRadius(q)
       ...
       use complexify 
        implicit none
        ! Input
        complex(kind=realType), dimension(3, 4) :: q
        complex(kind=realType) :: getCellRadius

        ! Working
        complex(kind=realType) :: c(3)

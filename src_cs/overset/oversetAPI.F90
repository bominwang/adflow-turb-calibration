 ...
 contains
    subroutine oversetComm(level, firstTime, coarseLevel, closedFamList)
       ...
       use complexify 
        ...
        complex(kind=realType) :: startTime, endTime, curQuality, aspect(3), fact
        ...
        complex(kind=realType), dimension(:), allocatable :: tmpReal
        complex(kind=realType), dimension(:, :), allocatable :: xMin, xMax
        ...
        complex(kind=realType), dimension(:), allocatable :: realRecvBuf
        ...
        spectralLoop: do sps = 1, nTimeIntervalsSpectral
            ...
            do i = 1, recvCount
                ...
                ! being complete.
                if (recvInfo(2, index) .ceq. 1) then
                    oBlocks(iDom)%realBufferReady = .True.
                else if (recvInfo(2, index) .ceq. 2) then
                    oBlocks(iDom)%intBufferReady = .True.
                else if (recvInfo(2, index) .ceq. 3) then
                    oFringes(iDom)%realBufferReady = .True.
                else if (recvInfo(2, index) .ceq. 4) then
                ...
                if ((i .ceq. recvCount) .and. .not. flag) then
            ...
            do nn = 1, nDom
                ...
                do ii = 1, nDonors
                    ...
                    if ((curI .cne. i) .or. (curJ .cne. j) .or. (curK .cne. k)) then
            ...
            refineLoop: do iRefine = 1, nRefine
                                ...
                                do jj = j, j + 1
                                    do ii = i, i + 1
                                        ...
                                        if (isFlooded(flowDoms(nn, level, sps)%status(ii, jj, kk)) .or. &
                                            ...
                                            (flowDoms(nn, level, sps)%iblank(ii, jj, kk) .ceq. -4) .or. &
                        ...
                        if (nn >= 1 .and. nn <= nDom) then
                            ...
                            do jj = 1, oFringes(iDom)%nDonor
                                ...
                                if (donorBlock < 0) then
                                    ...
                                    do ii = iStart, iEnd
                                        if ((fringes(ii)%donorBlock .ceq. abs(donorBlock)) .and. &
                                            (fringes(ii)%dIndex .ceq. dIndex)) then
                ...
                do kk = 1, nOfringeSend
                    ...
                    do jj = cumFringeRecv(kk), cumFringeRecv(kk + 1) - 1
                        ...
                        if (donorBlock < 0) then
                            ...
                            do ii = iStart, iEnd
                                if ((fringes(ii)%donorProc .ceq. donorProc) .and. &
                                    (fringes(ii)%donorBlock .ceq. abs(donorBlock)) .and. &
                                    (fringes(ii)%dIndex .ceq. dIndex)) then
                ...
                do nn = 1, nDom
                    ...
                    do k = 2, kl
                        do j = 2, jl
                            do i = 2, il
                                if (flowDoms(nn, level, sps)%iblankLast(i, j, k) .cne. iblank(i, j, k)) then
                                    localChanged = .True.
                                end if
                                ! Now save the value
    ...
    contains
       ...
       use complexify 
            ...
            ! Working Variables
            complex(kind=realType), dimension(3, nDom) :: xMinLocal, xMaxLocal
       ...
       use complexify 
            ...
            do nn = 1, nDom
                ...
                do jDom = 1, nDomTotal
                    ...
                    if (clusters(iDom) .ceq. clusters(jDom)) then
    ...
    subroutine writePartitionedMesh(fileName)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(:, :, :, :), allocatable :: xtmp
       ...
       use complexify 
        ...
        do while (blocksAvailable)
            ...
            do while ((.not. blocksAvailable) .and. (blockID .lt. cgnsnDom))
                ...
                if (cgnsDoms(blockID)%cluster .ceq. 0) then
                    blocksAvailable = .true.
                end if
            end do
    ...
    contains
       ...
       use complexify 
            ...
            do connID = 1, cgnsDoms(blockID)%n1to1
                ...
                if (cgnsDoms(connBlock)%cluster .ceq. 0) then
                ...
                else if (cgnsDoms(connBlock)%cluster .cne. clusterID) then ! Check symmetry
       ...
       use complexify 
        ...
        do i = 1, cgnsNDom
            do j = 1, cgnsDoms(i)%nBocos
                ...
                if ((bc .ceq. NSWallAdiabatic) .or. (bc .ceq. NSWallIsoThermal) .or. (bc .ceq. EulerWall)) then

                    if (cgnsDoms(i)%bocoInfo(j)%iBeg .ceq. cgnsDoms(i)%bocoInfo(j)%iEnd) &
                        cgnsDoms(i)%viscousDir(1) = .True.

                    if (cgnsDoms(i)%bocoInfo(j)%jBeg .ceq. cgnsDoms(i)%bocoInfo(j)%jEnd) &
                        cgnsDoms(i)%viscousDir(2) = .True.

                    if (cgnsDoms(i)%bocoInfo(j)%kBeg .ceq. cgnsDoms(i)%bocoInfo(j)%kEnd) &
    ...
    subroutine flagCellsInSurface(pts, npts, conn, nconn, flag, ncell, blockids, nblocks, k_min)
       ...
       use complexify 
        ...
        complex(kind=realType), intent(in), dimension(3, npts) :: pts
        ...
        complex(kind=realType) :: dStar, frac, volLocal
        complex(kind=realType), dimension(3) :: minX, maxX, v1, v2, v3, axisVec
        complex(kind=realType), dimension(3) :: diag1, diag2, diag3, diag4, vertexPt
        complex(kind=realType) :: dd1, dd2, dd3, dd4, diag_max
        type(adtType) :: ADT
        complex(kind=realType), dimension(:, :), allocatable :: norm
        ...
        complex(kind=realType) :: coor(4), uvw(5)
        complex(kind=realType) :: dummy(3, 2)
    ...
    contains
       ...
       use complexify 
            ...
            complex(kind=realType) :: shp(4), xp(3), normal(3), v1(3), dp
       ...
       use complexify 
            ...
            logical :: checkNearby
            integer(kind=intType) :: jj
            complex(kind=realType) :: shp(4), xp(3), normal(3), v1(3), projLen
            complex(kind=realType) :: vertexVec(3), projVec(3), projDist
    ...
    subroutine setBlockPriority(blkName, value, setValue)
       ...
       use complexify 
        ...
        complex(kind=realType), intent(in) :: value
        ...
        if (cgnsNDom > 0) then
            setValue = .False.
            do iDom = 1, CGNSnDom
                if (trim(cgnsDoms(iDom)%zoneName) .ceq. trim(blkName)) then
                    setValue = .True.

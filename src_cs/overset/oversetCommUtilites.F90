 ...
 contains
    ...
    subroutine getCommPattern(oMat, sendList, nSend, recvList, nRecv)
       ...
       use complexify 
        ...
        do nn = 1, nDom
            ...
            do jj = 1, nUniqueProc
                if (procsForThisRow(jj) .cne. myid) then
        ...
        do iDom = 1, oMat%nRow
            ...
            rowLoop: do jj = oMat%rowPtr(iDom), oMat%rowPtr(iDom + 1) - 1
                ...
                if (oMat%assignedProc(jj) .ceq. myID) then
    ...
    subroutine getOSurfCommPattern(oMat, oMatT, sendList, nSend, &
       ...
       use complexify 
        ...
        do iDom = 1, nDomTotal
            if ((toRecv(iDom) .ceq. 1) .and. rBufSize(iDom) > 0) then
                nRecv = nRecv + 1
                recvList(1, nRecv) = blkProc(iDom)
                recvList(2, nRecv) = iDom
            end if
    ...
    subroutine oversetLoadBalance(overlap)
       ...
       use complexify 
        ...
        complex(kind=realType) :: evenCost, potentialSum, targetCost
        complex(Kind=realType) :: totalSearch, totalBuild

        complex(kind=realType), dimension(0:nProc - 1) :: procCosts
        complex(kind=realType), dimension(0:nProc) :: cumProcCosts
        complex(kind=realType), dimension(overlap%nRow) :: buildCost
        complex(kind=realType), parameter :: tol = 0.1_realType
        ...
        complex(kind=realType), pointer, dimension(:) :: data
        ...
        curRow = 1
        do while (rowPtr(curRow + 1) - rowPtr(curRow) .ceq. 0)
            curRow = curRow + 1
        end do
        ...
        masterLoop: do while (curRow <= overlap%nRow .and. iProc <= nProc)
            ...
            if ((data(jj) .cne. zero) .and. .not. (blockTaken(jj))) then

                if ((procCosts(iProc) .ceq. 0) .or. (iProc .ceq. nProc - 1)) then
                    ! Must be added
                    ...
                    if (potentialSum < targetCost - tol * evenCost) then
                        ...
                        restOfRow: do jj1 = jj + 1, rowPtr(curRow + 1) - 1
                            ...
                            if ((data(jj1) .cne. zero) .and. .not. (blockTaken(jj1))) then
            ...
            if (increment) then
                ...
                if (jj .ceq. rowPtr(curRow + 1)) then
                    ...
                    findNextNonZeroRow: do while (jj .ceq. rowPtr(curRow + 1))
                        curRow = curRow + 1
    ...
    subroutine exchangeSurfaceDelta(zipperFamList, level, sps, commPattern, internal)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(:), allocatable :: pSave
        complex(kind=realType), dimension(:, :), pointer :: deltaPtr
    ...
    subroutine updateOversetConnectivity(level, sps)
       ...
       use complexify 
        ...
        integer, dimension(mpi_status_size) :: mpiStatus
        complex(kind=realType) :: frac(3), frac0(3), xCen(3)
        ...
        complex(kind=realType) :: fracTol = 1e-4
    ...
    subroutine updateOversetConnectivity_d(level, sps)
       ...
       use complexify 
        ...
        integer(kind=intType) :: size, procID, index, iii, jjj
        integer, dimension(mpi_status_size) :: mpiStatus
        complex(kind=realType) :: frac(3), fracd(3), frac0(3), xCen(3), xCend(3), weight(8)
        integer(kind=intType), dimension(8), parameter :: indices = (/1, 2, 4, 3, 5, 6, 8, 7/)
    ...
    subroutine updateOversetConnectivity_b(level, sps)
       ...
       use complexify 
        ...
        integer(kind=intType) :: size, procID, index, iii, jjj
        integer, dimension(mpi_status_size) :: mpiStatus
        complex(kind=realType) :: frac(3), fracd(3), frac0(3), xCen(3), xCend(3), weight(8), add(3)
        integer(kind=intType), dimension(8), parameter :: indices = (/1, 2, 4, 3, 5, 6, 8, 7/)

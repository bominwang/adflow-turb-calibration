 ...
 contains
    subroutine makeGapBoundaryStrings(zipperFamList, level, sps, master)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(3) :: s1, s2, s3, s4, v1, v2, v3, v4, x0
        complex(kind=realType) :: fact, timeA, minNorm

        complex(kind=realType), dimension(:, :, :), allocatable :: patchNormals
        complex(kind=realType), dimension(:, :), allocatable :: patchH
                    ...
                    do j = jBeg, jEnd       ! <------- Node loop
                        do i = iBeg + 1, iEnd  ! <------- Face Loop
                            if (gcp(i + 1, j + 1) >= 0 .and. gcp(i + 1, j + 2) >= 0) then
                                ...
                                if (((below .ceq. 0) .and. (above .ceq. 1)) .or. ((below .ceq. 1) .and. (above .ceq. 0))) then
                    ...
                    do j = jBeg + 1, jEnd   ! <------- Face loop
                        do i = iBeg, iEnd  ! <------- Node Loop
                            if (gcp(i + 1, j + 1) >= 0 .and. gcp(i + 2, j + 1) >= 0) then
                                ...
                                if (((left .ceq. 0) .and. (right .ceq. 1)) .or. ((left .ceq. 1) .and. (right .ceq. 0))) then
                                    localStrings(c)%nNodes = localStrings(c)%nNodes + 2
                                    localStrings(c)%nElems = localStrings(c)%nElems + 1

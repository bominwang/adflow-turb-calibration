...
#include <petsc/finclude/petsc.h>
        ...
        BCGroupLoop: do iBCGroup = 1, nFamExchange
            ...
            do i = 1, size(BCFamGroups(iBCGroup)%famList)
                do j = 1, size(zipperFamlist)
                    if (BCFamGroups(iBCGroup)%famList(i) .ceq. zipperFamList(j)) then
                        nFam = nFam + 1
                    end if
                end do
            end do
            ...
            if (debugZipper .and. (myid .ceq. 0)) then
                    ...
                    if (famInList(BCData(mm)%famID, famList)) then
                        do j = BCData(mm)%jnBeg + 1, BCData(mm)%jnEnd
                            do i = BCData(mm)%inBeg + 1, BCData(mm)%inEnd
                                ii = ii + 1
                                if (oSurfs(iDom)%iBlank(ii) .ceq. -2) then
                ...
                famInclude: if (famInList(BCData(mm)%famID, famList)) then
                    ...
                    do j = jBeg, jEnd
                        do i = iBeg, iEnd
                            ...
                            if ((BCData(mm)%iBlank(i, j) .ceq. -1) .and. validCell(i, j)) then

                                ! Reset the side flag
                                side = .False.

                                if (validCell(i - 1, j) .and. (BCData(mm)%iBlank(i - 1, j) .ceq. 1)) then
                                    side(1) = .True.
                                end if
                                ...
                                if (validCell(i, j - 1) .and. (BCData(mm)%iBlank(i, j - 1) .ceq. 1)) then
                                    side(3) = .True.
                                end if
    ...
    contains
       ...
       use complexify 
            ...
            validCell = .False.
            if (gcp(i + 1, j + 1) >= 0 .and. (frx(i + 1, j + 1) .ceq. 0)) then
                validCell = .True.
            end if
    ...
    subroutine surfaceDeviation(famList, level, sps)
       ...
       use complexify 
        ...
        complex(kind=realType) :: deviation
    ...
    function checkDeviation(P0, P1, P2, P3)
       ...
       use complexify 
        ...
        ! Input Parameters
        complex(kind=realType), intent(in), dimension(3) :: P0, P1, P2, P3

        ! Function value
        complex(kind=realType) :: checkDeviation

        ! Working Parameters
        complex(kind=realType) :: t0, t1, t2, t3
        complex(kind=realType), dimension(3) :: A1, A2, A3, B1, B2
        complex(kind=realType), parameter :: alpha = half
        integer(kind=intType), parameter :: N = 20
        integer(kind=intType) :: i
        complex(kind=realType) :: t, P(3), Q(3), s
    ...
    subroutine writeWalls(famList)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(:), allocatable :: xBuffer
    ...
    contains
       ...
       use complexify 
            ...
            complex(kind=realType), intent(in), dimension(:) :: xx
        ...
        domainLoop2: do nn = 1, nDom
            ...
            bocoLoop2: do mm = 1, nBocos
                famInclude2: if (famInList(BCData(mm)%famID, famList)) then
                    ...
                    do j = jBeg, jEnd
                        do i = iBeg, iEnd
                            if ((BCData(mm)%iBlank(i, j) .ceq. 1) .and. (nE(i, j) .ceq. 0) .and. (nC(i, j) .ceq. 0)) then
                                BCData(mm)%iBlank(i, j) = 0
                            end if
                        end do
                    end do
    ...
    contains
            ...
            do j = jBeg, jEnd
                do i = iBeg, iEnd
                    if (BCData(mm)%iBlank(i, j) >= 0) then
                        ...
                        if (BCData(mm)%iBlank(i, j - 1) .ceq. 1) &
                            e(1) = 1

                        if (BCData(mm)%iBlank(i + 1, j) .ceq. 1) &
                            e(2) = 1
                        ...
                        if ((BCData(mm)%iBlank(i - 1, j - 1) .ceq. 1) .and. (e(4) .ceq. 0) .and. (e(1) .ceq. 0)) &
                            nC(i, j) = nC(i, j) + 1

                        if ((BCData(mm)%iBlank(i + 1, j - 1) .ceq. 1) .and. (e(1) .ceq. 0) .and. (e(2) .ceq. 0)) &
                            nC(i, j) = nC(i, j) + 1

                        if ((BCData(mm)%iBlank(i + 1, j + 1) .ceq. 1) .and. (e(2) .ceq. 0) .and. (e(3) .ceq. 0)) &
                            nC(i, j) = nC(i, j) + 1

                        if ((BCData(mm)%iBlank(i - 1, j + 1) .ceq. 1) .and. (e(3) .ceq. 0) .and. (e(4) .ceq. 0)) &
                            nC(i, j) = nC(i, j) + 1
       ...
       use complexify 
            ...
            do j = jBeg, jEnd
                do i = iBeg, iEnd
                    if (BCData(mm)%iBlank(i, j) .ceq. 1) then
                        ...
                        if (BCData(mm)%iBlank(i - 1, j - 1) .ceq. 1) &
                            nC(i, j) = nC(i, j) + 1

 ...
 contains
       ...
       use complexify 
        ...
        do while (curElem < master%nElems)
            ...
            if (nElems > 1) then
                ...
                ! periodic chain
                if (master%elemUsed(firstElem) .ceq. 0) then
            ...
            do while ((master%elemUsed(curElem) .ceq. 1) .and. (curElem < master%nElems))
                curElem = curElem + 1
            end do
        end do
        ...
        do i = 1, string%nElems
            ...
            do jj = 1, 2
                ...
                do j = 1, string%nte(1, n(jj)) ! Loop over the element numbers already here:
                    ...
                    if ((m(1) .ceq. n(1)) .and. (m(2) .ceq. n(2))) then
                        duplicateElement = .True.
                    else if ((m(1) .ceq. n(2)) .and. (m(2) .ceq. n(1))) then
    ...
    subroutine createSubStringFromElems(p, s, id)
       ...
       use complexify 
        ...
        do i = 1, s%nElems
            ...
            if (nodeUsed(n1) .ceq. 0) then
                k = k + 1
                nodeUsed(n1) = k
            end if

            if (nodeUsed(n2) .ceq. 0) then
                k = k + 1
                nodeUsed(n2) = k
            end if
        end do
    ...
    subroutine selfZip(s, cutOff, nZipped)
       ...
       use complexify 
        ...
        complex(kind=realType), intent(in) :: cutOff

        ! Working
        integer(kind=intType) :: i, j, k, N, ii, im1, ip1
        logical :: lastNodeZippered, added
        complex(kind=realType), dimension(3) :: v1, v2, norm
        complex(kind=realType) :: cosCutoff, cosTheta, r2, v1nrm, v2nrm
        ...
        do while (ii <= N)
            ...
            if (s%isPeriodic .and. (ii .ceq. N)) then
                ip1 = 1
            end if
    ...
    subroutine crossZip(str1, N1, N2, str2, N3, N4, debugZipper, failed)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(3) :: ptA, ptB, ptAp, ptBp
        !real(kind=realType), dimension(3) :: ptAPrev, ptBPRev
        complex(kind=realType), dimension(3) :: Aoff, Boff, ApOff, BpOff
        complex(kind=realType), dimension(3) :: normA, normB, normAp, normBp
        complex(kind=realType), dimension(3) :: perpA, perpB, perpAp, perpBp
        !real(kind=realType), dimension(3) :: normAPrev, normBPrev
        !real(kind=realType), dimension(3) :: perpAPrev, perpBPrev
        complex(kind=realType), dimension(3) :: triNorm1, quadNorm1
        complex(kind=realType), dimension(3) :: triNorm2, quadNorm2
        ...
        complex(kind=realType) :: sum1, sum2, h, dpa, dpb
        !am real(kind=realType), parameter :: cutOff = 0.95*3
        complex(kind=realType), parameter :: cutOff = 0.85 * 3
    ...
    contains
       ...
       use complexify 
            ...
            complex(kind=realType) :: vecAngle

            ! Working
            complex(kind=realType), dimension(3) :: vecA, vecB
       ...
       use complexify 
            ...
            if ((e1 .ceq. e3) .or. (e1 .ceq. e4)) then
                elemBetweenNodes = e1
            else
                elemBetweenNodes = e2
            end if
       ...
       use complexify 
            ...
            ! Input/Output
            complex(kind=realType), intent(in), dimension(3) :: pt1, pt2, pt3
            complex(kind=realType) :: triArea

            ! Working
            complex(kind=realType), dimension(3) :: norm
       ...
       use complexify 
        ...
        strLoop: do iString = 1, nStrings
            ...
            do while (curIStart > 0)
                ...
                if ((jStart .ceq. jEnd) .and. .not. fullLoop2 .and. &
                    (s2%otherID(1, jStart) .cne. s1%myID)) then
                ...
                if (((iStart .ceq. iEnd) .and. .not. fullLoop1) .or. &
                    ((jStart .ceq. jEnd) .and. .not. fullLoop2)) then
                ...
                if (((istart .ceq. iend) .and. fullLoop1) .and. &
                    ((jstart .ceq. jend) .and. fullLoop2)) then
                ...
                else if (((iStart .ceq. iEnd) .and. fullLoop1) .and. .not. fullLoop2) then

                    ! Project jStart and jEnd onto s1
                    iStart = s2%otherID(2, jStart)
                    iEnd = s2%otherID(2, jEnd)

                else if (((jStart .ceq. jEnd) .and. fullLoop2) .and. .not. fullLoop1) then
    ...
    contains
       ...
       use complexify 
            ...
            nodeLoop: do i = 1, s%nNodes
                if (s%xZipNodeUsed(i) .ceq. 0) then
                    startNode = i
                    exit nodeLoop
                end if
            end do nodeLoop
       ...
       use complexify 
            ...
            ! one.
            if (s%xZipNodeUsed(nextNode) .ceq. 1) then
                nextNode = i
            end if
        end function nextNode
       ...
       use complexify 
            ...
            ! If the next node is used. The next node is set the current
            ! one.
            if (s%xZipNodeUsed(prevNode) .ceq. 1) then
                prevNode = i
            end if
       ...
       use complexify 
            ...
            traverseLoop: do
                ...
                if ((nextI .ceq. i) .or. (s%otherID(1, nextI) .cne. checkID)) then
    ...
    subroutine makePocketZip(p, strings, nStrings, pocketMaster, debugZipper)
       ...
       use complexify 
        ...
        do while (i <= p%nEdges)
            ...
            if (str1 .cne. str2) then
                if (.not. strings(str1)%isperiodic .and. &
                    .not. strings(str2)%isperiodic .and. &
                    ((cn1 .ceq. 1) .or. (cn1 .ceq. nNodes1)) .and. ((cn2 .ceq. 1) .or. (cn2 .ceq. nNodes2))) then
            ...
            if ((nsum1 .ceq. nsum2) .and. (ndiff1 + ndiff2 .ceq. 0)) then
        ...
        do while (curElem < pocketMaster%nElems)
            ...
            ! Scan through until we find the next unused element:
            do while ((pocketMaster%elemUsed(curElem) .ceq. 1) .and. &
    ...
    subroutine pocketZip(s)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(3) :: v1, v2, norm, c
        complex(kind=realType) :: cosCutoff, cosTheta, r2, v1nrm, v2nrm, costhetaMax
        complex(kind=realType) :: dp, dpMax

        integer(Kind=intType), dimension(:), allocatable :: nodeMap, badNode
        type(kdtree2_result), dimension(:), allocatable :: results
        logical :: added, iiMinSet
        complex(kind=realType), parameter :: fact = 0.95_realType
        N = s%nNodes
    ...
    subroutine computeTriSurfArea(master, area)
       ...
       use complexify 
        ...
        complex(kind=realType), intent(out) :: area
    ...
    function triOverlap(pt1, pt2, pt3, str, i1, i2)
       ...
       use complexify 
        ...
        ! Input/Output
        complex(kind=realType), dimension(3), intent(in) :: pt1, pt2, pt3
        ...
        integer(kind=intType) :: i
        complex(kind=realType) :: triNorm(3)
    ...
    subroutine shortenString(s, nodeMap)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(:, :), pointer :: nodeDataTmp
    ...
    subroutine addPotentialTriangle(s, im1, ii, ip1, nodeMap, results, added)
       ...
       use complexify 
        ...
        nodeFoundLoop: do k = 1, nFound
            ...
            notPartofTriangle: if ((idx .cne. s%pNodes(im1)) .and. &
                                   (idx .cne. s%pNodes(ii)) .and. (idx .cne. s%pNodes(ip1))) then
    ...
    subroutine closestSymmetricNode(s1, s2, i, j)
       ...
       use complexify 
        ...
        complex(kind=realType) :: minDist, dist
        ...
        do ii = 1, s1%nNodes
            ...
            if (s2%otherID(2, s1%otherID(2, ii)) .ceq. ii) then
    ...
    subroutine stringMatch(strings, nStrings, debugZipper)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(3) :: xj, xjp1, xjm1, normj
        complex(kind=realType), dimension(3) :: xk, xkp1, xkm1, normk
        complex(kind=realType), dimension(3) :: myPt, otherPt, eNorm
        complex(kind=realType) :: fact, dStar, curDist, minDist, edgeLength
        integer(kind=intTYpe) :: otherID, otherIndex, closestOtherIndex, closestOtherString
        integer(kind=intType) :: id, index
        complex(kind=realType) :: timeA, pt(3), v(3), cosTheta, cutOff, dist, maxH, ratio

        if (nStrings .ceq. 0) then
            return
        end if
            ...
            nodeLoop: do j = 1, str%nNodes
                ...
                outerLoop: do
                    ...
                    innerLoop: do k = 1, nSearch
                        ...
                        if (master%cNodes(1, idx) .ceq. str%myID) then
                            cycle innerLoop
                        end if
                        ...
                        ! --------------------------------------------
                        if (master%cNodes(2, idx) .ceq. 0) then
                            cycle innerLoop
                        end if
                        ...
                        ! isn't me.
                        if (closestOtherIndex .ceq. -1) then
                        ...
                        if (otherID .cne. closestOtherString) then
                            if (overlappedEdges2( &
                    ...
                    if (nSearch .ceq. master%Nnodes) then
                        exit outerLoop
                    end if
    ...
    subroutine writeOversetString(str, strings, n, fileID)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(3) :: myPt, otherPT, vec
        complex(kind=realType) :: maxH, dist, ratio
    ...
    subroutine pointInTriangle(x1, x2, x3, pt, inTri)
       ...
       use complexify 
        implicit none
        complex(kind=realType), dimension(3), intent(in) :: x1, x2, x3, pt
    ...
    contains
       ...
       use complexify 
            implicit none
            logical :: sameSide
            complex(kind=realType), dimension(3) :: p1, p2, a, b, cp1, cp2
    ...
    function positiveTriArea(p1, p2, p3, norm)
       ...
       use complexify 
        implicit none
        complex(kind=realType), intent(in), dimension(3) :: p1, p2, p3, norm
        complex(kind=realType), dimension(3) :: n
    ...
    subroutine getNodeInfo(str, j, checkLeft, checkRight, concave, xj, xjm1, xjp1, normj)
       ...
       use complexify 
        ...
        logical :: checkLeft, checkRight, concave
        complex(kind=realType), dimension(3) :: xj, xjm1, xjp1, normj
        complex(kind=realType), dimension(3) :: v
        checkLeft = .True.
        checkRight = .True.
        xj = str%x(:, j)
        normj = str%norm(:, j)
        ...
        if (str%isPeriodic) then
            if (j > 1 .and. j < str%nNodes) then
                xjm1 = str%x(:, j - 1)
                xjp1 = str%x(:, j + 1)
            else if (j .ceq. 1) then
                xjm1 = str%x(:, str%nNodes)
                xjp1 = str%x(:, j + 1)
            else if (j .ceq. str%nNodes) then
                xjm1 = str%x(:, j - 1)
                xjp1 = str%x(:, 1)
            end if
        else
    ...
    function nodeInFrontOfEdges(pt, concave, checkLeft, checkRight, xj, xjm1, xjp1, normj)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(3), intent(in) :: pt, xj, xjm1, xjp1, normj
    ...
    function overlappedEdges(str, j, pt)
       ...
       use complexify 
        ...
        ! Input/output
        complex(kind=realType), dimension(3), intent(in) :: pt
        type(oversetString), intent(in) :: str
        ...
        ! Working
        integer(kind=intType) :: i
        complex(kind=realType), dimension(3) :: v, p1, p2, u, normA, normB, x0, norm
        complex(kind=realType) :: uNrm, x1, x2, x3, x4, y1, y2, y3, y4, idet, Px, Py
        complex(kind=realType) :: u1, u2, v1, v2, w1, w2
        complex(kind=realType) :: s1, s2, tmp, line(2), vec(2), tol
        ...
        elemLoop: do i = 1, str%nElems
            ...
            if ((str%conn(1, i) .ceq. j) .or. (str%conn(2, i) .ceq. j)) then
                cycle
            end if
    ...
    function overlappedEdges2(str, pt1, norm, pt2)
       ...
       use complexify 
        ...
        ! Input/output
        complex(kind=realType), dimension(3), intent(in) :: pt1, pt2, norm
        type(oversetString), intent(in) :: str
        logical :: overlappedEdges2

        ! Working
        integer(kind=intType) :: i
        complex(kind=realType), dimension(3) :: v, p1, p2, u, normA, normB, x0
        complex(kind=realType) :: uNrm, x1, x2, x3, x4, y1, y2, y3, y4, idet, Px, Py
        complex(kind=realType) :: u1, u2, v1, v2, w1, w2
        complex(kind=realType) :: s1, s2, tmp, line(2), vec(2), tol

 ...
 contains
    ...
    subroutine computeFringeProcArray(fringes, n, fringeProc, cumFringeProc, nFringeProc)
       ...
       use complexify 
        ...
        do i = 1, n
            if (currentProc .cne. fringes(i)%donorProc) then
    ...
    subroutine wallsOnBlock(wallsPresent)
       ...
       use complexify 
        ...
        do mm = 1, cgnsDoms(nbkGlobal)%nBocos
            if ((cgnsDoms(nbkGlobal)%bocoInfo(mm)%BCType .ceq. NSWallAdiabatic) .or. &
                (cgnsDoms(nbkGlobal)%bocoInfo(mm)%BCType .ceq. NSWallIsothermal) .or. &
                (cgnsDoms(nbkGlobal)%bocoInfo(mm)%BCType .ceq. EulerWall)) then
                wallsPresent = .True.
            end if
        end do
       ...
       use complexify 
        ...
        do while (first <= last)
            if (arr(middle) < searchNode) then
                first = middle + 1
            else if (arr(middle) .ceq. searchNode) then
                searchInd = middle
                exit
            else
                last = middle - 1
       ...
       use complexify 
        ...
        do while (first <= last)
            if (arr(middle) < search) then
                first = middle + 1
            else if (arr(middle) .ceq. search) then
                searchInd = middle
                exit
            else
                last = middle - 1
    ...
    subroutine qsortFringeType(arr, nn, sortType)
       ...
       use complexify 
        ...
        if (sortType .ceq. sortByDonor) then
            fringeSortType = sortByDonor
        else if (sortType .ceq. sortByReceiver) then
    ...
    subroutine addToFringeBuffer(intBuffer, realBuffer, n, fringe)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(:, :), pointer :: realBuffer
        type(fringeType) :: fringe
        integer(kind=intType), intent(inout) :: n
        ...
        complex(kind=realType), dimension(:, :), pointer :: tmpReal
       ...
       use complexify 
                    ...
                    do i = 2, il
                        if (iblank(i, j, k) .ceq. 1) then
                            ...
                            stencilLoop: do i_stencil = 1, N_visc_drdw
                                ...
                                if (.not. ((iBlank(ii, jj, kk) .ceq. 1) .or. (iblank(ii, jj, kk) .ceq. -1))) then
                                    badCell = .True.
                                end if
                            end do stencilLoop
        ...
        do nn = 1, nDom
            ...
            do k = 2, kl
                do j = 2, jl
                    do i = 2, il
                        if ((iblank(i, j, k) .ceq. 0) .or. (iblank(i, j, k) .ceq. -2) .or. &
                            (iblank(i, j, k) .ceq. -3) .or. (iblank(i, j, k) .ceq. -4)) then
        ...
        if (lastCall .and. (totalOrphans .gt. 0)) then
            ...
            do nn = 1, nDom
                ...
                do k = 2, kl
                    do j = 2, jl
                        do i = 2, il
                            if (flowDoms(nn, level, sps)%iblankLast(i, j, k) .ceq. -5) iBlank(i, j, k) = -5
                        end do
                    end do
                end do
            end do
    ...
    subroutine computeQuadraticWeights(xSearch, xElem, uvwQuadratic)
       ...
       use complexify 
        ...
        ! Input variables
        complex(kind=realType), intent(in) :: xSearch(3), xElem(3, -1:1, -1:1, -1:1)
        complex(kind=realType), intent(inout) :: uvwQuadratic(3)

        ! Working variables
        ! -----------------------------------------------------------------
        ! newton related
        integer(kind=intType) :: n, niter
        complex(kind=realType) :: resid, residtol
        complex(kind=realType) :: B(3, 4)

        ! others
        integer(kind=intType) :: j, l, iii, jjj, kkk
        complex(kind=realType) :: ff(27), shp(3, 3), psi(3)
        complex(kind=realType) :: dff(27, 3), dshp(3, 3, 3) !-> differentials wrt 3 psi dirs
        logical :: ok_flag
        ! -----------------------------------------------------------------
    ...
    subroutine matrixinv3by3(a, ok_flag)
       ...
       use complexify 
        ...
        ! Input variables
        complex(kind=realType), intent(inout) :: a(3, 4)
        logical, intent(out) :: ok_flag

        ! Working variables
        integer(kind=intType) :: n
        complex(kind=realType) :: det, cofactor(3, 3), ainv(3, 3), rin(3, 1), rout(3, 1), myeps
                ...
                do j = 2, jl
                    do i = 2, il
                        ...
                        if (iblank(i, j, k) .ceq. -4) then
                            ...
                            if (forcedRecv(i, j, k) .cne. 0) then
    ...
    subroutine getWorkArray(overlap, work)
       ...
       use complexify 
        ...
        nWork = 0
        do jj = 1, overlap%nnz
            if (overlap%assignedProc(jj) .ceq. myid) then
                nWork = nWork + 1
            end if
        end do
...
#include <petsc/finclude/petsc.h>
        ...
        complex(kind=realType), dimension(:), pointer :: localPtr
        complex(kind=realType) :: vals(5)
        Vec CGNSVec
    ...
    subroutine fracToWeights(frac, weights)
       ...
       use complexify 
        implicit none
        complex(kind=realType), intent(in), dimension(3) :: frac
        complex(kind=realType), intent(out), dimension(8) :: weights
    ...
    subroutine newtonUpdate(xCen, blk, frac0, frac)
       ...
       use complexify 
        ...
        ! Input
        complex(kind=realType), dimension(3), intent(in) :: xCen
        complex(kind=realType), dimension(3, 3, 3, 3), intent(in) :: blk
        complex(kind=realType), dimension(3), intent(in) :: frac0
        ! Output
        complex(kind=realType), dimension(3), intent(out) :: frac

        ! Working
        complex(kind=realType), dimension(3, 1:8) :: xn
        complex(kind=realType) :: u, v, w, uv, uw, vw, wvu, du, dv, dw
        complex(kind=realType) :: a11, a12, a13, a21, a22, a23, a31, a32, a33, val
        complex(kind=realType) :: f(3), x(3)

 ...
 contains
    ...
    subroutine whalo1(level, start, end, commPressure, commGamma, &
       ...
       use complexify 
        ...
        commVarGamma = .false.
        if (commGamma .and. ((cpModel .ceq. cpTempCurveFits))) &
            commVarGamma = .true.
        ...
        bothPAndE: if (commPressure .and. start <= irhoE .and. &
            ...
            domains: do nn = 1, nDom
                do mm = 1, flowDoms(nn, level, 1)%nBocos
                    if (flowDoms(nn, level, 1)%BCType(mm) .ceq. slidingInterface) then
    ...
    subroutine wOversetGeneric_d(nVar, level, sps, commPattern, Internal)
       ...
       use complexify 
        ...
        integer(kind=intType) :: i, j, k, ii, jj
        integer(kind=intType) :: d1, i1, j1, k1, d2, i2, j2, k2
        complex(kind=realType), dimension(:), pointer :: weight, weightd
    ...
    subroutine correctPeriodicCoor(level, sp, nPeriodic, periodicData)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(3, 3) :: rotMatrix
        complex(kind=realType), dimension(3) :: rotCenter, translation
...
#include <petsc/finclude/petsc.h>
        ...
        complex(kind=realType), dimension(:, :) :: vars
            ...
            domainLoop: do nn = 1, nDom
                ...
                bocoLoop: do mm = 1, nBocos
                    ...
                    if ((((BCType(mm) .ceq. SubsonicInflow) .or. &
                          (BCType(mm) .ceq. SupersonicInflow)) .and. (isInflow)) &
                        .or. &
                        ((BCType(mm) .ceq. SubsonicOutflow) .or. &
                         (BCType(mm) .ceq. SupersonicOutflow)) .and. (.not. isInflow)) then
...
#include <petsc/finclude/petsc.h>
        ...
        ! Input variables
        logical, intent(in) :: isInflow
        complex(kind=realType), dimension(:, :) :: vars, varsd
        integer(kind=intType) :: sps

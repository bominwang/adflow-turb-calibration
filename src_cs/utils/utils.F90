...
contains
    ...
    function TSbeta(degreePolBeta, coefPolBeta, &
       ...
       use complexify 
        ...
        complex(kind=realType), intent(in) :: omegaFourBeta, t

        complex(kind=realType), dimension(0:*), intent(in) :: coefPolBeta
        complex(kind=realType), dimension(0:*), intent(in) :: cosCoefFourBeta
        complex(kind=realType), dimension(*), intent(in) :: sinCoefFourBeta
        !
        !      Local variables.
        !
        integer(kind=intType) :: nn

        complex(kind=realType) :: beta, val

        ! Return immediately if this is a steady computation.

        if (equationMode .ceq. steady) then
            TSBeta = zero
            return
        end if
    ...
    function TSbetadot(degreePolBeta, coefPolBeta, &
       ...
       use complexify 
        ...
        !      Function type
        !
        complex(kind=realType) :: TSbetadot
        !
        !      Function arguments.
        !
        integer(kind=intType), intent(in) :: degreePolBeta
        ...
        complex(kind=realType) :: betadot, val
    ...
    function TSMach(degreePolMach, coefPolMach, &
       ...
       use complexify 
        ...
        complex(kind=realType), intent(in) :: omegaFourMach, t

        complex(kind=realType), dimension(0:*), intent(in) :: coefPolMach
        complex(kind=realType), dimension(0:*), intent(in) :: cosCoefFourMach
        complex(kind=realType), dimension(*), intent(in) :: sinCoefFourMach
        !
        !      Local variables.
        !
        integer(kind=intType) :: nn

        complex(kind=realType) :: intervalMach, val
    ...
    function TSMachdot(degreePolMach, coefPolMach, &
       ...
       use complexify 
        ...
        !      Function type
        !
        complex(kind=realType) :: TSmachdot
        !
        !      Function arguments.
        !
        integer(kind=intType), intent(in) :: degreePolMach
        ...
        complex(kind=realType) :: machdot, val
    ...
    function TSalpha(degreePolAlpha, coefPolAlpha, &
       ...
       use complexify 
        ...
        complex(kind=realType), intent(in) :: omegaFourAlpha, t

        complex(kind=realType), dimension(0:*), intent(in) :: coefPolAlpha
        complex(kind=realType), dimension(0:*), intent(in) :: cosCoefFourAlpha
        complex(kind=realType), dimension(*), intent(in) :: sinCoefFourAlpha
        !
        !      Local variables.
        !
        integer(kind=intType) :: nn

        complex(kind=realType) :: alpha, val
    ...
    function TSalphadot(degreePolAlpha, coefPolAlpha, &
       ...
       use complexify 
        ...
        !      Function type
        !
        complex(kind=realType) :: TSalphadot
        !
        !      Function arguments.
        !
        integer(kind=intType), intent(in) :: degreePolAlpha
        ...
        complex(kind=realType) :: alphadot, val
    ...
    function derivativeRigidRotAngle(degreePolRot, &
       ...
       use complexify 
        ...
        !      Function type
        !
        complex(kind=realType) :: derivativeRigidRotAngle
        !
        !      Function arguments.
        !
        ...
        complex(kind=realType), intent(in) :: omegaFourRot, t

        complex(kind=realType), dimension(0:*), intent(in) :: coefPolRot
        complex(kind=realType), dimension(0:*), intent(in) :: cosCoefFourRot
        complex(kind=realType), dimension(*), intent(in) :: sinCoefFourRot
        !
        !      Local variables.
        !
        integer(kind=intType) :: nn

        complex(kind=realType) :: dPhi, val
    ...
    function myDim(x, y)
       ...
       use complexify 
         complex(kind=realType) x, y
        complex(kind=realType) :: myDim
    ...
    subroutine rotMatrixRigidBody(tNew, tOld, rotationMatrix, &
       ...
       use complexify 
        ...
        !      Subroutine arguments.
        !
        complex(kind=realType), intent(in) :: tNew, tOld

        complex(kind=realType), dimension(3), intent(out) :: rotationPoint
        complex(kind=realType), dimension(3, 3), intent(out) :: rotationMatrix
        !
        !      Local variables.
        !
        integer(kind=intType) :: i, j

        complex(kind=realType) :: phi
        complex(kind=realType) :: cosX, cosY, cosZ, sinX, sinY, sinZ

        complex(kind=realType), dimension(3, 3) :: mNew, mOld
    ...
    function secondDerivativeRigidRotAngle(degreePolRot, &
       ...
       use complexify 
        ...
        !      Function type
        !
        complex(kind=realType) :: secondDerivativeRigidRotAngle
        !
        !      Function arguments.
        !
        integer(kind=intType), intent(in) :: degreePolRot
    ...
    function rigidRotAngle(degreePolRot, coefPolRot, &
       ...
       use complexify 
        ...
        !      Function type
        !
        complex(kind=realType) :: rigidRotAngle
        !
        !      Function arguments.
        !
        integer(kind=intType), intent(in) :: degreePolRot
    ...
    subroutine setBCPointers(nn, spatialPointers)
       ...
       use complexify 
        ...
        if (spatialPointers) then
            ...
            if (equations .ceq. RANSEquations) then
                select case (BCFaceID(nn))
                case (iMin)
                    dd2Wall => d2Wall(2, :, :)
                case (iMax)
    ...
    subroutine computeRootBendingMoment(cf, cm, bendingMoment)
       ...
       use complexify 
        ...
        complex(kind=realType), intent(in), dimension(3) :: cf, cm
        complex(kind=realType), intent(out) :: bendingMoment

        !Subroutine Variables
        complex(kind=realType) :: elasticMomentx, elasticMomenty, elasticMomentz
        bendingMoment = zero
        if (liftIndex .ceq. 2) then
    ...
    subroutine computeLeastSquaresRegression(y, x, npts, m, b)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(npts) :: x, y
        complex(kind=realType) :: m, b

        !local variables
        complex(kind=realType) :: sumx, sumy, sumx2, sumxy
        integer(kind=intType) :: i
    ...
    subroutine computeTSDerivatives(force, moment, coef0, dcdalpha, &
       ...
       use complexify 
        ...
        !     Subroutine arguments.
        !
        complex(kind=realType), dimension(3, nTimeIntervalsSpectral) :: force, moment
        complex(kind=realType), dimension(8) :: dcdq, dcdqdot
        complex(kind=realType), dimension(8) :: dcdalpha, dcdalphadot
        complex(kind=realType), dimension(8) :: Coef0

        ! Working Variables
        complex(kind=realType), dimension(nTimeIntervalsSpectral, 8) :: baseCoef
        complex(kind=realType), dimension(8) :: coef0dot
        complex(kind=realType), dimension(nTimeIntervalsSpectral, 8) :: ResBaseCoef
        complex(kind=realType), dimension(nTimeIntervalsSpectral) :: intervalAlpha, intervalAlphadot
        complex(kind=realType), dimension(nTimeIntervalsSpectral) :: intervalMach, intervalMachdot
        complex(kind=realType), dimension(nSections) :: t
        integer(kind=intType) :: i, sps, nn
        !speed of sound: for normalization of q derivatives
        complex(kind=realType) :: a
        complex(kind=realType) :: fact, factMoment
        ! Functions
        complex(kind=realType), dimension(nTimeIntervalsSpectral) :: dPhix, dPhiy, dphiz
        complex(kind=realType), dimension(nTimeIntervalsSpectral) :: dPhixdot, dPhiydot, dphizdot
        complex(kind=realType) :: derivativeRigidRotAngle, secondDerivativeRigidRotAngle
    ...
    subroutine getDirAngle(freeStreamAxis, liftAxis, liftIndex, alpha, beta)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(3), intent(in) :: freeStreamAxis
        complex(kind=realType), dimension(3), intent(in) :: liftAxis
        complex(kind=realType), intent(out) :: alpha, beta
        integer(kind=intType), intent(out) :: liftIndex
        !
        !     Local variables.
        !
        complex(kind=realType) :: rnorm
        integer(kind=intType) :: flowIndex, i
        complex(kind=realType), dimension(3) :: freeStreamAxisNorm
    ...
    subroutine stabilityDerivativeDriver
       ...
       use complexify 
        ...
        !     Local variables.
        !
        complex(kind=realType), dimension(8) :: dcdalpha, dcdalphadot, dcdbeta, &
                                             dcdbetadot, dcdMach, dcdMachdot
        complex(kind=realType), dimension(8) :: dcdp, dcdpdot, dcdq, dcdqdot, dcdr, dcdrdot
        complex(kind=realType), dimension(8) :: Coef0, Coef0dot
    ...
    subroutine setCoefTimeIntegrator
       ...
       use complexify 
        ...
        case (firstOrder)
            ...
            if (useALE .and. (equationMode .ceq. unsteady)) then
    ...
    function myNorm2(x)
       ...
       use complexify 
        implicit none
        complex(kind=realType), dimension(3), intent(in) :: x
        complex(kind=realType) :: myNorm2
    ...
    function isWallType(bType)
       ...
       use complexify 
        ...
        isWallType = .False.
        if ((bType .ceq. NSWallAdiabatic) .or. &
            (bType .ceq. NSWallIsoThermal) .or. &
            (bType .ceq. EulerWall)) then
            isWallType = .True.
        end if
    ...
    subroutine cross_prod(a, b, c)
        ...
        ! Inputs
       use complexify 
         complex(kind=realType), dimension(3), intent(in) :: a, b

        ! Outputs
        complex(kind=realType), dimension(3), intent(out) :: c
    ...
    subroutine siAngle(angle, mult, trans)
       ...
       use complexify 
        ...
        complex(kind=realType), intent(out) :: mult, trans

        ! Determine the situation we are having here.

        if (angle .ceq. Radian) then
    ...
    subroutine siDensity(mass, len, mult, trans)
       ...
       use complexify 
        ...
        if ((mass .ceq. Kilogram) .and. (len .ceq. Meter)) then
    ...
    subroutine siPressure(mass, len, time, mult, trans)
       ...
       use complexify 
        ...
        if ((mass .ceq. Kilogram) .and. (len .ceq. Meter) .and. (time .ceq. Second)) then
    ...
    subroutine siTurb(mass, len, time, temp, turbName, mult, trans)
       ...
       use complexify 
        ...
        if ((mass .ceq. Kilogram) .and. (len .ceq. Meter) .and. &
            (time .ceq. Second) .and. (temp .ceq. Kelvin)) then
...
!---------------------------------------------------------------------------
    ...
    subroutine maxEddyv(eddyvisMax)
       ...
       use complexify 
        ...
        !      Subroutine arguments.
        !
        complex(kind=realType), intent(out) :: eddyvisMax
        !
        !      Local variables.
        !
        integer(kind=intType) :: i, j, k

        complex(kind=realType) :: eddyvis
    ...
    subroutine maxHdiffMach(hdiffMax, MachMax)
       ...
       use complexify 
        ...
        !      Subroutine arguments.
        !
        complex(kind=realType), intent(out) :: hdiffMax, MachMax
        !
        !      Local variables.
        !
        integer(kind=intType) :: i, j, k

        complex(kind=realType) :: hdiff, hInf, Mach2
    ...
    subroutine reallocateReal(realArray, newSize, oldSize, &
       ...
       use complexify 
        ...
        !      Subroutine arguments.
        !
        complex(kind=realType), dimension(:), pointer :: realArray
        integer(kind=intType), intent(in) :: newSize, oldSize
        logical, intent(in) :: alwaysFreeMem
        !
        !      Local variables.
        !
        complex(kind=realType), dimension(:), pointer :: tmp
    ...
    subroutine setBufferSizes(level, sps, determine1to1Buf, determineOversetBuf)
       ...
       use complexify 
        ...
        nVarComm = nw + 1
        if (cpModel .ceq. cpTempCurveFits) nVarComm = nVarComm + 1
    ...
    subroutine spectralInterpolCoef(nsps, t, alpScal, alpMat)
       ...
       use complexify 
        ...
        complex(kind=realType), intent(in) :: t

        complex(kind=realType), dimension(0:nsps - 1), intent(out) :: alpScal
        complex(kind=realType), dimension(nSections, 0:nsps - 1, 3, 3), &
            intent(out) :: alpMat
        !
        !      Local variables.
        !
        ...
        complex(kind=realType) :: nspsInv, mInv, tm, alp

        complex(kind=realType), dimension(3, 3) :: rp, tmp
       ...
       use complexify 
        ...
        spectralModes: do mm = 1, nTimeIntervalsSpectral
            domains: do nn = 1, nDom
                ...
                if (smoother .ceq. RungeKutta) then
    ...
    subroutine getLiftDirFromSymmetry(liftDir)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(:, :, :), pointer :: xx
        complex(kind=realType) :: cp(3), v1(3), v2(3)
        ...
        if ((sym(1) .ceq. 0) .and. (sym(2) .ceq. 0) .and. (sym(3) .ceq. 0)) then
            ! Pass - no sym, can't determine lift dir:
        else if ((sym(1) .cne. 0) .and. (sym(2) .ceq. 0) .and. (sym(3) .ceq. 0)) then
            ! Pass - x dir can't be symmetry
        else if ((sym(1) .ceq. 0) .and. (sym(2) .cne. 0) .and. (sym(3) .ceq. 0)) then
            liftDir = 3
        else if ((sym(1) .ceq. 0) .and. (sym(2) .ceq. 0) .and. (sym(3) .cne. 0)) then
            liftDir = 2
        else
       ...
       use complexify 
        ...
        spectralLoop: do sps = 1, nTimeIntervalsSpectral
            ...
            do nn = 1, nDom
                ...
                if ((equationMode .ceq. unSteady) .and. useALE) then
                    deallocate ( &
                ...
                if ((equationMode .ceq. unsteady) .and. &
                    (timeIntegrationScheme .ceq. explicitRK)) then
    ...
    subroutine deallocateBlock(nn, level, sps)
       ...
       use complexify 
        ...
        do i = 1, flowDoms(nn, level, sps)%nViscBocos
            ...
            if (ierr .cne. 0) deallocationFailure = .true.
...
# endif
       ...
       use complexify 
        ...
        do nn = 1, nDom
            do i = 1, flowDoms(nn, 1, 1)%nBocos
                if (flowDoms(nn, 1, 1)%BCType(i) .ceq. EulerWall) &
    ...
    subroutine convergenceHeader
       ...
       use complexify 
        ...
        if (printIterations) then
            ...
            if ((equationMode .ceq. unsteady) .and. &
                (timeIntegrationScheme .ceq. explicitRK)) writeIterations = .false.
            ...
            if (equationMode .ceq. unsteady) then
                nCharWrite = nCharWrite + 7 + fieldWidth + 1
            else if (equationMode .ceq. timeSpectral) then
    ...
    subroutine sumAllResiduals(mm)
       ...
       use complexify 
        ...
        complex(kind=realType) :: state_sum, ovv
    ...
    subroutine getCellCenters(level, n, xCen)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(3, n), intent(out) :: xCen
    ...
    subroutine getCellCGNSBlockIDs(level, n, cellID)
       ...
       use complexify 
        ...
        ! Input/Output
        integer(kind=intType), intent(in) :: level, n
        complex(kind=realType), dimension(n), intent(out) :: cellID
    ...
    subroutine getRotMatrix(vec1, vec2, rotMat)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(3), intent(in) :: vec1, vec2
        complex(kind=realType), dimension(3, 3), intent(out) :: rotMat

        integer(kind=intType) :: i
        complex(kind=realType), dimension(3) :: vec3
        complex(kind=realType), dimension(3, 3) :: Wmat, Wmat2
        complex(kind=realType) :: theta, dotProd, mag1, mag2, cosine
    ...
    complex(kind=realType) function norm2cplx(v)
        ! if input is real:
        ...
        complex(kind=realType), dimension(:), intent(in) :: v

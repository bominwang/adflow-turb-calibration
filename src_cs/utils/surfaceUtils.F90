            ...
            bocos: do mm = 1, nBocos
                famInclude: if (famInList(BCData(mm)%famID, famList)) then
                    ...
                    if (regularOrdering) then
                        ...
                        do j = 0, nj - 2
                            do i = 0, ni - 2
                                if (.not. includeZipper .or. (BCData(mm)%iBlank(i + iBeg + 1, j + jBeg + 1) .ceq. 1)) then
...
#include <petsc/finclude/petsc.h>
        ...
        complex(kind=realType), intent(inout) :: points(3, npts)
        integer(kind=intType), intent(in) :: nFamList, famList(nFamList)
        logical, intent(in) :: includeZipper
    ...
    subroutine mapVector(vec1, n1, famList1, nf1, vec2, n2, famList2, nf2, includeZipper, nDim)
       ...
       use complexify 
        ...
        complex(kind=realType), intent(in) :: vec1(nDim, n1)
        complex(kind=realType), intent(inout) :: vec2(nDim, n2)
        logical, intent(in) :: includeZipper

 ...
 contains
    ...
    subroutine computeTtot(rho, u, v, w, p, Ttot)
       ...
       use complexify 
        ...
        !
        complex(kind=realType), intent(in) :: rho, p, u, v, w
        complex(kind=realType), intent(out) :: Ttot
        !
        !      Local variables.
        !
        integer(kind=intType) :: i

        complex(kind=realType) :: govgm1, T, kin
    ...
    subroutine computeGamma(T, gamma, mm)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(mm), intent(in) :: T
        complex(kind=realType), dimension(mm), intent(out) :: gamma
        !
        !      Local variables.
        !
        integer(kind=intType) :: i, ii, nn, start
        complex(kind=realType) :: cp, T2
    ...
    subroutine computePtot(rho, u, v, w, p, ptot)
       ...
       use complexify 
        ...
        complex(kind=realType), intent(in) :: rho, p, u, v, w
        complex(kind=realType), intent(out) :: ptot
        !
        !      Local parameters.
        !
        complex(kind=realType), parameter :: dtStop = 0.01_realType
        !
        !      Local variables.
        !
        integer(kind=intType) :: i, ii, mm, nn, nnt, start

        complex(kind=realType) :: govgm1, kin
        complex(kind=realType) :: T, T2, tt, dt, h, htot, cp, scale, alp
        complex(kind=realType) :: intCport, intCportT, intCportTt
        !
        ! Determine the cp model used.
        ...
        case (cpTempCurveFits)
            ...
            if (T <= cpTrange(0)) then
                ...
                do ii = 1, cpTempFit(nn)%nterm
                    ...
                    if (cpTempFit(nn)%exponents(ii) .ceq. 0_intType) then
                        intCportT = intCportT &
            ...
            if (htot <= cpHint(0)) then
                ...
                newton: do
                    ...
                    do ii = 1, cpTempFit(nnt)%nterm
                        ...
                        if (cpTempFit(nnt)%exponents(ii) .ceq. -1_intType) then
            ...
            if (Tt <= cpTrange(0)) then
                ...
                intCportTt = zero
                do ii = 1, cpTempFit(nnt)%nterm
                    if (cpTempFit(nnt)%exponents(ii) .ceq. 0_intType) then
    ...
    subroutine computeEtotBlock(iStart, iEnd, jStart, jEnd, kStart, kEnd, &
       ...
       use complexify 
        ...
        complex(kind=realType) :: ovgm1, factK, scale
        !
        ! Determine the cp model used in the computation.
    ...
    subroutine etot(rho, u, v, w, p, k, etotal, correctForK)
       ...
       use complexify 
        ...
        !      Subroutine arguments.
        !
        complex(kind=realType), intent(in) :: rho, p, k
        complex(kind=realType), intent(in) :: u, v, w
        complex(kind=realType), intent(out) :: etotal
        logical, intent(in) :: correctForK
    ...
    subroutine eint(rho, p, k, einternal, correctForK)
       ...
       use complexify 
        ...
        !
        complex(kind=realType), intent(in) :: rho, p, k
        complex(kind=realType), intent(out) :: eInternal
        logical, intent(in) :: correctForK
        !
        !      Local parameter.
        !
        ...
        complex(kind=realType) :: ovgm1, factK, pp, t, t2, scale
        ...
        case (cpTempCurveFits)
            ...
            if (t <= cpTrange(0)) then
                ...
                do ii = 1, cpTempFit(nn)%nterm
                    if (cpTempFit(nn)%exponents(ii) .ceq. -1) then
                        eInternal = eInternal &
                                    + cpTempFit(nn)%constants(ii) * log(t)
                    else
                        mm = cpTempFit(nn)%exponents(ii) + 1
    ...
    subroutine computePressure(iBeg, iEnd, jBeg, jEnd, kBeg, kEnd, &
       ...
       use complexify 
        ...
        complex(kind=realType) :: gm1, factK, v2, scale, e0, e
        complex(kind=realType) :: TRefInv, T, dT, T2, alp, cv
    ...
    subroutine computeLamViscosity(includeHalos)
       ...
       use complexify 
        ...
        !
        integer(kind=intType) :: i, j, k, ii
        complex(kind=realType) :: muSuth, TSuth, SSuth, T, pp
        logical :: correctForK
        integer(kind=intType) :: iBeg, iEnd, iSize, jBeg, jEnd, jSize, kBeg, kEnd, kSize
    ...
    subroutine adjustInflowAngle()
       ...
       use complexify 
        ...
        !Local Vars
        complex(kind=realType), dimension(3) :: refDir1, refDir2
...
!
        ...
        complex(kind=realType) :: phi, dphiX, dphiY, dphiZ
        complex(kind=realType) :: cosX, cosY, cosZ, sinX, sinY, sinZ

        complex(kind=realType), dimension(3, 3) :: dm, m
...
!     Subroutine arguments.
!
        complex(kind=realType), dimension(3), intent(in) :: refDirection
        complex(kind=realType) :: alpha, beta
        complex(kind=realType), dimension(3), intent(out) :: windDirection
        integer(kind=intType) :: liftIndex
!
!     Local variables.
!
        complex(kind=realType) :: rnorm, x1, y1, z1, xbn, ybn, zbn, xw, yw, zw
...
!
        ...
        complex(kind=realType), intent(in) :: angle, x, y, z
        complex(kind=realType), intent(out) :: xp, yp, zp
    ...
    subroutine allNodalGradients
       ...
       use complexify 
        ...
        complex(kind=realType) :: oneOverV, ubar, vbar, wbar, a2
        complex(kind=realType) :: sx, sx1, sy, sy1, sz, sz1
    ...
    subroutine fixAllNodalGradientsFromAD
       ...
       use complexify 
        ...
        ! WorkingVariables
        integer(kind=intType) :: i, j, k
        complex(kind=realType) :: oneOverV
    ...
    subroutine computeEtotCellCpfit(i, j, k, scale, correctForK)
       ...
       use complexify 
        ...
        complex(kind=realType), intent(in) :: scale
        logical, intent(in) :: correctForK
        !
        !      Local variables.
        !
        ...
        complex(kind=realType) :: pp, t, t2, cv, eint

...
contains
    ...
    subroutine referenceState
       ...
       use complexify 
        ...
        complex(kind=realType) :: gm1, ratio
        complex(kind=realType) :: nuInf, ktmp, uInf2
        complex(kind=realType) :: vinf, zinf, tmp1(1), tmp2(1)
    ...
    subroutine infChangeCorrection(oldWinf, correctionTol, correctionType)
       ...
       use complexify 
        ...
        complex(kind=realType), intent(in), dimension(nwf) :: oldWinf
        complex(kind=realType), intent(in) :: correctionTol
        character*(*), intent(in) :: correctionType
        integer(kind=intType) :: sps, nn, i, j, k, l
        complex(kind=realType) :: deltaWinf(nwf)

        ! variables for rotation update
        complex(kind=realType), dimension(3) :: vec1, vec2, vCell
        complex(kind=realType), dimension(3, 3) :: rotMat
        complex(kind=realType) :: mag1, mag2
        ...
        if (correctionType .ceq. "offset") then
        ...
        else if (correctionType .ceq. "rotate") then
    ...
    subroutine allocMemFlovarPart1(sps, level)
       ...
       use complexify 
        ...
        domains: do nn = 1, nDom
            ...
            if (level <= mgStartlevel .or. (turbTreatment .ceq. coupled)) then
            ...
            fineLevelTest: if (level .ceq. 1) then
                ...
                if ((equationMode .ceq. unsteady) .and. &
                    (timeIntegrationScheme .ceq. BDF)) then
                    allocate ( &
                    ...
                    ! Added by HDN
                else if ((equationMode .ceq. unsteady) .and. &
                         (timeIntegrationScheme .ceq. MD)) then
                    allocate ( &
                        flowDoms(nn, level, sps)%wOld(nOldLevels, 2:il, 2:jl, 2:kl, nw), &
                        stat=ierr)
                    if (ierr .cne. 0) &
                ...
                sps1RansTest: if (sps .ceq. 1) then
    ...
    subroutine copySpectralSolution
       ...
       use complexify 
        ...
        complex(kind=realType) :: dt, tnew, told, tmp
        complex(kind=realType) :: theta, cosTheta, sinTheta

        complex(kind=realType), dimension(3) :: rotPoint, uu
        complex(kind=realType), dimension(3) :: xt, yt, zt
        complex(kind=realType), dimension(3, 3) :: rotMat

        complex(kind=realType), dimension(nSections, 3, 3) :: rotMatSec
    ...
    subroutine determineSolFileNames
       ...
       use complexify 
        ...
        case (unsteady)
            ...
            if ((myID .ceq. 0) .and. nOldSolAvail < nOldLevels) then
        ...
        case (timeSpectral)
            ...
            if (nSolsRead .ceq. 1) then
                copySpectral = .true.
            else if (nSolsRead .cne. nTimeIntervalsSpectral) then
    ...
    subroutine initDepvarAndHalos(halosRead)
       ...
       use complexify 
        ...
        !
        integer(kind=intType) :: nn, mm
        complex(kind=realType) :: relaxBleedsOr
    ...
    subroutine initFlowfield
       ...
       use complexify 
        ...
        if (equationMode .ceq. unsteady) nn = nTimeStepsFine * nn
        if (myID .ceq. 0) call allocConvArrays(nn)
        if ((equationMode .ceq. unsteady) .and. (myId .ceq. 0)) &
    ...
    subroutine interpolateSpectralSolution
       ...
       use complexify 
        ...
        complex(kind=realType) :: t

        complex(kind=realType), dimension(nSolsRead) :: alpScal
        complex(kind=realType), dimension(nSections, nSolsRead, 3, 3) :: alpMat
        ...
        spectralLoop: do sps = 1, nTimeIntervalsSpectral
            ...
            domains: do nn = 1, nDom
                ...
                varLoop: do l = 1, nw
                    ...
                    velTest: if ((l .ceq. ivx) .or. (l .ceq. ivy) .or. (l .ceq. ivz)) then
    ...
    subroutine setUniformFlow
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(3) :: dirLoc, dirGlob
        ...
        testCorrection: if ((equationMode .ceq. timeSpectral) .and. &
    ...
    subroutine velMagnAndDirectionSubface(vmag, dir, BCData, mm)
       ...
       use complexify 
        ...
        complex(kind=realType), intent(out) :: vmag
        complex(kind=realType), dimension(3), intent(inout) :: dir
    ...
    subroutine timeSpectralCoef(coefSpectral, matrixCoefSpectral, &
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(nSections, 3, 3), &
            intent(out) :: diagMatCoefSpectral
        !
        !      Local variables.
        !
        integer(kind=intType) :: pp, nn, mm, ii, i, j, ntot
        complex(kind=realType) :: coef, dAngle, angle, fact, slicesFact

        complex(kind=realType), dimension(3, 3) :: rotMat, tmp
        ...
        sectionLoop: do mm = 1, nSections
            ...
            scalarLoop: do nn = 1, (nTimeIntervalsSpectral - 1)
                ...
                if (mod(nTimeIntervalsSpectral, 2_intType) .ceq. 0) &
            ...
            slicesLoop: do pp = 1, (sections(mm)%nSlices - 1)
                ...
                fact = one
                if (mod(pp * nTimeIntervalsSpectral, 2_intType) .cne. 0) &
                    fact = -one
    ...
    subroutine timeSpectralMatrices
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(3, 3) :: tmpMat

        complex(kind=realType), dimension(:, :), allocatable :: coefSpectral
        ...
        if (equationMode .cne. timeSpectral) return
    ...
    subroutine readRestartFile()
       ...
       use complexify 
        ...
        fileOpenLoop: do solID = 1, nSolsRead
            ...
            if ((cellDim .cne. 3) .or. (physDim .cne. 3)) then
        ...
        solLoop: do solID = 1, nSolsRead
            ...
            domains: do nn = 1, nDom
                ...
                if ((cgnsDoms(nbkGlobal)%il .cne. sizes(1)) .or. &
                     (cgnsDoms(nbkGlobal)%jl .cne. sizes(2)) .or. &
                     (cgnsDoms(nbkGlobal)%kl .cne. sizes(3))) &
                     call terminate("readRestartFile", &
                ...
                do cgnsSol = 1, nSols
                    ...
                    if (trim(cgnsName) .cne. "Nodal Blanks") exit
                end do
                ...
                if ((solID .ceq. 1) .or. (equationMode .ceq. timeSpectral)) then
                    if ((rindSizes(1) .ceq. 0) .or. (rindSizes(2) .ceq. 0) .or. (rindSizes(3) .ceq. 0) .or. &
                        (rindSizes(4) .ceq. 0) .or. (rindSizes(5) .ceq. 0) .or. (rindSizes(6) .ceq. 0)) &
                        halosRead = .false.
                end if
                ...
                if (location .ceq. CellCenter) then
                ...
                else if (location .ceq. Vertex) then
                ...
                testPrim: if ((solID .ceq. 1) .or. (equationMode .ceq. timeSpectral)) then
    ...
    subroutine getSortedZoneNumbers
       ...
       use complexify 
        ...
        cgnsDomains: do nn = 1, cgnsNdom
            ...
            if (zonetype .cne. structured) then

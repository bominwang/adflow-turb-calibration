 ...
 contains
       ...
       use complexify 
        ...
        case (unsteady)
            ...
            testMultipleUnsteady: if (deforming_Grid .and. .false.) then
                ...
                if (ii .ceq. len_trim(gridFile)) then
        ...
        case (timeSpectral)
            ...
            testMultipleTS: if (deforming_Grid .and. .false.) then
                ...
                if (ii .ceq. len_trim(gridFile)) then
                    ...
                    if (nGridsRead .ceq. nTimeIntervalsSpectral) &
    ...
    subroutine determineInterfaceIDs
       ...
       use complexify 
        ...
        do nn = 1, cgnsNDom
            do mm = 1, cgnsDoms(nn)%nBocos
                if (cgnsDoms(nn)%bocoInfo(mm)%actualFace .and. &
                    (cgnsDoms(nn)%bocoInfo(mm)%familyID .ceq. 0)) then
        ...
        do ii = 1, cgnsNSliding
            ...
            validInterface = .true.
            if (len_trim(namesSorted(nn)) .cne. len_trim(namesSorted(mm))) &
                validInterface = .false.

            jj = len_trim(namesSorted(nn)) - 2
            if (jj <= 0) then
                validInterface = .false.
            else if (namesSorted(nn) (:jj) .cne. namesSorted(mm) (:jj)) then
                validInterface = .false.
            end if
        ...
        do nn = 1, cgnsNDom
            do mm = 1, cgnsDoms(nn)%nBocos
                if (cgnsDoms(nn)%bocoInfo(mm)%actualFace .and. &
                    ...
                    (cgnsDoms(nn)%bocoInfo(mm)%BCType .ceq. SlidingInterface)) then
    ...
    subroutine timePeriodSpectral
       ...
       use complexify 
        ...
        !      Local parameter.
        !
        complex(kind=realType), parameter :: tol = 1.e-5_realType
        !
        !      Local variables.
        !
        integer :: ierr
        ...
        complex(kind=realType) :: tt, omega
        complex(kind=realType) :: timePeriod
        ...
        externalTest: if (flowType .ceq. externalFlow) then
...
!!$         endif
    ...
    function commonTimeSpectral(t1, t2)
       ...
       use complexify 
        ...
        !      Function definition
        !
        complex(kind=realType) :: commonTimeSpectral
        !
        !      Function arguments.
        !
        complex(kind=realType), intent(in) :: t1, t2
        !
        !      Local parameters.
        !
        ...
        complex(kind=realType) :: tt1, tt2, tt, ratio
    ...
    subroutine timeRotMatricesSpectral
       ...
       use complexify 
        ...
        integer(kind=intType) :: nn
        complex(kind=realType) :: tmp, theta, cosTheta, sinTheta
        ...
        sectionLoop: do nn = 1, nSections
            ...
            testUnity: if (.not. sections(nn)%rotating .or. &
                           (sections(nn)%nSlices .ceq. 1)) then
    ...
    subroutine fineGridSpectralCoor
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(nSections) :: dt, t
    ...
    subroutine updateCoorFineMesh(dtAdvance, sps)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(*), intent(in) :: dtAdvance
        !
        !      Local variables.
        !
        integer(kind=intType) :: i, j, k, nn

        complex(kind=realType) :: displX, displY, displZ
        complex(kind=realType) :: tNew, tOld
        complex(kind=realType) :: angleX, angleY, angleZ, dx, dy, dz
        complex(kind=realType) :: xiX, xiY, xiZ, etaX, etaY, etaZ
        complex(kind=realType) :: zetaX, zetaY, zetaZ, xp, yp, zp, t
        complex(kind=realType) :: phi, cosPhi, sinPhi, eta, zeta
    ...
    subroutine allocCoorFineGrid
       ...
       use complexify 
        ...
        blockLoop: do nn = 1, nDom
            ...
            if (deforming_Grid .and. (equationMode .ceq. unsteady)) then
    ...
    subroutine checkPartitioning(np, load_inbalance, face_inbalance)
       ...
       use complexify 
        ...
        complex(kind=realType), intent(out) :: load_inbalance, face_inbalance
    ...
    subroutine determineSections
       ...
       use complexify 
        ...
        !
        complex(kind=realType), parameter :: threshold = 0.1_realType
        !
        !      Local variables.
        !
        integer :: ierr
        ...
        complex(kind=realType) :: r11, r12, r13, r21, r22, r23
        complex(kind=realType) :: r31, r32, r33
        complex(kind=realType) :: d1, d2, a1, a0, lamr, lami, angle, dAngle
        ...
        do nn = 1, nDom
            ...
            if (debug) then
                if (mm .ceq. 0) call terminate("determineSections", &
        ...
        loopCGNSDom: do nn = 1, cgnsNDom
            ...
            if (debug) then
                if (ii .ceq. 0) call terminate("determineSections", &
                                            "Entry not found in sorted.")
            end if
            ...
            if (angle .ceq. zero) angle = two * pi
        ...
        do nn = 1, cgnsNDom
            do mm = 1, cgnsDoms(nn)%nBocos
                if (cgnsDoms(nn)%bocoInfo(mm)%actualFace .and. &
                    ...
                    ii = bsearchIntegers(sectionID(nn), sorted(1:nSections))
                    if (secSliding(slideID, 1) .ceq. 0) then
                        secSliding(slideID, 1) = ii
                    else if (secSliding(slideID, 1) .cne. ii) then

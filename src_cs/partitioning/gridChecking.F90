 ...
 contains
    ...
    subroutine checkFacesBlock(blockIsBad, faceID, multiple)
       ...
       use complexify 
        ...
        do k = 2, kl
            do j = 2, jl
                if (iMinFace(j, k) .cne. 1) then
                    multiple = iMinFace(j, k)
                    faceID = iMin
                    iMinBad = .true.
                end if

                if (iMaxFace(j, k) .cne. 1) then
        ...
        do k = 2, kl
            do i = 2, il
                if (jMinFace(i, k) .cne. 1) then
                    multiple = jMinFace(i, k)
                    faceID = jMin
                    jMinBad = .true.
                end if

                if (jMaxFace(i, k) .cne. 1) then
        ...
        do j = 2, jl
            do i = 2, il
                if (kMinFace(i, j) .cne. 1) then
                    multiple = kMinFace(i, j)
                    faceID = kMin
                    kMinBad = .true.
                end if

                if (kMaxFace(i, j) .cne. 1) then
    ...
    subroutine check1to1Subfaces
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(:), allocatable :: badDist
        complex(kind=realType), dimension(:), allocatable :: badDistGlobal
        complex(kind=realType), dimension(:, :), allocatable :: realBuf
        complex(kind=realType), dimension(:, :), allocatable :: realRecv
        ...
        do ii = 1, nMessagesReceive
            ...
            if (debug) then
                if ((size .ceq. mpi_undefined) .or. (mod(size, 10) .cne. 0)) &
                    call terminate("check1to1Subfaces", &
            ...
            if (debug) then
                if ((size .ceq. mpi_undefined) .or. (mod(size, 3) .cne. 0)) &
                    call terminate("check1to1Subfaces", &
        ...
        if ((myID .ceq. 0) .and. nBadGlobal > 0) then
    ...
    subroutine periodicTransformSubface(coor, nn, rotCenter, &
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(3), intent(in) :: rotCenter
        complex(kind=realType), dimension(3), intent(in) :: rotAngles
        complex(kind=realType), dimension(3), intent(in) :: translation

        complex(kind=realType), dimension(3, nn), intent(inout) :: coor
        !
        !      Local variables.
        !
        ...
        complex(kind=realType) :: dx, dy, dz

        complex(kind=realType), dimension(3) :: trans
    ...
    subroutine checkSubfaceCoor(subfaceInfo, coor, nFace, &
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(*), intent(inout) :: badDist
        complex(kind=realType), dimension(3, *), intent(in) :: coor
        !
        !      Local parameter.
        !
        complex(kind=realType), parameter :: relTol = 0.05_realType
        !
        !      Local variables.
        !
        ...
        complex(kind=realType) :: dist2, dist2I, dist2J, dist2K, tol
        complex(kind=realType) :: factI, factJ, factK, diffMax
        ...
        subfaceLoop: do nn = 1, nFace
            ...
            do mm = 1, n1to1
                ...
                if ((min(iBeg, iEnd) .ceq. min(inBeg(jj), inEnd(jj))) .and. &
                    (max(iBeg, iEnd) .ceq. max(inBeg(jj), inEnd(jj))) .and. &
                    (min(jBeg, jEnd) .ceq. min(jnBeg(jj), jnEnd(jj))) .and. &
                    (max(jBeg, jEnd) .ceq. max(jnBeg(jj), jnEnd(jj))) .and. &
                    (min(kBeg, kEnd) .ceq. min(knBeg(jj), knEnd(jj))) .and. &
                    (max(kBeg, kEnd) .ceq. max(knBeg(jj), knEnd(jj)))) exit
            ...
            do k = kBeg, kEnd, stepK
                ...
                factK = one
                if (km1 .ceq. k) factK = factK * four
                if (kp1 .ceq. k) factK = factK * four
                ...
                do j = jBeg, jEnd, stepJ
                    ...
                    factJ = one
                    if (jm1 .ceq. j) factJ = factJ * four
                    if (jp1 .ceq. j) factJ = factJ * four
                    ...
                    do i = iBeg, iEnd, stepI
                        ...
                        factI = one
                        if (im1 .ceq. i) factI = factI * four
                        if (ip1 .ceq. i) factI = factI * four

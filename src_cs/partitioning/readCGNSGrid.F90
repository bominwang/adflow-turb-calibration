 ...
 contains
    ...
    subroutine readBlockSizes
       ...
       use complexify 
        ...
        if ((cgnsCelldim .cne. 3) .or. (cgnsPhysdim .cne. 3)) then
        ...
        do nn = 2, nGridsRead
            ...
            if (cgnsNzones .cne. cgnsNDom) then
                write (errorMessage, *) "File ", trim(gridFiles(nn)), &
                     ": Different number of blocks than&
                     & in file ", trim(gridFiles(1))
                if (myID .ceq. 0) call terminate("readBlockSizes", errorMessage)
        ...
        if ((myID .ceq. 0) .and. nDoubleBoundFaces > 0) then
        ...
        if ((myID .ceq. 0) .and. noUnits) then

            if ((equations .ceq. NSEquations) .or. (equations .ceq. RANSEquations) .or. (equationMode .ceq. unsteady)) then
       ...
       use complexify 
        ...
        nFam: do nn = 1, cgnsNFamilies
            ...
            case (1)
                ...
                testUserDefined: if (cgnsFamilies(nn)%BCTypeCGNS .ceq. &
                                     CG_UserDefined) then
    ...
    subroutine readZoneInfo(cgnsBase, nZone, sortedFamName, &
       ...
       use complexify 
        ...
        if (cgnsDoms(nZone)%zonetype .cne. Structured) then
            write (errorMessage, *) "Zone ", &
                trim(cgnsDoms(nZone)%zoneName), &
                " of the grid file is not structured"
            if (myID .ceq. 0) call terminate("readZoneInfo", errorMessage)
            call mpi_barrier(ADflow_comm_world, ierr)
        end if
        ...
        do nn = 2, nGridsRead
            ...
            if ((cgnsDoms(nZone)%il .cne. sizesBlock(1)) .or. &
                (cgnsDoms(nZone)%jl .cne. sizesBlock(2)) .or. &
                (cgnsDoms(nZone)%kl .cne. sizesBlock(3))) then
        ...
        do i = 1, 3
            ...
            if ((ierr .ceq. CG_OK) .and. (len .cne. CG_Null)) then
        ...
        if (cgnsDoms(nZone)%gridUnitsSpecified) then
            ...
            if ((myID .ceq. 0) .and. LRefSpecified .and. &
                (cgnsDoms(nZone)%LRef .cne. LRef)) then
        ...
        testOverwrite: if (overwrite) then
            ...
            if (ierr .ceq. CG_OK) then
                ...
                if ((ierr .ceq. CG_OK) .and. (angle .cne. CG_Null)) then
       ...
       use complexify 
        ...
        do i = 1, ngeneral
            ...
            case (Abutting1to1)
                ...
                if ((location .ceq. Vertex) .and. &
                    (ptsetType .ceq. PointRange) .and. &
                    (donorZoneType .ceq. Structured) .and. &
                    (donorPtsetType .ceq. PointListDonor)) then
        ...
        do i = 2, nNonMatch
            mm = i - 1
            if ((subfaceNonMatch(i)%iBeg .ceq. subfaceNonMatch(mm)%iBeg) .and. &
                (subfaceNonMatch(i)%jBeg .ceq. subfaceNonMatch(mm)%jBeg) .and. &
                (subfaceNonMatch(i)%kBeg .ceq. subfaceNonMatch(mm)%kBeg) .and. &
                (subfaceNonMatch(i)%iEnd .ceq. subfaceNonMatch(mm)%iEnd) .and. &
                (subfaceNonMatch(i)%jEnd .ceq. subfaceNonMatch(mm)%jEnd) .and. &
                (subfaceNonMatch(i)%kEnd .ceq. subfaceNonMatch(mm)%kEnd)) then
    ...
    subroutine read1to1Conn(cgnsInd, cgnsBase, nZone)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(3) :: rotCenter, rotAngles
        complex(kind=realType), dimension(3) :: tlation
    ...
    subroutine checkTransform(transform, nZone, n1to1, printWarning)
       ...
       use complexify 
        ...
        do nDirFace = 1, 3
            if (zoneRange(nDirFace, 1) .ceq. zoneRange(nDirFace, 2)) exit
        end do
        ...
        do nDirDonor = 1, 3
            if (donorRange(nDirDonor, 1) .ceq. donorRange(nDirDonor, 2)) exit
        end do
        ...
        if (sumTransform .cne. 6) then
            ...
            if (sumTransform .cne. 6) then
                ...
                if ((myID .ceq. 0) .and. printWarning) then
                    print "(a)", "#"
                    print "(a)", "#                          Warning"
                    write (errorMessage, strings) "1 to 1 subface ", trim(connectName), " of zone ", trim(zoneName), &
        ...
        haloDir = 0
        haloDir(nDirFace) = 1
        if (zoneRange(nDirFace, 1) .ceq. 1) haloDir(nDirFace) = -1
        ...
        if (donorRange(nDirDonor, 1) .ceq. 1) donorDir(nDirDonor) = 1
        ...
        if (haloDir(nDirDonor) .ceq. 0) then
    ...
    subroutine readGeneralConn(cgnsInd, cgnsBase, nZone)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(3) :: rotationCenter
        complex(kind=realType), dimension(3) :: rotationAngles
        complex(kind=realType), dimension(3) :: translation
        ...
        nConnLoop: do nn = 1, ngeneral
            ...
            do jj = 1, 3
                if (myRange(jj, 1) .cne. myRange(jj, 2)) exit
            end do
            ...
            if (donorData(1, 1) .ceq. donorData(1, ndataDonor)) then

                if (donorData(1, 1) .ceq. 1) then
                    connNonMatch(i)%donorFaceIDs(j) = iMin
                else
                    connNonMatch(i)%donorFaceIDs(j) = iMax
                end if

            else if (donorData(2, 1) .ceq. donorData(2, ndataDonor)) then

                if (donorData(2, 1) .ceq. 1) then
                    connNonMatch(i)%donorFaceIDs(j) = jMin
                else
                    connNonMatch(i)%donorFaceIDs(j) = jMax
                end if

            else if (donorData(3, 1) .ceq. donorData(3, ndataDonor)) then

                if (donorData(3, 1) .ceq. 1) then
            ...
            checkConsistency: if (j .ceq. 1) then
                ...
                if (ii(1) .cne. ii(2)) wrongData = .true.
    ...
    subroutine readBocos(cgnsInd, cgnsBase, nZone, &
       ...
       use complexify 
        ...
        bocoLoop: do i = 1, cgnsNBocos
            ...
            if (cgnsDoms(nZone)%bocoInfo(i)%ptsetType .ceq. pointrange) then
            ...
            else if (cgnsDoms(nZone)%bocoInfo(i)%ptsetType .ceq. pointlist) then
            ...
            match = 0
            if (bcRange(1, 1) .ceq. bcRange(1, 2)) match = match + 1
            if (bcRange(2, 1) .ceq. bcRange(2, 2)) match = match + 1
            if (bcRange(3, 1) .ceq. bcRange(3, 2)) match = match + 1
            ...
            checkActualFace: if (cgnsDoms(nZone)%bocoInfo(i)%actualFace) then
                ...
                case (FamilySpecified)
                    ...
                    if (ierr .cne. CG_OK) then
                        ...
                        if (myID .ceq. 0) call terminate("readBocos", errorMessage)
                        call mpi_barrier(ADflow_comm_world, ierr)
                ...
                case (CG_UserDefined)
                    ...
                    if (cgnsDoms(nZone)%bocoInfo(i)%BCType .ceq. BCNull) then
                        write (errorMessage, strings) "Zone ", trim(cgnsDoms(nZone)%zoneName), &
                            ", boundary face ", trim(cgnsDoms(nZone)%bocoInfo(i)%bocoName), &
    ...
    contains
       ...
       use complexify 
            ...
            loopDataArrays: do k = 1, nArrays
                ...
                if (ierr .cne. 0) call terminate("loopDataArrays", &
    ...
    logical function checkForDoubleBoundFace(nZone, nBound)
       ...
       use complexify 
        ...
        do i = 1, cgnsDoms(nZone)%n1to1
            ...
            if ((rangeBound(1, 1) .ceq. rangeFace(1, 1)) .and. &
                (rangeBound(1, 2) .ceq. rangeFace(1, 2)) .and. &
                (rangeBound(2, 1) .ceq. rangeFace(2, 1)) .and. &
                (rangeBound(2, 2) .ceq. rangeFace(2, 2)) .and. &
                (rangeBound(3, 1) .ceq. rangeFace(3, 1)) .and. &
                (rangeBound(3, 2) .ceq. rangeFace(3, 2))) then
    ...
    function internalBC(cgnsBocoType, userDefinedName)
       ...
       use complexify 
        ...
        case (BCWall, BCWallViscous, BCWallViscousHeatFlux)
            ...
            if (equations .ceq. EulerEquations) internalBC = EulerWall
        ...
        case (internalFlow)
            ...
            if (internalBC .ceq. FarField) internalBC = bcNotValid
    ...
    subroutine readPeriodicSubface(cgnsInd, cgnsBase, zone, conn, &
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(3), intent(out) :: rotationCenter
        complex(kind=realType), dimension(3), intent(out) :: rotationAngles
        complex(kind=realType), dimension(3), intent(out) :: translation
        ...
        testPeriodic: if (ierr .ceq. CG_OK) then
       ...
       use complexify 
        ...
        domainLoop: do i = 1, nDom
            ...
            nGridLoop: do nn = 1, nGridsRead
                ...
                checkNcoords: if (nCoords .ceq. 3) then
            ...
            if (ierr .cne. 0) call terminate("readGrid", &

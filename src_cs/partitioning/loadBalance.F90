 ...
 contains
    ...
    subroutine loadBalanceGrid
       ...
       use complexify 
        ...
        nDom = 0
        do i = 1, nBlocks
            if (part(i) .ceq. myID) nDom = nDom + 1
        ...
        domains: do i = 1, nBlocks
            myBlock: if (part(i) .ceq. myID) then
                ...
                if (deforming_Grid .and. &
                    ((equationMode .ceq. unsteady) .or. &
                     (equationMode .ceq. timeSpectral))) &
                    flowDoms(nn, 1, 1:mm)%blockIsMoving = .true.
                ...
                do j = 1, blocks(i)%nSubface
                    ...
                    if ((flowDoms(nn, 1, 1)%BCType(j) .ceq. NSWallAdiabatic) .or. &
                        (flowDoms(nn, 1, 1)%BCType(j) .ceq. NSWallIsothermal)) &
    ...
    subroutine blockDistribution
       ...
       use complexify 
        ...
        if (commNeglected .and. (myID .ceq. 0)) then
            print "(a)", "#"
            print "(a)", "#                    Warning"
    ...
    contains
       ...
       use complexify 
            ...
            do nn = 1, tmpBlock%nSubface
                if (tmpBlock%inBeg(nn) .ceq. tmpBlock%inEnd(nn)) then
                    tmpBlock%BCFaceID(nn) = iMax
                    if (tmpBlock%inBeg(nn) .ceq. 1) tmpBlock%BCFaceID(nn) = iMin
                else if (tmpBlock%jnBeg(nn) .ceq. tmpBlock%jnEnd(nn)) then
                    tmpBlock%BCFaceID(nn) = jMax
                    if (tmpBlock%jnBeg(nn) .ceq. 1) tmpBlock%BCFaceID(nn) = jMin
                else
                    tmpBlock%BCFaceID(nn) = kMax
                    if (tmpBlock%knBeg(nn) .ceq. 1) tmpBlock%BCFaceID(nn) = kMin
                end if
            end do
       ...
       use complexify 
            ...
            do i = 2, nProc
                if (nCell(i) .ceq. nCell(nCellDiff)) then
    ...
    subroutine sortSubfaces(oldSubfaceID, blockID)
       ...
       use complexify 
        ...
        do i = 2, blockID%nBocos
            if (bcPriorSort(i) .ceq. bcPriorSort(nDiff)) then
       ...
       use complexify 
        ...
        bocoLoop: do j = 1, cgnsDoms(cgnsID)%nBocos
            ...
            overlap: if (iBeg <= blocks(ii)%iEndor .and. &
                ...
                mm = 0
                if (iBeg .ceq. iEnd) mm = mm + 1
                if (jBeg .ceq. jEnd) mm = mm + 1
                if (kBeg .ceq. kEnd) mm = mm + 1
                ...
                if (iBeg .ceq. iEnd) then

                    blocks(ii)%BCFaceID(jj) = iMax
                    if (iBeg .ceq. blocks(ii)%iBegor) blocks(ii)%BCFaceID(jj) = iMin

                else if (jBeg .ceq. jEnd) then

                    blocks(ii)%BCFaceID(jj) = jMax
                    if (jBeg .ceq. blocks(ii)%jBegor) blocks(ii)%BCFaceID(jj) = jMin

                else

                    blocks(ii)%BCFaceID(jj) = kMax
                    if (kBeg .ceq. blocks(ii)%kBegor) blocks(ii)%BCFaceID(jj) = kMin
                ...
                if (blocks(ii)%BCType(jj) .ceq. BCNotValid) then
       ...
       use complexify 
        ...
        n1to1Loop: do j = 1, cgnsDoms(cgnsID)%n1to1
            ...
            overlap: if (iBeg <= blocks(ii)%iEndor .and. &
                ...
                kk = 0
                if (iBeg .ceq. iEnd) kk = kk + 1
                if (jBeg .ceq. jEnd) kk = kk + 1
                if (kBeg .ceq. kEnd) kk = kk + 1
                ...
                donorLoop: do k = 1, splitInfo(mm)%nSubblocks
                    ...
                    donorOverlap: if (diBeg <= ranges(k, 1, 2) .and. &
                        ...
                        if (iBeg .ceq. iEnd) then
                            ...
                            blocks(ii)%BCFaceID(jj) = iMax
                            if (iBeg .ceq. blocks(ii)%iBegor) &
                                blocks(ii)%BCFaceID(jj) = iMin

                        else if (jBeg .ceq. jEnd) then

                            blocks(ii)%BCFaceID(jj) = jMax
                            if (jBeg .ceq. blocks(ii)%jBegor) &
                                blocks(ii)%BCFaceID(jj) = jMin

                        else

                            blocks(ii)%BCFaceID(jj) = kMax
                            if (kBeg .ceq. blocks(ii)%kBegor) &
    ...
    contains
       ...
       use complexify 
            ...
            do mm = 1, splitInfo%nSubblocks
                ...
                if (ranges(mm, i, i2) .ceq. indFace) then
    ...
    subroutine graphPartitioning(emptyPartitions, commNeglected)
       ...
       use complexify 
        ...
        complex(kind=4) :: ubvec_temp(2) ! Explict float for ubvec
        !
        ...
        graphVertex: do i = 1, nBlocks
            ...
            do j = 1, blocks(i)%n1to1
                ...
                if (blocks(i)%neighBlock(ii) .cne. i) then
                    nEdges = nEdges + 1
            ...
            ii = min(nEdges, 1_intType)  ! Be aware of nEdges .ceq. 0
            do j = 2, nEdges
                if (tmp(j) .cne. tmp(ii)) then
                    ii = ii + 1
                    tmp(ii) = tmp(j)
                end if
            end do
    ...
    subroutine splitBlock(compBlock, nSub, nCells, ranges)
       ...
       use complexify 
        ...
        do nn = 1, compBlock%nBocos
            ...
            if ((compBlock%BCType(nn) .ceq. NSWallAdiabatic) .or. &
                (compBlock%BCType(nn) .ceq. NSWallIsothermal)) then
        ...
        if (smoother .ceq. RungeKutta) viscousDir = .false.
       ...
       use complexify 
        ...
        loopDir: do mm = 1, 3
            ...
            do
                ...
                if (nf(mm) .ceq. nMG) exit
                if (mod(nc(mm), ii) .cne. 0) exit
        ...
        if (nMG > 1 .and. (max(c(1), c(2), c(3)) .ceq. 1)) then

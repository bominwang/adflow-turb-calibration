...
contains
    ...
    subroutine checkMonitor
       ...
       use complexify 
        ...
        do i = 1, nn
            ...
            case (cgnsClv)
                sortNumber(i) = 103
                if ((equations .ceq. EulerEquations) .or. &
                    (flowType .ceq. internalFlow)) then
                    sortNumber(i) = 11003
                    nMonSum = nMonSum - 1
                end if
        ...
        monMachOrHMax = .false.
        do i = (nMonSum + 1), nMon
            if ((monNames(i) .ceq. cgnsHdiffMax) .or. &
                (monNames(i) .ceq. cgnsMachMax)) monMachOrHMax = .true.
        end do
    ...
    subroutine dummyreadParamFile
       ...
       use complexify 
        ...
        if (cpModel .ceq. cpTempCurveFits) call readCpTempCurveFits
    ...
    subroutine extractMGInfo
       ...
       use complexify 
        ...
        if ((equationMode .ceq. unsteady) .and. &
            (timeIntegrationScheme .ceq. explicitRK)) mgDescription = "sg"
        ...
        if (mgDescription .ceq. "sg") then
            ...
            if (error .cne. 0) call terminate("extractMgInfo", &
        ...
        else if (mgDescription(stringLen:stringLen) .ceq. "v") then
            ...
            if (.not. digitsOnlyInString(mgDescription(:stringLen - 1))) then
                ...
                if (myID .ceq. 0) call terminate("extractMgInfo", errorMessage)
            end if
        ...
        else if (mgDescription(stringLen:stringLen) .ceq. "w") then
            ...
            nMinus = 0
            do i = 1, stringLen
                if (mgDescription(i:i) .cne. " ") then
                    ii = ii + 1
                    mgDescription(ii:ii) = mgDescription(i:i)
                    if (mgDescription(ii:ii) .ceq. "-") nMinus = nMinus + 1
                end if
            end do
            stringLen = ii
            ...
            do
                ii = i
                if (mgDescription(i:i) .ceq. "-") i = i + 1
    ...
    recursive function computeNstepsWcycle(nLevels) result(nSteps)
       ...
       use complexify 
        ...
        if (nLevels < 2) then
            ...
            if (myID .ceq. 0) call terminate("computeNstepsWcycle", errorMessage)
        else if (nLevels .ceq. 2) then
            nSteps = 4
        else
    ...
    recursive subroutine setEntriesWcycle(counter, nLevels)
       ...
       use complexify 
        ...
        if (nLevels < 2) then
            ...
            write (errorMessage, *) "Wrong value of nLevels", nLevels
            if (myID .ceq. 0) call terminate("setEntriesWcycle", errorMessage)
    ...
    subroutine isoVariables(variables)
       ...
       use complexify 
        ...
        do
            ...
            if (len_trim(variables) .ceq. 0) exit
    ...
    subroutine monitorVariables(variables)
       ...
       use complexify 
        ...
        if (allocated(monNames)) then
            ...
            if (ierr .cne. 0) call terminate("monitorVariables", &
    ...
    subroutine readCpTempCurveFits
       ...
       use complexify 
        ...
        complex(kind=realType) :: T1, T2, e0
        ...
        do ii = 1, cpTempFit(1)%nterm
            ...
            if (cpTempFit(1)%exponents(ii) .ceq. -1_intType) then
        ...
        nRanges2: do nn = 2, cpNparts
            ...
            do ii = 1, cpTempFit(mm)%nterm
                if (cpTempFit(mm)%exponents(ii) .ceq. -1_intType) then
            ...
            do ii = 1, cpTempFit(nn)%nterm
                if (cpTempFit(nn)%exponents(ii) .ceq. -1_intType) then
    ...
    subroutine findNextInfoLine(readUnit, string)
       ...
       use complexify 
        ...
        do
            ...
            if ((len_trim(string) > 0) .and. ((string(:1) .cne. "#"))) exit
        end do
    ...
    subroutine checkInputParam
       ...
       use complexify 
        ...
        complex(kind=realType) :: vecLength, dot
        ...
        if (spaceDiscrCoarse .ceq. none) spaceDiscrCoarse = spaceDiscr

        if (riemannCoarse .ceq. none) riemannCoarse = riemann

        ! Set dirScaling to .false. if a scheme other than scalar
        ! dissipation is used.

        if (spaceDiscr .cne. dissScalar) dirScaling = .false.

        ! Determine whether or not the spectral radIi are needed for
        ! the flux computations.

        radiiNeededFine = .false.
        if (spaceDiscr .ceq. dissScalar) radiiNeededFine = .true.

        radiiNeededCoarse = .false.
        if (spaceDiscrCoarse .ceq. dissScalar) radiiNeededCoarse = .true.
        !
        ...
        if (surfaceSolFile .ceq. "") &
            surfaceSolFile = trim(solfile)//"Surface"

        if ((cpModel .ceq. cpTempCurveFits) .and. (cpFile .ceq. "")) then
            if (myID .ceq. 0) &
                call terminate("checkInputParam", &
        ...
        if ((fileFormatRead .ceq. cgnsFormat) .or. &
            (fileFormatWrite .ceq. cgnsFormat)) then
            if (myID .ceq. 0) &
                call terminate("checkInputParam", &
        ...
        if (equationMode .ceq. unsteady) then
            if (timeIntegrationScheme .ceq. explicitRK) &
        ...
        !
        if ((flowType .ceq. internalFlow) .and. gridMotionSpecified) then
            if (myID .ceq. 0) &
                 call terminate("checkInputParam", &
        ...
        if ((equations .ceq. RANSEquations) .and. (turbModel .ceq. none)) then
            if (myID .ceq. 0) &
                call terminate("checkInputParam", &
        ...
        if (flowType .ceq. externalFlow) then
        ...
        if (liftDirSpecified .and. (flowType .ceq. externalFlow)) then
        ...
        !
        testSpectral: if (equationMode .ceq. timeSpectral) then
        ...
        !       unsteady computation.
        !
        testUnsteady: if (equationMode .ceq. unsteady) then
        ...
        if ((myID .ceq. 0) .and. (equations .ceq. EulerEquations) .and. &
            Reynolds > zero) then
        ...
        if (flowType .ceq. internalFlow) then
            ...
            if (((equations .ceq. NSEquations) .or. &
                 (equations .ceq. RANSEquations)) .and. &
        ...
        if (degreePolXRot >= 0 .or. degreePolYRot >= 0 .or. &
            ...
            if ((equationMode .ceq. steady) .and. (myID .ceq. 0)) then
                print "(a)", "#"
                print "(a)", "#                      Warning"
        ...
        if (precisionGrid .ceq. precisionDouble) gridPrecisionWarning = .true.
        if (precisionSol .ceq. precisionDouble) solPrecisionWarning = .true.
#endif

        if (gridPrecisionWarning .and. (myID .ceq. 0)) then
            print "(a)", "#"
            print "(a)", "#                      Warning"
        ...
        if (solPrecisionWarning .and. (myID .ceq. 0)) then
            print "(a)", "#"
            print "(a)", "#                      Warning"
        ...
        !       set to zero.
        !
        if (equations .cne. RANSEquations) wallFunctions = .false.
        ...
        if (turbRelax .ceq. turbRelaxNotDefined) then
            turbRelax = turbRelaxImplicit
            if (turbModel .ceq. v2f) turbRelax = turbRelaxExplicit
        end if

        ! V2f should only be solved with explicit underrelaxation.

        if ((equations .ceq. RANSEquations) .and. (turbModel .ceq. v2f) .and. &
            (turbRelax .ceq. turbRelaxImplicit)) then
    ...
    subroutine initializeIsoSurfaceVariables(values, nValues)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(nValues), intent(in) :: values

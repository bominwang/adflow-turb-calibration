module wallDistance
    ...
    complex(kind=realType), dimension(:, :), allocatable :: coorVisc
    ...
    complex(kind=realType), dimension(:, :, :), allocatable :: rotMatrixSections
#endif
...
contains
    ...
    subroutine updateWallDistancesQuickly(nn, level, sps)
       ...
       use complexify 
        ...
        complex(kind=realType) :: xp(3), xc(3), u, v
            ...
            do k = 2, kl
                do j = 2, jl
                    do i = 2, il
                        ...
                        if (flowDoms(nn, level, sps)%surfNodeIndices(1, i, j, k) .ceq. 0) then
    ...
    subroutine computeNormalSpacing(level, sps)
       ...
       use complexify 
        ...
        complex(kind=realType) :: nnx, nny, nnz, vecx, vecy, vecz, dot

        complex(kind=realType), dimension(:, :, :), pointer :: xFace, xInt
    ...
    subroutine determineDistance(level, sps)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(3, 2) :: dummy

        integer(kind=intType), dimension(:), allocatable :: elementID

        complex(kind=realType), dimension(:), allocatable :: dist2
        complex(kind=realType), dimension(:), allocatable :: dist2per
        complex(kind=realType), dimension(:, :), allocatable :: coor, uvw
        complex(kind=realType), dimension(:, :), allocatable :: coorPer
    ...
    subroutine localViscousSurfaceMesh(multSections, level, sps)
       ...
       use complexify 
        ...
        complex(kind=realType) :: length, dot, xx, yy, zz, r1, r2, aaa, bbb
        complex(kind=realType) :: theta, cosTheta, sinTheta

        complex(kind=realType), dimension(3, 3) :: a
        complex(kind=realType), dimension(nSections) :: thetaNMin, &
                                                     thetaNMax, &
                                                     thetaPMin, &
                                                     thetaPMax, tmp
        complex(kind=realType), dimension(nSections, 3) :: rad1, rad2, axis
        ...
        !
        do nn = 1, nSections
            if (sections(nn)%nSlices .ceq. 1) cycle
        ...
        do nn = 1, nDom
            ...
            if (sections(sec)%nSlices .ceq. 1) cycle
        ...
        do nn = 1, nSections
            ...
            testRot: if ((sections(nn)%nSlices .ceq. 1) .or. &
                         (thetaPMin(nn) .ceq. zero)) then
        ...
        loopDomains: do nn = 1, nDom
            ...
            loopBocos: do mm = 1, nBocos
                testViscous: if ((BCType(mm) .ceq. NSWallAdiabatic) .or. &
       ...
       use complexify 
        ...
        do nn = 1, nDom
            do mm = 1, flowDoms(nn, level, 1)%nBocos
                if ((flowDoms(nn, level, 1)%BCType(mm) .ceq. NSWallAdiabatic) .or. &
                    (flowDoms(nn, level, 1)%BCType(mm) .ceq. NSWallIsothermal)) then
    ...
    subroutine determineWallAssociation(level, sps)
       ...
       use complexify 
        ...
        complex(kind=realType) :: coor(4), uvw(5), uvw2(5)
        complex(kind=realType), dimension(3, 2) :: dummy
        complex(kind=realType), parameter :: tol = 1e-12
        integer(kind=intType), dimension(:), pointer :: frontLeaves, frontLeavesNew, &
                                                        BBint, wallFamList
        type(adtBBoxTargetType), dimension(:), pointer :: BB
        complex(kind=realType), dimension(3) :: xp
                ...
                do j = 2, jl
                    do i = 2, il
                        ...
                        if (cellID > 0) then
                            ...
                            if (uvw(4) > nearWallDist**2 .or. (walls(c)%nCells .ceq. 0)) then

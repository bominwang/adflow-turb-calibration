 ...
 contains
       ...
       use complexify 
        ...
        do i = 1, nHalo
            if (entityHalo(i)%nPeriodicSubfaces > 0) then
                ...
                if (entityHalo(i)%donorProc .ceq. myID) then
    ...
    contains
       ...
       use complexify 
            ...
            do i = 2, nPerHalo
                if (perHalo(i) .ceq. perHalo(i - 1)) then
                ...
                do k = myNodeRange(3, 1), myNodeRange(3, 2), step(3)
                    do j = myNodeRange(2, 1), myNodeRange(2, 2), step(2)
                        do i = myNodeRange(1, 1), myNodeRange(1, 2), step(1)
                            ...
                            if (nodeIndex(nn)%entryList(iH, jH, kH) .ceq. 0) then
                ...
                do k = myCellRange(3, 1), myCellRange(3, 2), step(3)
                    do j = myCellRange(2, 1), myCellRange(2, 2), step(2)
                        do i = myCellRange(1, 1), myCellRange(1, 2), step(1)
                            ...
                            if (debug) then
                                if (cellIndex(nn)%entryList(i, j, k) .cne. 0) &
    ...
    subroutine finalCommStructures(entityHalo, nHalo, commPattern, &
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(:, :), allocatable :: bufInt
        complex(kind=realType), dimension(:, :), allocatable :: recvBufInt
        ...
        do i = 0, (nProc - 1)
            ii = i + 1
            if (nHaloPerProc(ii) > 0 .and. (i .cne. myId)) then
        ...
        do i = 0, (nProc - 1)
            if (sendInfo(i) > 0 .and. (i .cne. myId)) then
        ...
        recvSendInfo: do i = 1, commPattern%nProcSend
            ...
            if (debug) then
                ...
                if (sizeRecv .cne. 4 * commPattern%nsend(ii)) &
                    call terminate("finalCommStructures", &
            ...
            iLoop: do i = iStart, iEnd
                ...
                jLoop: do j = jStart, jEnd
                    ...
                    kLoop: do k = kStart, kEnd
                        ...
                        if (offset > 0 .and. (nHaloDirK .ceq. levOfInd)) cycle
    ...
    subroutine indirectHalosPerLevel(level, iihalo, entityHalo, &
       ...
       use complexify 
        ...
        do i = 1, nProc
            if (nHaloPerProc(i) > 0 .and. (i .cne. (myID + 1))) then
        ...
        recvproc: do i = 1, nProcsRecv
            ...
            if (debug) then
                if ((sizeRecv .ceq. mpi_undefined) .or. &
                    (mod(sizeRecv, nItemSend) .cne. 0)) &
                    call terminate("indirectHalosPerLevel", &
                                   "Unexpected size of message")
            end if
        ...
        secondRecv: do i = 1, nProcsSend
            ...
            if (debug) then
                if ((sizeRecv .ceq. mpi_undefined) .or. &
                    (mod(sizeRecv, nItemReturn) .cne. 0)) &
                    call terminate("indirectHalosPerLevel", &
                                   "Unexpected size of message")
            end if
       ...
       use complexify 
        ...
        do ii = 1, nHalos
            ...
            if (jj .ceq. 0) then
                ...
                if (entityHalo(jj)%donorProc .ceq. -1) then
    ...
    subroutine storeHalosInList(buffer, bufSize, proc, level, &
       ...
       use complexify 
        ...
        if (debug) then
            if (nn .cne. nHaloPerLev(level - 1) + nHaloPerProc(proc + 1)) &
                call terminate("storeHalosInList", &

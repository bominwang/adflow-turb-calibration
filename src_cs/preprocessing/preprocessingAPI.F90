 ...
 contains
    ...
    subroutine preprocessing
       ...
       use complexify 
        ...
        if (((equationMode .ceq. unsteady) .or. &
             (equationMode .ceq. timeSpectral)) .and. nsMin < nsMax) then
        ...
        do nn = 1, nDom
            ...
            do mm = 1, nBocos
                if (BCType(mm) .ceq. OversetOuterBound) then
                    local = .True.
                end if
            end do
        end do
            ...
            domains: do nn = 1, nDom
                ...
                subfaces: do mm = 1, flowDoms(nn, level, 1)%nSubface
                    ...
                    case (iMin)
                        ...
                        if (jBeg .ceq. 1) flowDoms(nn, level, 1)%jcBeg(mm) = 1

                        flowDoms(nn, level, 1)%jcEnd(mm) = jEnd
                        if (jEnd .ceq. jl) flowDoms(nn, level, 1)%jcEnd(mm) = je

                        flowDoms(nn, level, 1)%kcBeg(mm) = kBeg + 1
                        if (kBeg .ceq. 1) flowDoms(nn, level, 1)%kcBeg(mm) = 1

                        flowDoms(nn, level, 1)%kcEnd(mm) = kEnd
                        if (kEnd .ceq. kl) flowDoms(nn, level, 1)%kcEnd(mm) = ke
                    ...
                    case (jMin)
                        ...
                        if (iBeg .ceq. 1) flowDoms(nn, level, 1)%icBeg(mm) = 1

                        flowDoms(nn, level, 1)%icEnd(mm) = iEnd
                        if (iEnd .ceq. il) flowDoms(nn, level, 1)%icEnd(mm) = ie
       ...
       use complexify 
        ...
        domains: do nn = 1, nDom
            ...
            subface: do mm = 1, nsubface
                ...
                if ((BCType(mm) .ceq. NSWallAdiabatic) .or. &
                    (BCType(mm) .ceq. NSWallIsothermal) .or. &
                    (BCType(mm) .ceq. EulerWall) .or. &
                    (BCType(mm) .ceq. Extrap)) then
                    por = boundFlux
                else if ((BCType(mm) .ceq. B2BMismatch) .and. &
                         (nonMatchTreatment .ceq. Conservative)) then
                    por = noFlux
                else
                    cycle
                end if
    ...
    subroutine checkSymmetry(level)
       ...
       use complexify 
        ...
        !
        complex(kind=realType), parameter :: tolDotmin = 0.9999985_realType
        !
        !      Local variables.
        !
        integer(kind=intType) :: nn, mm, sps, i, j

        complex(kind=realType) :: fact, dotMin, dot, mult

        complex(kind=realType), dimension(3) :: faceNorm
        ...
        spectral: do sps = 1, nTimeIntervalsSpectral
            domains: do nn = 1, nDom
                ...
                bocos: do mm = 1, nBocos
                    ...
                    symmetry: if (BCType(mm) .ceq. symm) then
                        ...
                        fineLevelTest: if ((level .ceq. 1) .and. (sps .ceq. 1)) then
...
!!$               enddo
    ...
    subroutine xhalo(level)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(:, :, :), pointer :: x0, x1, x2

        complex(kind=realType) :: length, dot

        complex(kind=realType), dimension(3) :: v1, v2, norm, tmp, tmp2
        complex(kind=realType), parameter :: tolDotmin = 0.99_realType
        ...
        spectralLoop: do sps = 1, nTimeIntervalsSpectral
            domains: do nn = 1, nDom
                ...
                loopBocos: do mm = 1, nBocos
                    ...
                    testSymmetry: if (BCType(mm) .ceq. Symm) then
                        ...
                        testSingular: if (length > eps) then
                            ...
                            if (iBeg .ceq. 1) iBeg = 0
                            if (iEnd .ceq. iiMax) iEnd = iiMax + 1

                            if (jBeg .ceq. 1) jBeg = 0
                            if (jEnd .ceq. jjMax) jEnd = jjMax + 1
        ...
        do i = 1, size(cgnsDoms)
            do j = 1, size(cgnsDoms(i)%bocoInfo)
                if (cgnsDoms(i)%bocoInfo(j)%actualFace) then
                    if (trim(cgnsDoms(i)%bocoInfo(j)%wallBCName) .ceq. "") then
                        if (myid .ceq. 0) then
            ...
            famLoop: do iFam = 1, totalFamilies
                domainLoop: do nn = 1, nDom
                    ...
                    bocoLoop: do mm = 1, nBocos
                        matchiFam: if (flowDoms(nn, 1, 1)%bcData(mm)%famID .ceq. iFam) then
                            select case (iBCGroup)

                            case (iBCGroupWalls)
                                if ((BCType(mm) .ceq. EulerWall) .or. &
                                    (BCType(mm) .ceq. NSWallAdiabatic) .or. &
                                    (BCType(mm) .ceq. NSwallIsoThermal)) then
                                    localFlag(iFam) = 1
                                end if

                            case (iBCGroupInflow)
                                if ((BCType(mm) .ceq. SubsonicInflow) .or. &
                                    (BCType(mm) .ceq. SupersonicInflow)) then
                                    localFlag(iFam) = 1
                                end if

                            case (iBCGroupOutflow)
                                if ((BCType(mm) .ceq. SubsonicOutflow) .or. &
                                    (BCType(mm) .ceq. SupersonicOutflow)) then
                                    localFlag(iFam) = 1
                                end if

                            case (iBCGroupSymm)
                                if ((BCType(mm) .ceq. Symm) .or. (BCType(mm) .ceq. SymmPolar)) then
                                    localFlag(iFam) = 1
                                end if

                            case (iBCGroupFarfield)
                                if (BCType(mm) .ceq. Farfield) then
                                    localFlag(iFam) = 1
                                end if
                            ...
                            case (iBCGroupOther)
                                ...
                                if ((BCType(mm) .ceq. BCNull) .or. &
                                    (BCType(mm) .ceq. MassBleedInflow) .or. &
                                    (BCType(mm) .ceq. MassbleedOutflow) .or. &
                                    (BCType(mm) .ceq. mDot) .or. &
                                    (BCType(mm) .ceq. BCThrust) .or. &
                                    (BCType(mm) .ceq. Extrap) .or. &
                                    (BCType(mm) .ceq. B2BMatch) .or. &
                                    (BCType(mm) .ceq. B2BMisMatch) .or. &
                                    (BCType(mm) .ceq. SlidingInterface) .or. &
                                    (BCType(mm) .ceq. DomainInterfaceAll) .or. &
                                    (BCType(mm) .ceq. DomainInterfaceRhoUVW) .or. &
                                    (BCType(mm) .ceq. DomainInterfaceP) .or. &
                                    (BCType(mm) .ceq. DomainInterfaceRho) .or. &
                                    (BCType(mm) .ceq. DomainInterfaceTotal)) then
                                    localFlag(iFam) = 1
                                end if
                            end select
                        end if matchiFam
    ...
    subroutine createNodeScatterForFamilies(famList, exch, sps, localIndices)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(:, :), allocatable :: localNodes, allNodes
       ...
       use complexify 
        ...
        do nn = 1, cgnsNFamilies
            if (cgnsFamilies(nn)%monitorMassflow .and. &
                (cgnsFamilies(nn)%BCType .cne. MassBleedInflow) .and. &
                (cgnsFamilies(nn)%BCType .cne. MassBleedOutflow) .and. &
                (cgnsFamilies(nn)%BCType .cne. SlidingInterface)) then
                mm = mm + 1
                orToMassFam(nn) = mm
            else
                orToMassFam(nn) = 0
       ...
       use complexify 
        ...
        spectral: do sps = 1, nTimeIntervalsSpectral
            domains: do nn = 1, nDom
                ...
                if ((level .ceq. 1) .and. deforming_Grid .and. &
                    (equationMode .ceq. unsteady)) then
    ...
    subroutine metric(level)
       ...
       use complexify 
        ...
        !      Local parameter.
        !
        complex(kind=realType), parameter :: thresVolume = 1.e-2_realType
        complex(kind=realType), parameter :: haloCellRatio = 1e-10_realType
        !
        !      Local variables.
        !
        integer :: ierr
        ...
        complex(kind=realType) :: fact, mult
        complex(kind=realType) :: xp, yp, zp, vp1, vp2, vp3, vp4, vp5, vp6

        complex(kind=realType), dimension(24) :: angles
        ...
        spectral: do sps = 1, nTimeIntervalsSpectral
            domains: do nn = 1, nDom
                ...
                do k = 1, ke
                    ...
                    checkK = .true.
                    if ((k .ceq. 1) .or. (k .ceq. ke)) checkK = .false.

                    do j = 1, je
                        m = j - 1

                        checkJ = .true.
                        if ((j .ceq. 1) .or. (j .ceq. je)) checkJ = .false.

                        do i = 1, ie
                            l = i - 1

                            checkI = .true.
                            if ((i .ceq. 1) .or. (i .ceq. ie)) checkI = .false.
                            ...
                            if (iblankAllocated) then
                                if (abs(iblank(i, j, k)) .ceq. 1) then
                                    checkBlank = .True.
                                end if
                            end if
                ...
                if (level .ceq. 1) then
                    if (nVolPos .ceq. 0) then       ! Left handed block.
        ...
        if (level .ceq. 1) then
            ...
            if ((myID .ceq. 0) .and. nVolBadGlobal > 0 .and. printWarnings) then
    ...
    contains
       ...
       use complexify 
            ...
            !        Function type.
            !
            complex(kind=realType) :: volpym
            !
            !        Function arguments.
            !
            complex(kind=realType), intent(in) :: xa, ya, za, xb, yb, zb
            complex(kind=realType), intent(in) :: xc, yc, zc, xd, yd, zd
    ...
    subroutine minMaxAngle(angles, p1, p2, p3, p4)
       ...
       use complexify 
        ...
        !      Subroutine arguments.
        !
        complex(kind=realType), dimension(3), intent(in) :: p1, p2, p3, p4
        complex(kind=realType), dimension(4), intent(out) :: angles
        !
        !      Local variables.
        !
        integer(kind=intType) :: i
        complex(kind=realType), dimension(4, 3) :: v
    ...
    subroutine writeBadVolumes(checkVolDoms, mode)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(3) :: xc
        complex(kind=realType) :: quantity
        ...
        procLoop: do proc = 0, (nProc - 1)
            ...
            testIWrite: if (proc .ceq. myID) then
    ...
    subroutine faceRotationMatrices(level, allocMem)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(:, :, :), pointer :: xFace
        complex(kind=realType), dimension(:, :, :, :), pointer :: rotFace

        complex(kind=realType), dimension(3) :: axis, vecR1, vecR2, rotCenter

        ! If this is not the finest level, return. The matrices are
        ! only needed on the finest grid.

        if (level .cne. 1) return

        ! Check if an upwind scheme is used. If not, return.

        if (spaceDiscr .cne. upwind) return

        ! Check if a limiter is used at all. The rotation matrices are
        ! only needed when a nonlinear construction is used.

        if ((limiter .ceq. firstOrder) .or. (limiter .ceq. noLimiter)) return
    ...
    contains
       ...
       use complexify 
            ...
            complex(kind=realType), dimension(:, :, :), intent(in) :: xx
            complex(kind=realType), dimension(2:, 2:, :, :), intent(out) :: rotMat
            !
            !        Local variables.
            !
            integer(kind=intType) :: i, j

            complex(kind=realType) :: r1, r2, rInv, cosTheta, sinTheta

            complex(kind=realType), dimension(3) :: xF
    ...
    subroutine updateCoordinatesAllLevels
       ...
       use complexify 
        ...
        complex(kind=realType) :: origGroundLevel
    ...
    subroutine unitVectorsInAxialPlane(axis, vecR1, vecR2)
       ...
       use complexify 
        ...
        !      Subroutine arguments.
        !
        complex(kind=realType), dimension(3), intent(in) :: axis
        complex(kind=realType), dimension(3), intent(out) :: vecR1, vecR2
        !
        !      Local variables.
        !
        complex(kind=realType) :: dot
...
#include <petsc/finclude/petsc.h>
    ...
    subroutine updateRotationRate(rotCenter, rotRate, blocks, nblocks)
       ...
       use complexify 
        ...
        complex(kind=realType), intent(in) :: rotCenter(3), rotRate(3)

    ...
    subroutine createCoarseBlocks(level)
       ...
       use complexify 
        ...
        domains: do nn = 1, nDom
            ...
            if (flowDoms(nn, levm1, 1)%iCoarsened .ceq. leftStarted) then
            ...
            if (flowDoms(nn, levm1, 1)%jCoarsened .ceq. leftStarted) then
            ...
            if (flowDoms(nn, levm1, 1)%kCoarsened .ceq. leftStarted) then
            ...
            if (flowDoms(nn, levm1, 1)%nx .ceq. 2 * flowDoms(nn, level, 1)%nx) &
                flowDoms(nn, level, 1)%iCoarsened = regular
            if (flowDoms(nn, levm1, 1)%ny .ceq. 2 * flowDoms(nn, level, 1)%ny) &
                flowDoms(nn, level, 1)%jCoarsened = regular
            if (flowDoms(nn, levm1, 1)%nz .ceq. 2 * flowDoms(nn, level, 1)%nz) &
    ...
    subroutine coarseDonorInfo(level)
       ...
       use complexify 
        ...
        unOwnedSubfaces: do nn = 1, nSubface1to1
            ...
            if (subface1to1(nn)%neighProc .cne. myID) then
        ...
        do nn = 1, nSubface1to1
            if (subface1to1(nn)%neighProc .ceq. myID) &

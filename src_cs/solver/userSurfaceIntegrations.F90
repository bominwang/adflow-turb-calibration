 ...
 contains
    ...
    subroutine integrateUserSurfaces(localValues, famList, sps)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(:), allocatable :: recvBuffer1, recvBuffer2
    ...
    subroutine integrateUserSurfaces_d(localValues, localValuesd, famList, sps)
       ...
       use complexify 
        ...
        integer(kind=intType) :: iSurf, i, j, k, jj, ierr, nn, iDim, nPts
        complex(kind=realType), dimension(:), allocatable :: recvBuffer1, recvBuffer2
        complex(kind=realType), dimension(:), allocatable :: recvBuffer1d, recvBuffer2d
    ...
    subroutine addIntegrationSurface(pts, conn, famName, famID, isInflow, nPts, nConn)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(3, nPts), intent(in) :: pts
    ...
    subroutine performInterpolation(pts, oBlocks, useDual, comm)
       ...
       use complexify 
        ...
        complex(kind=realType) :: xc(4), weight(8)
        integer mpiStatus(MPI_STATUS_SIZE)

        complex(kind=realType) :: uvw(5), uvw2(5), donorQual, xcheck(3)
        ...
        complex(kind=realType), dimension(:, :), allocatable :: donorFrac, realSend
                    ...
                    if (donorQual < donorFrac(4, i)) then
                        ...
                        if (useDual) then
                            ...
                            do kkk = 0, 1
                                do jjj = 0, 1
                                    do iii = 0, 1
                                        if (.not. ((iblank(ii + iii, jj + jjj, kk + kkk) .ceq. 1) .or. &
                                                   (iblank(ii + iii, jj + jjj, kk + kkk) .ceq. -1))) then
                                            invalid = .True.
                                        end if
                                    end do
                                end do
    ...
    subroutine commUserIntegrationSurfaceVars(recvBuffer, varStart, varEnd, comm)
       ...
       use complexify 
        ...
        ! Input/Output
        complex(kind=realType), dimension(:) :: recvBuffer
        integer(kind=intType), intent(in) :: varStart, varEnd
        type(userSurfCommType) :: comm

        ! Working
        complex(kind=realType), dimension(:), allocatable :: sendBuffer
    ...
    subroutine commUserIntegrationSurfaceVars_d(recvBuffer, recvBufferd, varStart, varEnd, comm)
       ...
       use complexify 
        ...
        ! Input/Output
        complex(kind=realType), dimension(:) :: recvBuffer, recvBufferd
        integer(kind=intType), intent(in) :: varStart, varEnd
        type(userSurfCommType) :: comm

        ! Working
        complex(kind=realType), dimension(:), allocatable :: sendBuffer, sendBufferd
        integer(Kind=intType) :: d1, i1, j1, k1, jj, k, nvar, i, ierr
        complex(kind=realType), dimension(8) :: weight

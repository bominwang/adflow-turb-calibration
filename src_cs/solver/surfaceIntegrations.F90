 ...
 contains
    ...
    subroutine getCostFunctions(globalVals, funcValues)
       ...
       use complexify 
        ...
        ! Input/Output
        complex(kind=realType), intent(in), dimension(:, :) :: globalVals
        complex(Kind=realType), intent(out), dimension(:) :: funcValues

        ! Working
        complex(kind=realType) :: fact, factMoment, ovrNTS
        complex(kind=realType), dimension(3, nTimeIntervalsSpectral) :: force, forceP, forceV, forceM, &
                                                                     moment, cForce, cForceP, cForceV, cForceM, &
                                                                     cMoment, coFx, coFy, coFz
        complex(kind=realType), dimension(3) :: VcoordRef, VFreestreamRef
        complex(kind=realType) :: mAvgPtot, mAvgTtot, mAvgRho, mAvgPs, mFlow, mAvgMn, mAvga, &
                               mAvgVx, mAvgVy, mAvgVz, gArea, mAvgVi, fxLift, fyLift, fzLift

        complex(kind=realType) :: vdotn, mag, u, v, w, ks_comp
        ...
        do sps = 1, nTimeIntervalsSpectral
            ...
            if (force(1, sps) .cne. zero) then
                coFx(:, sps) = coFx(:, sps) / force(1, sps)
            else
                coFx(:, sps) = zero
            end if

            if (force(2, sps) .cne. zero) then
                coFy(:, sps) = coFy(:, sps) / force(2, sps)
            else
                coFy(:, sps) = zero
            end if

            if (force(3, sps) .cne. zero) then
        ...
        if ((fxLift + fyLift + fzLift) .cne. zero) then
    ...
    subroutine wallIntegrationFace(localValues, mm)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(nLocalValues), intent(inout) :: localValues
        integer(kind=intType) :: mm

        ! Local variables.
        complex(kind=realType), dimension(3) :: Fp, Fv, Mp, Mv
        complex(kind=realType), dimension(3) :: COFSumFx, COFSumFy, COFSumFz
        complex(kind=realType) :: yplusMax, sepSensorKs, sepSensor, sepSensorAvg(3), &
                               sepSensorArea, Cavitation, cpmin_ks_sum, sepSensorKsArea
        integer(kind=intType) :: i, j, ii, blk

        complex(kind=realType) :: pm1, fx, fy, fz, fn
        complex(kind=realType) :: vectTangential(3)
        complex(kind=realType) :: vectDotProductFsNormal
        complex(kind=realType) :: xc, xco, yc, yco, zc, zco, qf(3), r(3), n(3), L
        complex(kind=realType) :: fact, rho, mul, yplus, dwall
        complex(kind=realType) :: V(3), sensor, sensor1, Cp, tmp, plocal, ks_exponent
        complex(kind=realType) :: tauXx, tauYy, tauZz
        complex(kind=realType) :: tauXy, tauXz, tauYz

        complex(kind=realType), dimension(3) :: refPoint
        complex(kind=realType), dimension(3, 2) :: axisPoints
        complex(kind=realType) :: mx, my, mz, cellArea, m0x, m0y, m0z, Mvaxis, Mpaxis
        complex(kind=realType) :: CpError, CpError2
        ...
        !
        visForce: if ((BCType(mm) .ceq. NSWallAdiabatic) .or. &
    ...
    subroutine KSaggregationFunction(g, max_g, g_rho, ks_g)
       ...
       use complexify 
         complex(kind=realType), intent(inout) :: ks_g
        complex(kind=realType), intent(in) :: g, max_g, g_rho
    ...
    subroutine flowIntegrationFace(isInflow, localValues, mm)
       ...
       use complexify 
        ...
        ! Local variables
        complex(kind=realType) :: massFlowRate, mass_Ptot, mass_Ttot, mass_Ps, mass_MN, mass_a, mass_rho, &
                               mass_Vx, mass_Vy, mass_Vz, mass_nx, mass_ny, mass_nz, mass_Vi
        complex(kind=realType) :: area_Ptot, area_Ps
        complex(kind=realType) :: govgm1, gm1ovg, viConst, viLocal, pratio
        complex(kind=realType) :: mReDim
        integer(kind=intType) :: i, j, ii, blk
        complex(kind=realType) :: internalFlowFact, inFlowFact, fact, xc, xco, yc, yco, zc, zco, mx, my, mz
        complex(kind=realType) :: sF, vmag, vnm, vnmFreeStreamRef, vxm, vym, vzm, Fx, Fy, Fz, u, v, w
        complex(kind=realType) :: pm, Ptot, Ttot, rhom, gammam, am
        complex(kind=realType) :: area, cellArea, overCellArea
        complex(kind=realType), dimension(3) :: Fp, Mp, FMom, MMom, refPoint, sFaceCoordRef
        complex(kind=realType), dimension(3) :: COFSumFx, COFSumFy, COFSumFz
        complex(kind=realType) :: MNm, massFlowRateLocal
        ...
        if (flowType .ceq. internalFlow) then
            internalFlowFact = -one
        end if
    ...
    subroutine getSolutionWrap(famLists, funcValues, nCost, nGroups, nFamMax)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(nCost, nGroups), intent(out) :: funcValues
    ...
    subroutine getSolution(famLists, funcValues, globalValues)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(:, :), intent(out) :: funcValues
        complex(kind=realType), optional, intent(out), dimension(:, :, :) :: globalValues

        ! Working
        complex(kind=realType), dimension(nLocalValues, nTimeIntervalsSpectral) :: localVal, globalVal
       ...
       use complexify 
        ...
        bocos: do mm = 1, nBocos
            ...
            famInclude: if (famInList(BCData(mm)%famID, famList)) then
                ...
                isInflowOutflow: if ((BCType(mm) .ceq. SubsonicInflow) .or. &
                                     (BCType(mm) .ceq. SupersonicInflow)) then
                    call flowIntegrationFace(.true., localValues, mm)
                else if ((BCType(mm) .ceq. SubsonicOutflow) .or. &
    ...
    subroutine computeCpMinFamily(famList)
       ...
       use complexify 
        ...
        complex(kind=realType) :: Cp, plocal, tmp, cpmin_local
                    ...
                    famInclude: if (famInList(BCData(mm)%famID, famList)) then
                        ...
                        isWall: if (isWallType(BCType(mm))) then
                            ...
                            do ii = 0, (BCData(mm)%jnEnd - bcData(mm)%jnBeg) * (bcData(mm)%inEnd - bcData(mm)%inBeg) - 1
                                ...
                                if (BCData(mm)%iblank(i, j) .ceq. one) then
    ...
    subroutine computeSepSenMaxFamily(famList)
       ...
       use complexify 
        ...
        integer(kind=intType) :: mm, nn, sps
        integer(kind=intType) :: i, j, ii, blk, ierr
        complex(kind=realType) :: sepsensor_local
        complex(kind=realType) :: vectTangential(3), v(3)
        complex(kind=realType) :: vectDotProductFsNormal, sensor
    ...
    subroutine integrateSurfaces_d(localValues, localValuesd, famList)
       ...
       use complexify 
        ...
        ! Input/output Variables
        complex(kind=realType), dimension(nLocalValues), intent(inout) :: localValues, localValuesd
        integer(kind=intType), dimension(:), intent(in) :: famList
    ...
    subroutine getSolution_d(famLists, funcValues, funcValuesd)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(:, :), intent(out) :: funcValues, funcValuesd

        ! Working
        complex(kind=realType), dimension(nLocalValues, nTimeIntervalsSpectral) :: localVal, globalVal
        complex(kind=realType), dimension(nLocalValues, nTimeIntervalsSpectral) :: localVald, globalVald
        integer(kind=intType) :: nn, sps, ierr, iGroup, nFam
        integer(kind=intType), dimension(:), pointer :: famList
    ...
    subroutine getSolution_b(famLists, funcValues, funcValuesd)
       ...
       use complexify 
        ...
        ! Input/Output Variables
        integer(kind=intType), dimension(:, :), target, intent(in) :: famLists
        complex(kind=realType), dimension(:, :) :: funcValues, funcValuesd

        ! Working
        complex(kind=realType), dimension(nLocalValues, nTimeIntervalsSpectral) :: localVal, globalVal
        complex(kind=realType), dimension(nLocalValues, nTimeIntervalsSpectral) :: localVald, globalVald
        complex(kind=realType), dimension(nLocalValues, nTimeIntervalsSpectral, size(famLists, 1)) :: globalValues
        integer(kind=intType) :: nn, sps, ierr, iGroup, nFam
        integer(kind=intType), dimension(:), pointer :: famList

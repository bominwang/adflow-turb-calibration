 ...
 contains
    ...
    subroutine timeStep_block(onlyRadii)
       ...
       use complexify 
        ...
        complex(kind=realType) :: uux, uuy, uuz, cc2, qsi, qsj, qsk, sx, sy, sz, rmu
        complex(kind=realType) :: ri, rj, rk, rij, rjk, rki
        complex(kind=realType) :: vsi, vsj, vsk, rfl, dpi, dpj, dpk
        complex(kind=realType) :: sFace, tmp
    ...
    subroutine gridVelocitiesFineLevel(useOldCoor, t, sps)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(*), intent(in) :: t  !
        !      Local variables.
        !
    ...
    subroutine gridVelocitiesFineLevel_TS_block(nn, sps)
       ...
       use complexify 
        ...
        complex(kind=realType) :: x_vc, y_vc, z_vc
        complex(kind=realType) :: x_fc, y_fc, z_fc
        complex(kind=realType) :: aInf
        complex(kind=realType) :: velxFreestream, velyFreestream, velzFreestream
    ...
    subroutine gridVelocitiesFineLevel_block(useOldCoor, t, sps, nn)
       ...
       use complexify 
        ...
        complex(kind=realType) :: oneOver4dt, oneOver8dt
        complex(kind=realType) :: velxGrid, velyGrid, velzGrid, ainf
        complex(kind=realType) :: velxGrid0, velyGrid0, velzGrid0

        complex(kind=realType), dimension(3) :: sc, xc, xxc
        complex(kind=realType), dimension(3) :: rotCenter, rotRate

        complex(kind=realType), dimension(3) :: rotationPoint
        complex(kind=realType), dimension(3, 3) :: rotationMatrix, &
                                                derivRotationMatrix

        complex(kind=realType) :: tNew, tOld
        complex(kind=realType), dimension(:, :), pointer :: sFace

        complex(kind=realType), dimension(:, :, :), pointer :: xx, ss
        complex(kind=realType), dimension(:, :, :, :), pointer :: xxOld

        complex(kind=realType) :: intervalMach, alphaTS, alphaIncrement, &
                               betaTS, betaIncrement
        complex(kind=realType), dimension(3) :: velDir
        complex(kind=realType), dimension(3) :: refDirection
...
! We do not consider the finite difference based velocity computation for now
    ...
    subroutine cellFaceVelocities(xc, rotCenter, rotRate, velxGrid, velyGrid, velzGrid, derivRotationMatrix, sc)
       ...
       use complexify 
        ...
        !      Subroutine arguments.
        !
        complex(kind=realType), dimension(3), intent(in) :: xc, rotCenter, rotRate
        complex(kind=realType), intent(in) :: velxGrid, velyGrid, velzGrid
        complex(kind=realType), dimension(3, 3), intent(in) :: derivRotationMatrix
        complex(kind=realType), dimension(3), intent(out) :: sc
        !
        !      Local variables.
        !
        complex(kind=realType), dimension(3) :: rotationPoint, xxc
    ...
    subroutine slipVelocitiesFineLevel_block(useOldCoor, t, sps, nn)
       ...
       use complexify 
        ...
        complex(kind=realType) :: oneOver4dt
        complex(kind=realType) :: velxGrid, velyGrid, velzGrid, ainf
        complex(kind=realType) :: velxGrid0, velyGrid0, velzGrid0

        complex(kind=realType), dimension(3) :: xc, xxc
        complex(kind=realType), dimension(3) :: rotCenter, rotRate
        ...
        complex(kind=realType), dimension(:, :, :), pointer :: uSlip
        complex(kind=realType), dimension(:, :, :), pointer :: xFace
        complex(kind=realType), dimension(:, :, :, :), pointer :: xFaceOld
...
! the stability portion is not ADed.
            ...
            bocoLoop2: do mm = 1, nViscBocos
                ...
                else if (BCFaceID(mm) .ceq. iMax) then
                ...
                else if (BCFaceID(mm) .ceq. jMin) then
                ...
                else if (BCFaceID(mm) .ceq. jMax) then
                ...
                else if (BCFaceID(mm) .ceq. kMin) then
                ...
                else if (BCFaceID(mm) .ceq. kMax) then
    ...
    subroutine normalVelocities_block(sps)
       ...
       use complexify 
        ...
        complex(kind=realType) :: weight, mult
    ...
    subroutine shiftSolution
       ...
       use complexify 
        ...
        complex(kind=realType) :: tOld, tNew
        complex(kind=realType) :: vvX, vvY, vvZ, vXi, vEta, vZeta
        complex(kind=realType) :: t, angleX, angleY, angleZ
        complex(kind=realType) :: phi, cosPhi, sinPhi
        complex(kind=realType) :: xiX, xiY, xiZ, etaX, etaY, etaZ
        complex(kind=realType) :: zetaX, zetaY, zetaZ
    ...
    subroutine computeUtau_block
       ...
       use complexify 
        ...
        complex(kind=realType) :: re, vvx, vvy, vvz, veln, veltmag

        complex(kind=realType), dimension(:, :, :), pointer :: ww, norm, uSlip
        complex(kind=realType), dimension(:, :), pointer :: dd2Wall, rrlv
        complex(kind=realType), dimension(:, :), pointer :: utau
    ...
    subroutine gridVelocitiesFineLevelPart1_block(useOldCoor, t, sps)
       ...
       use complexify 
        ...
        complex(kind=realType) :: tNew, tOld
        complex(kind=realType), dimension(:, :), pointer :: sFace
        complex(kind=realType), dimension(:, :, :), pointer :: sVelo
    ...
    subroutine utauWF(rFilv)
       ...
       use complexify 
        ...
        !      Subroutine argument.
        !
        complex(kind=realType), intent(in) :: rFilv
        !
        !      Local variables.
        !
        ...
        complex(kind=realType) :: rbar, ubar, vbar, wbar, vvx, vvy, vvz
        complex(kind=realType) :: fmx, fmy, fmz, frhoe
        complex(kind=realType) :: veln, velnx, velny, velnz, tx, ty, tz
        complex(kind=realType) :: veltx, velty, veltz, veltmag
        complex(kind=realType) :: txnx, txny, txnz, tynx, tyny, tynz
        complex(kind=realType) :: tznx, tzny, tznz
        complex(kind=realType) :: tautn, tauWall, utau, re

        complex(kind=realType), dimension(:, :, :), pointer :: ww1, ww2
        complex(kind=realType), dimension(:, :, :), pointer :: ss, rres
        complex(kind=realType), dimension(:, :, :), pointer :: norm
        complex(kind=realType), dimension(:, :), pointer :: rrlv2, dd2Wall2
    ...
    subroutine slipVelocitiesCoarseLevels(sps)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(:, :, :), pointer :: uSlip
        complex(kind=realType), dimension(:, :, :), pointer :: uSlipFine
    ...
    subroutine gridVelocitiesCoarseLevels(sps)
       ...
       use complexify 
        ...
        complex(kind=realType) :: jjWeight, kkWeight, weight

        complex(kind=realType), dimension(:), pointer :: jWeight, kWeight

        complex(kind=realType), dimension(:, :), pointer :: sFine, sFace
        complex(kind=realType), dimension(:, :, :, :), pointer :: sf
    ...
    subroutine exchangeCellGridVelocities(level, sps)
       ...
       use complexify 
        ...
        complex(kind=realType) :: alp
        complex(kind=realType), dimension(3) :: vv
    ...
    subroutine correctPeriodicGridVel(level, sps, nPeriodic, &
       ...
       use complexify 
        ...
        complex(kind=realType) :: vx, vy, vz

        complex(kind=realType), dimension(3, 3) :: rotMatrix

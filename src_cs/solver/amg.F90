...
contains
    ...
    subroutine setupAMG(inputMat, nCell, blockSize, levels, nSmooth)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(:), pointer :: indPtr
            ...
            if (lvl > 1) then
                do nn = 1, nDom
                    do iDim = 1, 3
                        ...
                        if (curSize .ceq. 1) then
                            sizes(iDim, nn, lvl) = curSize
                        else if (mod(curSize, 2) .ceq. 0) then
                            ! Evenly divides
                            sizes(iDim, nn, lvl) = curSize / 2
                        else if (mod(curSize, 2) .ceq. 1) then
    ...
    subroutine restrictVec(x, y, interp)
       ...
       use complexify 
        ...
        complex(kind=realType), pointer :: xPtrBlk(:, :), yPtrBlk(:, :)

 ...
 contains
    ...
    subroutine transferToCoarseGrid
       ...
       use complexify 
        ...
        complex(kind=realType) :: vola, tmp, weigth, blankFact

        complex(kind=realType), dimension(:, :, :, :), pointer :: ww
        complex(kind=realType), dimension(:, :, :), pointer :: pp, vvol, rrev
    ...
    subroutine transferToFineGrid(corrections)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(:, :, :, :), pointer :: ww, ww1, res
        complex(kind=realType), dimension(:, :, :), pointer :: pp, pp1
        ...
        fact = one
        if (corrections .and. (mgBoundCorr .ceq. bcDirichlet0)) fact = zero
        ...
        if (.not. corrections .and. (equations .ceq. RANSEquations)) then
        ...
        mgLoop: do nn = 1, nstepsCycling
            ...
            case (0_intType)
                ...
                if (nn > 1) then
                    ...
                    if (cycling(nn - 1) .cne. 1_intType) then
    ...
    subroutine setCycleStrategy
       ...
       use complexify 
        ...
        do i = 1, nMGSteps
            ...
            if (((thisLevel .ceq. groundLevel) .and. (cycleStrategy(i) .cne. 1)) .or. &
            ...
            if ((thisLevel .ceq. groundLevel) .and. (cycleStrategy(i) .ceq. -1)) exit
    ...
    subroutine setCorrectionsCoarseHalos(sps, nn, coarseLevel, &
       ...
       use complexify 
        ...
        complex(kind=realType), intent(in) :: fact
        !
        !      Local variables.
        !
        ...
        complex(kind=realType) :: nnx, nny, nnz, vn

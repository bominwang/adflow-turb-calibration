...
contains
    ...
    subroutine addActuatorRegion(pts, conn, axis1, axis2, famName, famID, &
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(3, nPts), intent(in), target :: pts
        integer(kind=intType), dimension(4, nConn), intent(in), target :: conn
        complex(kind=realType), intent(in), dimension(3) :: axis1, axis2
        character(len=*) :: famName
        complex(kind=realType) :: thrust, torque, heat, relaxStart, relaxEnd
        ...
        complex(kind=realType), dimension(3) :: minX, maxX, v1, v2, v3, xCen, axisVec
        type(adtType) :: ADT
        complex(kind=realType) :: axisVecNorm
    ...
    subroutine writeActuatorRegions(fileName)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(:), allocatable :: pts, allPts
        complex(kind=realType), dimension(:, :), allocatable :: tmp, uniquePts
    ...
    subroutine integrateActuatorRegions(localValues, famList, sps)
       ...
       use complexify 
        ...
        complex(kind=realType) :: PLocal, PLocald

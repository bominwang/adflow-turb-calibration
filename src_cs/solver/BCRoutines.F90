...
contains
    ...
    subroutine applyAllBC_block(secondHalo)
       ...
       use complexify 
        ...
        !$AD II-LOOP
        do nn = 1, nBocos
            if (BCType(nn) .ceq. symmPolar) then
        ...
        !$AD II-LOOP
        do nn = 1, nViscBocos
            if (BCType(nn) .ceq. NSWallAdiabatic) then
                call setBCPointers(nn, .False.)
        ...
        !$AD II-LOOP
        do nn = 1, nViscBocos
            if (BCType(nn) .ceq. NSWallIsoThermal) then
                call setBCPointers(nn, .False.)
        ...
        !$AD II-LOOP
        do nn = 1, nBocos
            if (BCType(nn) .ceq. farField) then
                call setBCPointers(nn, .False.)
        ...
        ! ------------------------------------
        do nn = 1, nBocos
            if ((BCType(nn) .ceq. subSonicOutFlow) .or. &
                (BCType(nn) .ceq. MassBleedOutflow)) then
                call setBCPointers(nn, .False.)
        ...
        ! ------------------------------------
        do nn = 1, nBocos
            if (BCType(nn) .ceq. subSonicInFlow) then
                call setBCPointers(nn, .False.)
        ...
        ! is identical.
        do nn = 1, nBocos
            if ((BCType(nn) .ceq. extrap) .or. &
                (BCType(nn) .ceq. SupersonicOutFlow)) then
                call setBCPointers(nn, .False.)
        ...
        !$AD II-LOOP
        do nn = 1, nBocos
            if (BCType(nn) .ceq. EulerWall) then
                call setBCPointers(nn, .True.)
        ...
        ! ------------------------------------
        do nn = 1, nBocos
            if (BCType(nn) .ceq. SupersonicInflow) then
                call setBCPointers(nn, .False.)
    ...
    subroutine bcSymm1stHalo(nn)
       ...
       use complexify 
        ...
        complex(kind=realType) :: vn, nnx, nny, nnz
    ...
    subroutine bcSymmPolar1stHalo(nn)
       ...
       use complexify 
        ...
        complex(kind=realType) :: nnx, nny, nnz, tmp, vtx, vty, vtz
    ...
    subroutine bcNSWallIsoThermal(nn, secondHalo, correctForK)
       ...
       use complexify 
        ...
        integer(kind=intType) :: i, j, ii
        integer(kind=intType) :: wallTreatment
        complex(kind=realType) :: rhok, t2, t1
    ...
    subroutine bcSubsonicOutflow(nn, secondHalo, correctForK)
       ...
       use complexify 
        ...
        complex(kind=realType) :: ovg, ovgm1, nnx, nny, nnz
        complex(kind=realType) :: pExit, pInt, r, a2, a, ac, ss
        complex(kind=realType) :: ue, ve, we, qne, qnh
    ...
    subroutine bcSubsonicInflow(nn, secondHalo, correctForK)
       ...
       use complexify 
        ...
        integer(kind=intType) :: i, j, l, ii
        complex(kind=realType), parameter :: twoThird = two * third
        complex(kind=realType) :: gm1, ovgm1
        complex(kind=realType) :: ptot, ttot, htot, a2tot, r, alpha, beta
        complex(kind=realType) :: aa2, bb, cc, dd, q, q2, a2, m2, scaleFact
        complex(kind=realType) :: ssx, ssy, ssz, nnx, nny, nnz
        complex(kind=realType) :: rho, velx, vely, velz, ratio, ts, govgm1
    ...
    subroutine bcEulerWall(nn, secondHalo, correctForK)
       ...
       use complexify 
        ...
        complex(kind=realType) :: sixa, siya, siza, sjxa, sjya, sjza
        complex(kind=realType) :: skxa, skya, skza, a1, b1
        complex(kind=realType) :: rxj, ryj, rzj, rxk, ryk, rzk
        complex(kind=realType) :: dpj, dpk, ri, rj, rk, qj, qk, vn
        complex(kind=realType) :: uux, uuy, uuz
        complex(kind=realType), dimension(iStart:iEnd, jStart:jEnd) :: grad
    ...
    subroutine bcFarfield(nn, secondHalo, correctForK)
       ...
       use complexify 
        ...
        complex(kind=realType) :: nnx, nny, nnz
        complex(kind=realType) :: gm1, ovgm1, ac1, ac2
        complex(kind=realType) :: r0, u0, v0, w0, qn0, vn0, c0, s0
        complex(kind=realType) :: re, ue, ve, we, qne, ce
        complex(kind=realType) :: qnf, cf, uf, vf, wf, sf, cc, qq
    ...
    subroutine bcExtrap(nn, secondHalo, correctForK)
       ...
       use complexify 
        ...
        ! Local parameter.
        complex(kind=realType), parameter :: factor = 0.5

        ! Local variables.
        integer(kind=intType) :: i, j, l, ii
        complex(kind=realType) :: fw2, fw3

        ! Set the extrapolation weights, depending on the situation.

        if (BCType(nn) .ceq. SupersonicOutflow) then
    ...
    subroutine pRhoSubsonicInlet(ww, pp, correctForK)
       ...
       use complexify 
        ...
        ! Subroutine arguments.
        complex(kind=realType), dimension(:, :, :) :: ww
        complex(kind=realType), dimension(:, :) :: pp
        logical, intent(in) :: correctForK

        ! Local variables.
        integer(kind=intType) :: i, j, ii, mm, nns, nnt, iii
        complex(kind=realType) :: govgm1, tt, ts, pt, ratio
        complex(kind=realType) :: intTs, intTt, val
    ...
    contains
       ...
       use complexify 
            ...
            complex(kind=realType), intent(out) :: int
    ...
    subroutine computeEtot(ww, pp, correctForK)
       ...
       use complexify 
        ...
        complex(kind=realType) :: ovgm1, factK
    ...
    subroutine extrapolate2ndHalo(correctForK)
       ...
       use complexify 
        ...
        ! Working variables
        complex(kind=realType), parameter :: factor = 0.5_realType
        integer(kind=intType) :: i, j, l, ii

 ...
 contains
    ...
    subroutine residual_block
       ...
       use complexify 
        ...
        complex(kind=realType), parameter :: K1 = 1.05_realType
        ! The line below is only used for the low-speed preconditioner part of this routine
        complex(kind=realType), parameter :: K2 = 0.6_realType ! Random given number

        complex(kind=realType), parameter :: M0 = 0.2_realType ! Mach number preconditioner activation
        complex(kind=realType), parameter :: alpha = 0_realType
        complex(kind=realType), parameter :: delta = 0_realType
        !real(kind=realType), parameter :: hinf = 2_realType ! Test phase
        complex(kind=realType), parameter :: Cpres = 4.18_realType ! Test phase
        complex(kind=realType), parameter :: TEMP = 297.15_realType

        !
        !     Local variables
        !
        complex(kind=realType) :: K3, h, velXrho, velYrho, velZrho, SoS, hinf
        complex(kind=realType) :: resM, A11, A12, A13, A14, A15, A21, A22, A23, A24, A25, A31, A32, A33, A34, A35
        complex(kind=realType) :: A41, A42, A43, A44, A45, A51, A52, A53, A54, A55, B11, B12, B13, B14, B15
        complex(kind=realType) :: B21, B22, B23, B24, B25, B31, B32, B33, B34, B35
        complex(kind=realType) :: B41, B42, B43, B44, B45, B51, B52, B53, B54, B55
        complex(kind=realType) :: rhoHdash, betaMr2
        complex(kind=realType) :: G, q
        complex(kind=realType) :: b1, b2, b3, b4, b5
        complex(kind=realType) :: dwo(nwf)
        logical :: fineGrid
        ...
        if (currentLevel .ceq. 1) discr = spaceDiscr

        fineGrid = .false.
        if (currentLevel .ceq. groundLevel) fineGrid = .true.
    ...
    subroutine sourceTerms_block(nn, res, iRegion, pLocal)
       ...
       use complexify 
        ...
        complex(kind=realType), intent(inout) :: pLocal

        ! Working
        integer(kind=intType) :: i, j, k, ii, iStart, iEnd
        complex(kind=realType) :: Ftmp(3), Vx, Vy, Vz, F_fact(3), Q_fact, Qtmp, reDim, factor, oStart, oEnd
    ...
    subroutine initres_block(varStart, varEnd, nn, sps)
       ...
       use complexify 
        ...
        complex(kind=realType) :: oneOverDt, tmp

        complex(kind=realType), dimension(:, :, :, :), pointer :: ww, wsp, wsp1
        complex(kind=realType), dimension(:, :, :), pointer :: volsp
        ...
        case (steady)
            ...
            steadyLevelTest: if (currentLevel .ceq. groundLevel) then
        ...
        case (unsteady)
            ...
            case (MD, BDF) ! Modified by HDN
                ...
                if ((smoother .ceq. RungeKutta) .and. rkStage > 0) then
                    ww => wn
                else
                    ww => w
                end if
                ...
                unsteadyLevelTest: if (currentLevel .ceq. groundLevel) then
                    ...
                    do l = varStart, varEnd
                        ...
                        if ((l .ceq. ivx) .or. (l .ceq. ivy) .or. (l .ceq. ivz)) then
        ...
        case (timeSpectral)
            ...
            spectralLevelTest: if (currentLevel .ceq. groundLevel) then
                ...
                timeLoopFine: do mm = 1, nTimeIntervalsSpectral
                    ...
                    varLoopFine: do l = varStart, varEnd
                        ...
                        if ((l .ceq. ivx) .or. (l .ceq. ivy) .or. (l .ceq. ivz)) then
                            ...
                            if (l .ceq. ivx) ll = 3 * sps - 2
                            if (l .ceq. ivy) ll = 3 * sps - 1
                            if (l .ceq. ivz) ll = 3 * sps
    ...
    subroutine computedwDADI
       ...
       use complexify 
        ...
        complex(kind=realType) :: gm1, epsval, fac, eps2
        complex(kind=realType) :: uvel, vvel, wvel, cijk, cijkinv, c2inv, uvw
        complex(kind=realType) :: ri1, ri2, ri3, rj1, rj2, rj3, rk1, rk2, rk3, uu
        complex(kind=realType) :: ri, rj, rk, qsi, qsj, qsk, currentCfl
        complex(kind=realType) :: xfact, cInf, cInf2
        complex(kind=realType) :: dw1, dw2, dw3, dw4, dw5
        complex(kind=realType) :: a1, a2, a3, a4, a5, a6, a7, mut, ge
        complex(kind=realType) :: viscTerm1, viscTerm2, viscTerm3
        complex(kind=realType) :: metterm
        complex(kind=realType) :: unsteadyImpl, mult
        complex(kind=realType) :: sqrt2, sqrt2inv, alph, alphinv
        complex(kind=realType) :: volhalf, volhalfrho, volfact
        complex(kind=realType) :: mutpI, mutmI, mutpJ, mutmJ, mutpK, mutmK, &
                               mettermp, mettermm, &
                               viscTermI, viscTermJ, viscTermK

        complex(kind=realType), dimension(:, :, :), pointer :: qq_i, qq_j, qq_k, &
                                                            cc_i, cc_j, cc_k, spectral_i, &
                                                            spectral_j, spectral_k, dual_dt
        complex(kind=realType), dimension(ie, 5) :: bbi, cci, ddi, ffi
        complex(kind=realType), dimension(je, 5) :: bbj, ccj, ddj, ffj
        complex(kind=realType), dimension(ke, 5) :: bbk, cck, ddk, ffk
        complex(kind=realType), dimension(ie) :: mettermi
        complex(kind=realType), dimension(je) :: mettermj
        complex(kind=realType), dimension(ke) :: mettermk
        complex(kind=realType), dimension(5) :: diagPlus, diagMinus
    ...
    subroutine tridiagsolve(bb, cc, dd, ff, nn)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(nn + 1, 5) :: bb, cc, dd, ff

        !     local variables

        integer(kind=intType) :: m, n
        complex(kind=realType) :: d0, d2
    ...
    subroutine residualAveraging
       ...
       use complexify 
        ...
        complex(kind=realType) :: currentCfl, rfl0, plim
        complex(kind=realType) :: dpi, dpj, dpk, r
        complex(kind=realType), dimension(il, max(jl, kl)) :: epz, d, t, rfl

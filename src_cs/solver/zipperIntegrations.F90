 ...
 contains
    ...
    subroutine flowIntegrationZipper(isInflow, conn, fams, vars, localValues, famList, sps, ptValid)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(:, :), intent(in) :: vars
        ...
        complex(kind=realType) :: sF, vmag, vnm, vxm, vym, vzm, Fx, Fy, Fz, u, v, w, vnmFreeStreamRef
        complex(kind=realType), dimension(3) :: Fp, Mp, FMom, MMom, refPoint, ss, x1, x2, x3, &
                                             norm, sFaceCoordRef
        complex(kind=realType) :: pm, Ptot, Ttot, rhom, gammam, MNm, massFlowRateLocal, am
        ...
        complex(kind=realType) :: internalFlowFact, inflowFact, xc, xco, yc, yco, zc, zco, mx, my, mz
    ...
    subroutine wallIntegrationZipper(conn, fams, vars, localValues, famList, sps)
       ...
       use complexify 
        ...
        integer(kind=intType), intent(in), dimension(:, :) :: conn
        integer(kind=intType), intent(in), dimension(:) :: fams
        complex(kind=realType), intent(in), dimension(:, :) :: vars
        complex(kind=realType), intent(inout) :: localValues(nLocalValues)
        integer(kind=intType), dimension(:), intent(in) :: famList
        integer(kind=intType), intent(in) :: sps
        ...
        integer(kind=intType) :: i, j
        complex(kind=realType), dimension(3) :: ss, norm, refPoint
        complex(kind=realType), dimension(3) :: p1, p2, p3, v1, v2, v3, x1, x2, x3
        complex(kind=realType) :: fact, triArea, fx, fy, fz, mx, my, mz, xc, xco, yc, yco, zc, zco
    ...
    subroutine integrateZippers_d(localValues, localValuesd, famList, sps)
       ...
       use complexify 
        ...
        integer(kind=intType), dimension(:), intent(in) :: famList
        integer(kind=intType), intent(in) :: sps
        complex(kind=realType), dimension(:, :), allocatable :: vars, varsd
        type(zipperMesh), pointer :: zipper

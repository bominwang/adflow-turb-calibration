 ...
 contains
    ...
    subroutine executeRkStage
       ...
       use complexify 
        ...
        !      Local parameter.
        !
        complex(kind=realType), parameter :: fiveThird = five * third
        !
        !      Local variables.
        !
        integer(kind=intType) :: sps, nn, i, j, k, l

        complex(kind=realType) :: tmp, unsteadyImpl, mult
        complex(kind=realType) :: dt, currentCfl, gm1, gm53
        complex(kind=realType) :: v2, ovr, dp, factK, ru, rv, rw
        ...
        if (resAveraging .ceq. noResAveraging) then
            smoothResidual = .false.
        else if (resAveraging .ceq. alwaysResAveraging) then
            smoothResidual = .true.
        else if (mod(rkStage, 2_intType) .ceq. 1) then
            smoothResidual = .true.
        else
            smoothResidual = .false.
        end if

...
module sa
    ...
    use constants
       use complexify 
     complex(kind=realType) :: cv13, kar2Inv, cw36, cb3Inv
    complex(kind=realType), dimension(:, :, :), allocatable :: qq
    complex(kind=realType), dimension(:, :, :), pointer :: ddw, ww, ddvt
    complex(kind=realType), dimension(:, :), pointer :: rrlv
...
contains
    ...
    subroutine saSource
       ...
       use complexify 
        ...
        complex(kind=realType) :: ss, sst, nu, dist2Inv, chi, chi2, chi3
        complex(kind=realType) :: rr, gg, gg6, termFw, fwSa, term1, term2
        complex(kind=realType) :: dfv1, dfv2, dft2, drr, dgg, dfw
        ...
        complex(kind=realType) :: strainMag2, strainProd, vortProd
        ...
        ! should be used
        if (turbProd .ceq. katoLaunder) then
            ...
            do k = 2, kl
                do j = 2, jl
                    do i = 2, il
                        ...
                        else if (turbProd .ceq. vorticity) then
    ...
    subroutine saViscous
       ...
       use complexify 
        ...
        complex(kind=realType) :: c1m, c1p, c10, b1, c1, d1, qs
    ...
    subroutine saSolve
       ...
       use complexify 
        ...
        integer(kind=intType) :: i, j, k, nn, ii
        complex(kind=realType), dimension(2:max(kl, il, jl)) :: bb, cc, dd, ff
        complex(kind=realType) :: voli, volmi, volpi, xm, ym, zm, xp, yp, zp
        complex(kind=realType) :: xa, ya, za, ttm, ttp, cnud, cam, cap
        complex(kind=realType) :: nutm, nutp, num, nup, cdm, cdp
        complex(kind=realType) :: c1m, c1p, c10, b1, c1, d1, qs
        complex(kind=realType) :: uu, um, up, factor, f, tu1p(1), nu, rblank
        ...
        factor = one
        if (turbRelax .ceq. turbRelaxImplicit) &
        ...
        do k = 2, kl
            do j = 2, jl
                do i = 2, il
                    ...
                    if (((i .ceq. 2) .and. flagI2(j, k)) .or. &
                        ((i .ceq. il) .and. flagIl(j, k)) .or. &
                        ((j .ceq. 2) .and. flagJ2(i, k)) .or. &
                        ((j .ceq. jl) .and. flagJl(i, k)) .or. &
                        ((k .ceq. 2) .and. flagK2(i, j)) .or. &
                        ((k .ceq. kl) .and. flagKl(i, j))) qq(i, j, k) = one
        ...
        !
        factor = one
        if (turbRelax .ceq. turbRelaxExplicit) factor = alfaTurb

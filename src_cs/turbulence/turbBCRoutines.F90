 ...
 contains
    ...
    subroutine applyAllTurbBCThisBlock(secondHalo)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(:, :, :, :), pointer :: bmt
        complex(kind=realType), dimension(:, :, :), pointer :: bvt, ww1, ww2
        ...
        bocos: do nn = 1, nBocos
                ...
                do j = BCData(nn)%jcBeg, BCData(nn)%jcEnd
                    do i = BCData(nn)%icBeg, BCData(nn)%icEnd
                        do l = nt1, nt2
                            ...
                            do m = nt1, nt2
                                if ((m .cne. l) .and. (bmt(i, j, l, m) .cne. zero)) &
                                    ww1(i, j, l) = ww2(i, j, l)
                            end do
                        end do
                    end do
            ...
            if (eddyModel) then
                ...
                if ((BCType(nn) .ceq. NSWallAdiabatic) .or. &
                    (BCType(nn) .ceq. NSWallIsothermal)) then
    ...
    subroutine bcTurbFarfield(nn)
       ...
       use complexify 
        ...
        complex(kind=realType) :: nnx, nny, nnz, dot
    ...
    subroutine bcTurbWall(nn)
       ...
       use complexify 
        ...
        complex(kind=realType) :: tmpd, tmpe, tmpf, nu

        complex(kind=realType), dimension(:, :, :, :), pointer :: bmt
        complex(kind=realType), dimension(:, :, :), pointer :: bvt, ww2
        complex(kind=realType), dimension(:, :), pointer :: rlv2, dd2Wall

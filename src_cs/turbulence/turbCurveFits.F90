 ...
 contains
    function curveUpRe(Re)
       ...
       use complexify 
        ...
        !      Function type.
        !
        complex(kind=realType) :: curveUpRe
        !
        !      Function arguments.
        !
        complex(kind=realType), intent(in) :: Re
        !
        !      Local variables.
        !
        integer(kind=intType) :: ii, nn, start
        complex(kind=realType) :: x, x2, x3, upRe
    ...
    subroutine curveTupYp(tup, yp, ntu1, ntu2)
       ...
       use complexify 
        ...
        complex(kind=realType), intent(in) :: yp

        complex(kind=realType), dimension(ntu1:ntu2), intent(out) :: tup
        !
        !      Local variables.
        !
        integer(kind=intType) :: ii, nn, start, mm
        complex(kind=realType) :: x, x2, x3, epsWall, fWall
                ...
                do mm = ntu1, ntu2
                    ...
                    case (itu2)  ! epsilon cannot be fitted logarithmically.
                        if (tuLogFit(mm)) then
                            ...
                            if (rvfN .ceq. 1) epsWall = 0.33_realType
                            if (rvfN .ceq. 6) epsWall = 0.27_realType
                    ...
                    case (itu4)
                        if (tuLogFit(mm)) then
                            ...
                            if (rvfN .ceq. 6) tup(mm) = exp(tup(mm)) * x
                        else
                            if (rvfN .ceq. 1) fWall = -0.0035_realType
                            if (rvfN .ceq. 6) fWall = zero

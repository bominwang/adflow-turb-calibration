...
!
    ...
    complex(kind=realtype) :: pad, fsd, vnpd, vnmd
    complex(kind=realtype) :: wwx, wwy, wwz, rvol
    complex(kind=realtype) :: rvold
    intrinsic mod
...
!
    ...
    complex(kind=realtype) :: kavgd, lam1d, lam2d, lam3d
...
!$bwd-of ii-loop 
          do ii=0,(ib+1)*(jb+1)*(kb+1)-1
            ...
            if (.not.(temp0 .le. 0.0_8 .and. ((temp .ceq. 0.0_8) .or. (temp &
&               .cne. int(temp))))) wd(i, j, k, irho) = wd(i, j, k, irho) &
...
branch = myIntStack(myIntPtr)
 myIntPtr = myIntPtr - 1
              if (branch .ceq. 0) call popreal8(sface)
            end do
          end do
        end do
        do k=kl,2,-1
...
!
      ...
      complex(kind=realtype) :: ovaavgd, ova2avgd, etad
...
&   w_yd, w_zd
    ...
    complex(kind=realtype) :: corrd, fracdivd

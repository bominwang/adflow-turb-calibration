 ...
 contains
    ...
    subroutine setupPCMatrix(useAD, useTranspose, frozenTurb, level)
       use complexify 
        ...
        complex(kind=realType) :: delta_x, one_over_dx
        complex(kind=realType) :: delta_x_turb, one_over_dx_turb
            ...
            spectralLoop: do sps = 1, nTimeIntervalsSpectral
                ...
                colorLoop: do iColor = 1, nColor
                    ...
                    kLoop: do k = 0, kb
                        jLoop: do j = 0, jb
                            iLoop: do i = 0, ib
                                if ((flowdoms(nn, 1, 1)%color(i, j, k) .ceq. icolor) .and. globalCell(i, j, k) >= 0) then
...
#include <petsc/finclude/petsc.h>
        ...
        complex(kind=realType) :: sum1, sum2, sum3, sum4, sum5
        complex(kind=realType) :: x1, x2, x3, x4, x5
        complex(kind=realType), pointer :: yPtr(:)
        complex(kind=realType) :: RHS(nw * 7)
...
#include <petsc/finclude/petsc.h>
        ...
        complex(kind=realType) :: blk(nw, nw), rhs(nw)
        complex(kind=realType), pointer :: yPtr(:)

...
contains
    ...
    subroutine computeMatrixFreeProductBwd(dwbar, funcsBar, fbar, useSpatial, useState, xvbar, &
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(:), intent(in) :: dwbar
        complex(kind=realType), dimension(:, :), intent(in) :: funcsBar
        complex(kind=realType), dimension(:, :, :) :: fBar
        logical, intent(in) :: useSpatial, useState
        integer(kind=intType), intent(in) :: famLists(:, :)
        character, dimension(:, :), intent(in) :: bcDataNames
        complex(kind=realType), dimension(:), intent(in) :: bcDataValues
        integer(kind=intType), dimension(:, :) :: bcDataFamLists
        logical, intent(in) :: BCVarsEmpty

        ! Ouput Variables
        complex(kind=realType), dimension(stateSize), intent(out) :: wbar
        complex(kind=realType), dimension(extraSize), intent(out) :: extrabar
        complex(kind=realType), dimension(spatialSize), intent(out) :: xvbar
        complex(kind=realType), dimension(size(bcDataValues)), intent(out) :: bcDataValuesbar

        ! Working variables
        integer(kind=intType) :: nn, sps, i, j, k, l, ii, level, nState, mm
        logical :: resetToRans
        complex(kind=realType), dimension(size(funcsBar, 1), size(funcsBar, 2)) :: funcs
        ...
        resetToRANS = .False.
        if (frozenTurbulence .and. (equations .ceq. RANSEquations)) then
    ...
    subroutine computeMatrixFreeProductBwdFast(dwbar, wbar, stateSize)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(stateSize), intent(in) :: dwbar

        ! Ouput Variables
        complex(kind=realType), dimension(stateSize), intent(out) :: wbar

        complex(kind=realType), dimension(:), allocatable :: extrabar
        complex(kind=realType), dimension(:), allocatable :: xvbar
        complex(kind=realType), dimension(:, :, :), allocatable :: fBar
    ...
    subroutine solveAdjointForRHS(inVec, outVec, nDOF, relativeTolerance)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(ndof), intent(in) :: inVec
        complex(kind=realType), dimension(ndof), intent(out) :: outVec
        complex(kind=realType), intent(in) :: relativeTolerance
        integer(kind=intTYpe) :: adjointConvergedReason
        ! Working variables
        integer(kind=intType) :: ierr, nn, sps
        complex(kind=realType) :: val
        ...
        if ((adjointConvergedReason .ceq. KSP_CONVERGED_RTOL) .or. &
            (adjointConvergedReason .ceq. KSP_CONVERGED_ATOL) .or. &
            (adjointConvergedReason .ceq. KSP_CONVERGED_HAPPY_BREAKDOWN)) then
            adjointFailed = .False.
        else
            adjointFailed = .True.
        end if
...
#include <petsc/finclude/petsc.h>
        ...
        complex(kind=realType), dimension(nState) :: RHS
    ...
    subroutine spectralPrecscribedMotion(input, nin, dXv, nout)
       ...
       use complexify 
        ...
        complex(kind=realType), intent(out) :: dXv(nout)
        complex(kind=realType), intent(in) :: input(nin)

        ! Local Variables
        integer(kind=intType) :: ierr, sps, i, nn, mm, counter0, counter1
        integer(kind=intType) :: nodes_on_block, cum_nodes_on_block
        complex(kind=realType), dimension(3) :: rotationPoint, r
        complex(kind=realType), dimension(3, 3) :: rotationMatrix
        complex(kind=realType) :: t(nSections), dt(nSections)
        complex(kind=realType) :: tOld, tNew, pt(3)
        complex(kind=realType), pointer :: xvec_pointer(:)
        complex(kind=realType) :: time(3)
    ...
    subroutine setupAllResidualMatricesfwd
       ...
       use complexify 
        ...
        complex(kind=realType) :: timeAdjLocal, timeAdj, time(2)
        integer(kind=intType) :: ierr
...
#include <petsc/finclude/petsc.h>
        ...
        ! Input Parameters
        integer(kind=intType) :: nState
        complex(kind=realType), dimension(nState) :: RHS, psi
        ...
        if (checkSolution) then
            ...
            if ((myid .ceq. 0) .and. printTiming) then
        ...
        if ((adjointConvergedReason .ceq. KSP_CONVERGED_RTOL) .or. &
            (adjointConvergedReason .ceq. KSP_CONVERGED_ATOL) .or. &
            (adjointConvergedReason .ceq. KSP_CONVERGED_HAPPY_BREAKDOWN) .or. &
...
#include <petsc/finclude/petsc.h>
        ...
        else if (PreCondType .ceq. 'mg') then
...
#include <petsc/finclude/petsc.h>
        ...
        complex(kind=realType), dimension(3) :: cellCenter
...
#include <petsc/finclude/petsc.h>
        ...
        complex(kind=realType), pointer :: dwb_pointer(:)
        complex(kind=realType), pointer :: wb_pointer(:)
...
#include <petsc/finclude/petsc.h>
        ...
        complex(kind=realType), pointer :: wd_pointer(:)
        complex(kind=realType), pointer :: dwd_pointer(:)
        integer(kind=intType) :: stateSize, costSize, fSize, fSIzeCell, spatialSize
        complex(kind=realType), dimension(:), allocatable :: Xvdot
        complex(kind=realType), dimension(:, :, :), allocatable :: fDot

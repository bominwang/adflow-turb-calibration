...
#include <petsc/finclude/petsc.h>
        ...
        complex(kind=realType) :: weights(8), acousticScaleSave
        ...
        resetToRANS = .False.
        if (frozenTurb .and. (equations .ceq. RANSEquations)) then
            ...
            spectralLoop: do sps = 1, nTimeIntervalsSpectral
                ...
                colorLoop: do iColor = 1, nColor
                    ...
                    stateLoop: do l = lStart, lEnd
                        ...
                        if (useAD) then
                            do k = 0, kb
                                do j = 0, jb
                                    do i = 0, ib
                                        if (flowdoms(nn, 1, 1)%color(i, j, k) .ceq. icolor) then
                    ...
                    kLoop: do k = 0, kb
                        jLoop: do j = 0, jb
                            iLoop: do i = 0, ib
                                colBlank: if ((flowDoms(nn, level, sps)%iblank(i, j, k) .ceq. 1) .or. &
                                              (flowDoms(nn, level, sps)%iBlank(i, j, k) .ceq. -1)) then
                                    ...
                                    if (flowDoms(nn, level, sps)%iblank(i, j, k) .ceq. 1) then
                                    ...
                                    colorCheck: if (flowdoms(nn, 1, 1)%color(i, j, k) .ceq. icolor) then
                                        ...
                                        stencilLoop: do i_stencil = 1, n_stencil
                                            ...
                                            onBlock: if (i + ii >= 2 .and. i + ii <= il .and. &
                                                ...
                                                rowBlank: if (flowDoms(nn, level, sps)% &
                                                              iBlank(i + ii, j + jj, k + kk) .ceq. 1) then

                                                    centerCell: if ((ii .ceq. 0) .and. (jj .ceq. 0) .and. (kk .ceq. 0)) then
...
#include <petsc/finclude/petsc.h>
    ...
    subroutine MyKSPMonitor(myKsp, n, rnorm, dummy, ierr)
       ...
       use complexify 
        ...
        complex(kind=realType), pointer, dimension(:, :) :: myKsp
        ...
        if (mod(n, adjMonStep) .ceq. 0) then
            if (myid .ceq. 0) write (*, "(I4, 1X, A, 1X, ES16.10)") n, 'KSP Residual norm', rnorm
        end if
...
#include <petsc/finclude/petsc.h>
        ...
        if (trim(preConSide) .ceq. 'right') then
        ...
        if (trim(kspObjectType) .ceq. 'richardson') then
            call KSPSetPCSide(kspObject, PC_LEFT, ierr)
            call EChk(ierr, __FILE__, __LINE__)
        end if
...
#include <petsc/finclude/petsc.h>
        ...
        Vec offProcVec
        logical :: overset
        complex(kind=realType), pointer :: tmpPointer(:)
                ...
                do k = 2, kl
                    do j = 2, jl
                        do i = 2, il
                            ...
                            blankedTest: if (iblank(i, j, k) .ceq. 1) then
                                ...
                                do jj = 1, N_stencil
                                    ...
                                    if (iblank(iii, jjj, kkk) .ceq. 1) then
                                    ...
                                    else if (iblank(iii, jjj, kkk) .ceq. -1) then
       ...
       use complexify 
        ...
        do nn = 1, nDom
            do sps = 1, nTimeIntervalsSpectral
                ...
                if ((equations .ceq. EulerEquations) .or. (spaceDiscr .ceq. dissMatrix)) then
    ...
    subroutine resetFDReference(level)
       ...
       use complexify 
        ...
        ! Working Parameters
        integer(kind=intType) :: i, j, k, l, nn, sps
        complex(kind=realType) :: sepSensor, Cavitation, axisMoment

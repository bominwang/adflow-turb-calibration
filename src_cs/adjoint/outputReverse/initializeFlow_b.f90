...
&   kpresent, winf, winfd
    ...
    complex(kind=realtype) :: nuinfd, ktmpd, uinf2d
    complex(kind=realtype) :: vinf, zinf, tmp1(1), tmp2(1)
    complex(kind=realtype) :: vinfd, zinfd
    intrinsic sqrt
...
&         kpresent)
    ...
    if (branch .ceq. 0) winfd(itu1) = winfd(itu1) + ktmpd
    call popcontrol1b(branch)
...
&                           itu1))
 ...
 100 if (rhoref/pref .ceq. 0.0_8) then
      tempd = 0.0_8
    else

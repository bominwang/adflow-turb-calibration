...
!
    ...
    complex(kind=realtype) :: qspd, qsmd, rqspd, rqsmd
    complex(kind=realtype) :: pa, fs, sface, vnp, vnm
    complex(kind=realtype) :: pad, fsd, sfaced, vnpd, vnmd
    complex(kind=realtype) :: wwx, wwy, wwz, rvol
    complex(kind=realtype) :: wwxd, wwyd, wwzd, rvold
    intrinsic mod
    complex(kind=realtype) :: temp
    complex(kind=realtype) :: tempd
    integer :: branch
    complex(kind=realtype) :: temp0
    complex(kind=realtype) :: temp1
    complex(kind=realtype) :: tempd0
    if (blockismoving .and. (equationmode .ceq. steady)) then
...
!
    ...
    complex(kind=realtype) :: plimd, sfaced
    ...
    complex(kind=realtype) :: rradd, dis2d, dis4d
    complex(kind=realtype) :: dp1, dp2, tmp, fs
    complex(kind=realtype) :: tmpd, fsd
    complex(kind=realtype) :: ddw1, ddw2, ddw3, ddw4, ddw5, ddw6
    complex(kind=realtype) :: ddw1d, ddw2d, ddw3d, ddw4d, ddw5d, ddw6d
    complex(kind=realtype) :: dr, dru, drv, drw, dre, drk, sx, sy, sz
    complex(kind=realtype) :: drd, drud, drvd, drwd, dred, drkd, sxd, syd, &
&   szd
    complex(kind=realtype) :: uavg, vavg, wavg, a2avg, aavg, havg
    complex(kind=realtype) :: uavgd, vavgd, wavgd, a2avgd, aavgd, havgd
    complex(kind=realtype) :: alphaavg, unavg, ovaavg, ova2avg
    complex(kind=realtype) :: alphaavgd, unavgd, ovaavgd, ova2avgd
    complex(kind=realtype) :: kavg, lam1, lam2, lam3, area
    complex(kind=realtype) :: kavgd, lam1d, lam2d, lam3d, aread
    complex(kind=realtype) :: abv1, abv2, abv3, abv4, abv5, abv6, abv7
    complex(kind=realtype) :: abv1d, abv2d, abv3d, abv4d, abv5d, abv6d, &
&   abv7d
    complex(kind=realtype), dimension(ie, je, ke, 3) :: dss
    complex(kind=realtype), dimension(ie, je, ke, 3) :: dssd
    logical :: correctfork
...
!     Complexify removed intrinsic line here
    intrinsic sqrt
    complex(kind=realtype) :: x1
    complex(kind=realtype) :: x1d
    complex(kind=realtype) :: x2
    complex(kind=realtype) :: x2d
    complex(kind=realtype) :: x3
    complex(kind=realtype) :: x3d
    complex(kind=realtype) :: y1
    complex(kind=realtype) :: y1d
    complex(kind=realtype) :: y2
    complex(kind=realtype) :: y2d
    complex(kind=realtype) :: y3
    complex(kind=realtype) :: y3d
    complex(kind=realtype) :: abs0
    complex(kind=realtype) :: min1
    complex(kind=realtype) :: min1d
    complex(realtype) :: max1
    complex(realtype) :: max1d
    complex(kind=realtype) :: max2
    complex(kind=realtype) :: max2d
    complex(kind=realtype) :: max3
    complex(kind=realtype) :: max3d
    complex(kind=realtype) :: max4
    complex(kind=realtype) :: max4d
    complex(kind=realtype) :: min2
    complex(kind=realtype) :: min2d
    complex(realtype) :: max5
    complex(realtype) :: max5d
    complex(kind=realtype) :: max6
    complex(kind=realtype) :: max6d
    complex(kind=realtype) :: max7
    complex(kind=realtype) :: max7d
    complex(kind=realtype) :: max8
    complex(kind=realtype) :: max8d
    complex(kind=realtype) :: min3
    complex(kind=realtype) :: min3d
    complex(realtype) :: max9
    complex(realtype) :: max9d
    complex(kind=realtype) :: max10
    complex(kind=realtype) :: max10d
    complex(kind=realtype) :: max11
    complex(kind=realtype) :: max11d
    complex(kind=realtype) :: max12
    complex(kind=realtype) :: max12d
    complex(kind=realtype) :: abs1
    complex(kind=realtype) :: abs1d
    complex(kind=realtype) :: abs2
    complex(kind=realtype) :: abs2d
    complex(kind=realtype) :: abs3
    complex(kind=realtype) :: abs3d
    complex(kind=realtype) :: abs4
    complex(kind=realtype) :: abs4d
    complex(kind=realtype) :: abs5
    complex(kind=realtype) :: abs5d
    complex(kind=realtype) :: abs6
    complex(kind=realtype) :: abs6d
    complex(kind=realtype) :: arg1
    complex(kind=realtype) :: arg1d
    complex(kind=realtype) :: temp
    complex(kind=realtype) :: temp0
    complex(kind=realtype) :: tempd
    complex(kind=realtype) :: tempd0
    ...
    integer :: branch
    complex(kind=realtype) :: temp2
    complex(kind=realtype) :: temp3
    complex(kind=realtype) :: tempd2
    complex(kind=realtype) :: tempd3
    if (rfil .ge. 0.) then
      abs0 = rfil
    else
      abs0 = -rfil
...
&         , k, 1)*sxd
        ...
        call popcontrol1b(branch)
        if (branch .ceq. 0) aread = aread + max9d
        temp3 = sk(i, j, k, 3)
        temp2 = sk(i, j, k, 2)
        temp1 = sk(i, j, k, 1)
        if (temp1**2 + temp2**2 + temp3**2 .ceq. 0.0_8) then
          tempd = 0.0_8
        else
...
&         , k, 1)*sxd
        ...
        call popcontrol1b(branch)
        if (branch .ceq. 0) aread = aread + max5d
        temp3 = sj(i, j, k, 3)
        temp2 = sj(i, j, k, 2)
        temp1 = sj(i, j, k, 1)
        if (temp1**2 + temp2**2 + temp3**2 .ceq. 0.0_8) then
...
&         , k, 1)*sxd
        ...
        call popcontrol1b(branch)
        if (branch .ceq. 0) aread = aread + max1d
        temp3 = si(i, j, k, 3)
        temp2 = si(i, j, k, 2)
        temp1 = si(i, j, k, 1)
        if (temp1**2 + temp2**2 + temp3**2 .ceq. 0.0_8) then
...
!
    ...
    complex(kind=realtype) :: sslimd
    complex(kind=realtype) :: sfil, fis2, fis4
    complex(kind=realtype) :: ppor, rrad, dis2, dis4
    complex(kind=realtype) :: rradd, dis2d, dis4d
    complex(kind=realtype) :: ddw1, ddw2, ddw3, ddw4, ddw5, fs
    complex(kind=realtype) :: ddw1d, ddw2d, ddw3d, ddw4d, ddw5d, fsd
    complex(kind=realtype), dimension(ie, je, ke, 3) :: dss
    complex(kind=realtype), dimension(ie, je, ke, 3) :: dssd
    complex(kind=realtype), dimension(0:ib, 0:jb, 0:kb) :: ss
    complex(kind=realtype), dimension(0:ib, 0:jb, 0:kb) :: ssd
!     Complexify removed intrinsic line here
    intrinsic mod
    intrinsic exp
!     Complexify removed intrinsic line here
...
!$bwd-of ii-loop 
          do ii=0,(ib+1)*(jb+1)*(kb+1)-1
            ...
            if (.not.(temp .le. 0.0_8 .and. ((temp0 .ceq. 0.0_8) .or. (temp0&
&               .cne. int(temp0))))) wd(i, j, k, irho) = wd(i, j, k, irho)&
&               - temp0*temp**(temp0-1)*p(i, j, k)*ssd(i, j, k)/temp1**2
          ...
          if (.not.(rhoinf .le. 0.0_8 .and. ((gammainf .ceq. 0.0_8) .or. &
(&             gammainf .cne. int(gammainf))))) rhoinfd = rhoinfd - &
...
!
    ...
    complex(kind=realtype) :: sxd, syd, szd
    complex(kind=realtype) :: factminmod, sface
    complex(kind=realtype) :: sfaced
    complex(kind=realtype), dimension(nw) :: left, right
    complex(kind=realtype), dimension(nw) :: leftd, rightd
    complex(kind=realtype), dimension(nw) :: du1, du2, du3
    complex(kind=realtype), dimension(nw) :: du1d, du2d, du3d
    complex(kind=realtype), dimension(nwf) :: flux
    complex(kind=realtype), dimension(nwf) :: fluxd
...
!       scheme.
!
      if (limused .ceq. firstorder) then
!
...
!
      ...
      complex(kind=realtype), dimension(:), intent(inout) :: du1d, du2d, &
&     du3d
      complex(kind=realtype), dimension(:) :: left, right
      complex(kind=realtype), dimension(:) :: leftd, rightd
      complex(kind=realtype), dimension(:, :, :, :, :), pointer :: &
&     rotmatrix
!
!        local variables.
!
      ...
      complex(kind=realtype) :: rl1d, rl2d, rr1d, rr2d, tmpd, dvxd, dvyd, &
&     dvzd
...
!     Complexify removed intrinsic line here
      ...
      complex(kind=realtype) :: y3
      complex(kind=realtype) :: y3d
      complex(kind=realtype) :: y4
      complex(kind=realtype) :: y4d
      complex(kind=realtype) :: x3
      complex(kind=realtype) :: x3d
      complex(kind=realtype) :: x4
      complex(kind=realtype) :: x4d
      complex(kind=realtype) :: x5
      complex(kind=realtype) :: x5d
      complex(kind=realtype) :: x6
      complex(kind=realtype) :: x6d
      complex(kind=realtype) :: max2
      complex(kind=realtype) :: max2d
      complex(kind=realtype) :: max3
      complex(kind=realtype) :: max3d
      complex(kind=realtype) :: max4
      complex(kind=realtype) :: max4d
      complex(kind=realtype) :: max5
      complex(kind=realtype) :: max5d
      complex(kind=realtype) :: max6
      complex(kind=realtype) :: max6d
      complex(kind=realtype) :: max7
      complex(kind=realtype) :: max7d
...
!
      ...
      complex(kind=realtype), dimension(*) :: leftd, rightd
      complex(kind=realtype), dimension(*) :: flux
      complex(kind=realtype), dimension(*) :: fluxd
!
!        local variables.
!
      complex(kind=realtype) :: porflux, rface
      complex(kind=realtype) :: rfaced
      complex(kind=realtype) :: etl, etr, z1l, z1r, tmp
      complex(kind=realtype) :: etld, etrd, z1ld, z1rd, tmpd
      complex(kind=realtype) :: dr, dru, drv, drw, dre, drk
      complex(kind=realtype) :: drd, drud, drvd, drwd, dred, drkd
      complex(kind=realtype) :: ravg, uavg, vavg, wavg, havg, kavg
      complex(kind=realtype) :: uavgd, vavgd, wavgd, havgd, kavgd
      complex(kind=realtype) :: alphaavg, a2avg, aavg, unavg
      complex(kind=realtype) :: alphaavgd, a2avgd, aavgd, unavgd
      complex(kind=realtype) :: ovaavg, ova2avg, area, eta
      complex(kind=realtype) :: ovaavgd, ova2avgd, aread, etad
      complex(kind=realtype) :: gm1, gm53
      complex(kind=realtype) :: lam1, lam2, lam3
      complex(kind=realtype) :: lam1d, lam2d, lam3d
      complex(kind=realtype) :: abv1, abv2, abv3, abv4, abv5, abv6, abv7
      complex(kind=realtype) :: abv1d, abv2d, abv3d, abv4d, abv5d, abv6d, &
&     abv7d
      complex(kind=realtype), dimension(2) :: ktmp
      complex(kind=realtype), dimension(2) :: ktmpd
      intrinsic sqrt
!     Complexify removed intrinsic line here
!     Complexify removed intrinsic line here
      complex(kind=realtype) :: x1
      ...
      complex(kind=realtype) :: x2
      complex(kind=realtype) :: x2d
      complex(realtype) :: max2
      complex(realtype) :: max2d
      complex(kind=realtype) :: abs1
      complex(kind=realtype) :: abs1d
      complex(kind=realtype) :: abs2
      complex(kind=realtype) :: abs2d
...
!          flux(irhoe) = -porflux*(tmp*dre)
          ...
          if (gammaface*temp1 .ceq. 0.0_8) then
            tempd0 = 0.0_8
          else
            tempd0 = gammaface*x2d/(left(irho)*2.0*sqrt(gammaface*temp1)&
&             )
          end if
          if (gammaface*temp0 .ceq. 0.0_8) then
            tempd1 = 0.0_8
          else
...
&             temp0)))
          ...
          tmp = one/(z1l+z1r)
          call popcontrol1b(branch)
          if (branch .ceq. 0) aread = aread + max2d
          if (sx**2 + sy**2 + sz**2 .ceq. 0.0_8) then
            tempd0 = 0.0_8
          else
...
&               correctfork)
          ...
          z1ld = z1ld + tempd
          z1rd = z1rd + tempd
          if (.not.(right(irho) .ceq. 0.0_8)) rightd(irho) = rightd(irho) +&
&             z1rd/(2.0*sqrt(right(irho)))
          if (.not.(left(irho) .ceq. 0.0_8)) leftd(irho) = leftd(irho) + &
...
!
    ...
    complex(kind=realtype) :: muld, mued, mutd, heatcoefd
    complex(kind=realtype) :: gm1, factlamheat, factturbheat
    complex(kind=realtype) :: u_x, u_y, u_z, v_x, v_y, v_z, w_x, w_y, w_z
    complex(kind=realtype) :: u_xd, u_yd, u_zd, v_xd, v_yd, v_zd, w_xd, &
&   w_yd, w_zd
    complex(kind=realtype) :: q_x, q_y, q_z, ubar, vbar, wbar
    complex(kind=realtype) :: q_xd, q_yd, q_zd, ubard, vbard, wbard
    complex(kind=realtype) :: corr, ssx, ssy, ssz, ss, fracdiv
    complex(kind=realtype) :: corrd, ssxd, ssyd, sszd, ssd, fracdivd
    complex(kind=realtype) :: tauxx, tauyy, tauzz
    complex(kind=realtype) :: tauxxd, tauyyd, tauzzd
    complex(kind=realtype) :: tauxy, tauxz, tauyz
    complex(kind=realtype) :: tauxyd, tauxzd, tauyzd
    complex(kind=realtype) :: tauxxs, tauyys, tauzzs
    complex(kind=realtype) :: tauxxsd, tauyysd, tauzzsd
    complex(kind=realtype) :: tauxys, tauxzs, tauyzs
    complex(kind=realtype) :: tauxysd, tauxzsd, tauyzsd
    complex(kind=realtype) :: exx, eyy, ezz
    complex(kind=realtype) :: exxd, eyyd, ezzd
    complex(kind=realtype) :: exy, exz, eyz
    complex(kind=realtype) :: exyd, exzd, eyzd
    complex(kind=realtype) :: wxy, wxz, wyz, wyx, wzx, wzy
    complex(kind=realtype) :: wxyd, wxzd, wyzd, wyxd, wzxd, wzyd
    complex(kind=realtype) :: den, ccr1, fact
    complex(kind=realtype) :: dend, factd
    complex(kind=realtype) :: fmx, fmy, fmz, frhoe
    complex(kind=realtype) :: fmxd, fmyd, fmzd, frhoed
...
! and i == il must be tested.
        if ((i .ceq. 1) .and. storewalltensor .and. visciminpointer(j, k) &
&           .gt. 0) then
          call pushcontrol1b(0)
        else
          call pushcontrol1b(1)
...
! and the i == il case.
        if ((i .ceq. il) .and. storewalltensor .and. viscimaxpointer(j, k) &
&           .gt. 0) then
...
&           wxy*tauxys+wxz*tauxzs)*two*exxd
          ...
          dend = -(mue*tempd1/den)
          call popcontrol1b(branch)
          if (branch .ceq. 0) dend = 0.0_8
          if (u_x**2 + u_y**2 + u_z**2 + v_x**2 + v_y**2 + v_z**2 + w_x&
&             **2 + w_y**2 + w_z**2 .ceq. 0.0_8) then
            tempd1 = 0.0_8
          else
...
! and j == jl must be tested.
        if ((j .ceq. 1) .and. storewalltensor .and. viscjminpointer(i, k) &
&           .gt. 0) then
          call pushcontrol1b(0)
        else
          call pushcontrol1b(1)
...
! and the j == jl case.
        if ((j .ceq. jl) .and. storewalltensor .and. viscjmaxpointer(i, k) &
&           .gt. 0) then
...
! and k == kl must be tested.
        if ((k .ceq. 1) .and. storewalltensor .and. visckminpointer(i, j) &
&           .gt. 0) then
          call pushcontrol1b(0)
        else
          call pushcontrol1b(1)
...
! and the k == kl case.
        if ((k .ceq. kl) .and. storewalltensor .and. visckmaxpointer(i, j) &
&           .gt. 0) then
...
&   w_yd, w_zd
    ...
    complex(kind=realtype) :: q_xd, q_yd, q_zd, ubard, vbard, wbard
    complex(kind=realtype) :: corr, ssx, ssy, ssz, ss, fracdiv
    complex(kind=realtype) :: ssxd, ssyd, sszd, ssd, fracdivd
    complex(kind=realtype) :: tauxx, tauyy, tauzz
    complex(kind=realtype) :: tauxxd, tauyyd, tauzzd
    complex(kind=realtype) :: tauxy, tauxz, tauyz
    complex(kind=realtype) :: tauxyd, tauxzd, tauyzd
...
!
    integer(kind=inttype) :: i, j, k, ind
    complex(kind=realtype) :: sslim, rhoi
    complex(kind=realtype) :: sslimd, rhoid
    complex(kind=realtype) :: sfil, fis2, fis4
    complex(kind=realtype) :: ppor, rrad, dis2
    complex(kind=realtype) :: rradd, dis2d
    complex(kind=realtype) :: dss1, dss2, ddw, fs
    complex(kind=realtype) :: dss1d, dss2d, ddwd, fsd
!     Complexify removed intrinsic line here
!     Complexify removed intrinsic line here
    intrinsic exp
!     Complexify removed intrinsic line here
!     Complexify removed intrinsic line here
    ...
    complex(kind=realtype) :: tmp4
    complex(kind=realtype) :: tmpd4
    complex(kind=realtype) :: tmp5
    complex(kind=realtype) :: tmpd5
    complex(kind=realtype) :: tmp6
    complex(kind=realtype) :: tmpd6
    complex(kind=realtype) :: tmp7
    complex(kind=realtype) :: tmpd7
    complex(kind=realtype) :: tmp8
    complex(kind=realtype) :: tmpd8
    complex(kind=realtype) :: tmp9
    complex(kind=realtype) :: tmpd9
    complex(kind=realtype) :: tmp10
    complex(kind=realtype) :: tmpd10
    complex(kind=realtype) :: tmp11
    complex(kind=realtype) :: tmpd11
    complex(kind=realtype) :: tmp12
    complex(kind=realtype) :: tmpd12
    complex(kind=realtype) :: tmp13
    complex(kind=realtype) :: tmpd13
    complex(kind=realtype) :: tmp14
    complex(kind=realtype) :: tmpd14
    complex(kind=realtype) :: tmp15
    complex(kind=realtype) :: tmpd15
    complex(kind=realtype) :: tmp16
    complex(kind=realtype) :: tmpd16
    complex(kind=realtype) :: tmp17
    complex(kind=realtype) :: tmpd17
    complex(kind=realtype) :: tmp18
    complex(kind=realtype) :: tmpd18
    complex(kind=realtype) :: tmp19
    complex(kind=realtype) :: tmpd19
    complex(kind=realtype) :: tmp20
    complex(kind=realtype) :: tmpd20
    complex(kind=realtype) :: tmp21
    complex(kind=realtype) :: tmpd21
    complex(kind=realtype) :: tmp22
    complex(kind=realtype) :: tmpd22
    complex(kind=realtype) :: tmp23
    complex(kind=realtype) :: tmpd23
    complex(kind=realtype) :: tmp24
    complex(kind=realtype) :: tmpd24
    complex(kind=realtype) :: tmp25
    complex(kind=realtype) :: tmpd25
    integer :: branch
    if (rfil .ge. 0.) then
      abs0 = rfil
    else
...
&             tmpd0
      ...
      if (branch .ceq. 0) then
        ...
        tempd = 0.001_realtype*sslimd/temp
        pinfcorrd = tempd
        if (rhoinf .le. 0.0_8 .and. ((gammainf .ceq. 0.0_8) .or. (gammainf &
&           .cne. int(gammainf)))) then
          rhoinfd = 0.0_8
        else
...
!
    ...
    complex(kind=realtype) :: dp1d, dp2d, ddwd, tmpd, fsd
    complex(kind=realtype) :: dr, dru, drv, drw, dre, drk, sx, sy, sz
    complex(kind=realtype) :: drd, drud, drvd, drwd, dred, drkd, sxd, syd, &
&   szd
    complex(kind=realtype) :: uavg, vavg, wavg, a2avg, aavg, havg
...
!     Complexify removed intrinsic line here
    ...
    complex(kind=realtype) :: min3
    complex(kind=realtype) :: min3d
    complex(realtype) :: max3
    complex(realtype) :: max3d
    complex(kind=realtype) :: abs1
    complex(kind=realtype) :: abs2
    complex(kind=realtype) :: abs3
    complex(kind=realtype) :: abs4
    complex(kind=realtype) :: abs5
    complex(kind=realtype) :: abs6
    complex(kind=realtype) :: abs7
    complex(kind=realtype) :: abs8
    complex(kind=realtype) :: abs9
    complex(kind=realtype) :: abs10
    complex(kind=realtype) :: abs11
    complex(kind=realtype) :: abs12
    complex(kind=realtype) :: temp
    complex(kind=realtype) :: temp0
    complex(kind=realtype) :: tempd
    complex(kind=realtype) :: temp1

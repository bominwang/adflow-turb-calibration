...
! ------------------------------------
    do nn=1,nbocos
      if ((bctype(nn) .ceq. subsonicoutflow) .or. (bctype(nn) .ceq. &
...
! is identical.
    do nn=1,nbocos
      if ((bctype(nn) .ceq. extrap) .or. (bctype(nn) .ceq. supersonicoutflow&
&     )) then
        call setbcpointers(nn, .false.)
...
! local variables.
    ...
    complex(kind=realtype) :: nnxd, nnyd, nnzd, tmpd, vtxd, vtyd, vtzd
    intrinsic mod
    intrinsic sqrt
...
! is needed.
    ...
    call popcontrol1b(branch)
    if (branch .ceq. 1) call popreal8array(ww1, size(ww1, 1)*size(ww1, 2)&
&                                   *size(ww1, 3))
    call computeetot_b(ww1, ww1d, pp1, pp1d, correctfork)
    rhokd = 0.0_8
    call popcontrol1b(branch)
    if (branch .ceq. 1) call popreal8array(pp1, size(pp1, 1)*size(pp1, 2)&
&                                  )
!$bwd-of ii-loop 
    do ii=0,isize*jsize-1
      i = mod(ii, isize) + istart
...
! local variables.
    ...
    complex(kind=realtype) :: rhokd, t2d, t1d
    intrinsic mod
...
! local variables.
    ...
    complex(kind=realtype) :: nnxd, nnyd, nnzd
    complex(kind=realtype) :: pexit, pint, r, a2, a, ac, ss
    complex(kind=realtype) :: pexitd, pintd, rd, a2d, ad, acd, ssd
    complex(kind=realtype) :: ue, ve, we, qne, qnh
    complex(kind=realtype) :: ued, ved, wed, qned, qnhd
    intrinsic mod
    intrinsic sqrt
    integer :: branch
    complex(kind=realtype) :: temp
...
&         pp1d(i, j)
      ...
      if (gamma2(i, j)*temp .ceq. 0.0_8) then
        tempd = 0.0_8
      else
...
&         temp))
      ...
      temp = pexit/ss
      if (temp .le. 0.0_8 .and. ((ovg .ceq. 0.0_8) .or. (ovg .cne. int(ovg)))&
&     ) then
        tempd = 0.0_8
      else
      ...
      if (r .le. 0.0_8 .and. ((temp .ceq. 0.0_8) .or. (temp .cne. int(temp)))&
&     ) then
        rd = pint*tempd
      else
...
&                                   *size(ww1, 3))
    ...
    if (branch .cne. 0) then
      if (branch .ceq. 1) then
        call popcontrol1b(branch)
        if (branch .ceq. 1) call popreal8array(ww1, size(ww1, 1)*size(ww1&
&                                       , 2)*size(ww1, 3))
!$bwd-of ii-loop 
        do ii=0,isize*jsize-1
          i = mod(ii, isize) + istart
...
&               twothird*pp1d(i, j)
          ...
          pp1d(i, j) = 0.0_8
          temp1 = ts/ttot
          if (temp1 .le. 0.0_8 .and. ((govgm1 .ceq. 0.0_8) .or. (govgm1 .cne.&
&             int(govgm1)))) then
            tempd1 = 0.0_8
          else
 ...
 100      qd = ssz*ww1d(i, j, ivz)
          ...
          q2 = q*q
          call popcontrol1b(branch)
          if (branch .cne. 0) m2d = 0.0_8
          a2d = a2d - q2*m2d/a2**2
          q2d = m2d/a2 - half*gm1*a2d
          a2totd = a2totd + a2d
          qd = 2*q*q2d
          call popcontrol1b(branch)
          if (branch .cne. 0) qd = 0.0_8
          tempd1 = qd/(two*aa2)
          ddd = tempd1
          bbd = -tempd1
          aa2d = -((dd-bb)*tempd1/aa2)
          if (max1 .ceq. 0.0_8) then
            max1d = 0.0_8
          else
          ...
          if (.not.(a2 .ceq. 0.0_8)) a2d = a2d + two*ovgm1*betad/(2.0*sqrt(&
&             a2))
          tempd = gamma2(i, j)*a2d
...
! local variables.
    ...
    complex(kind=realtype) :: sixad, siyad, sizad, sjxad, sjyad, sjzad
    complex(kind=realtype) :: skxa, skya, skza, a1, b1
    complex(kind=realtype) :: skxad, skyad, skzad
    complex(kind=realtype) :: rxj, ryj, rzj, rxk, ryk, rzk
    complex(kind=realtype) :: rxjd, ryjd, rzjd, rxkd, rykd, rzkd
    complex(kind=realtype) :: dpj, dpk, ri, rj, rk, qj, qk, vn
    complex(kind=realtype) :: dpjd, dpkd, rid, rjd, rkd, qjd, qkd, vnd
    complex(kind=realtype) :: uux, uuy, uuz
    complex(kind=realtype) :: uuxd, uuyd, uuzd
    complex(kind=realtype), dimension(istart:iend, jstart:jend) :: grad
    complex(kind=realtype), dimension(istart:iend, jstart:jend) :: gradd
    intrinsic mod
!     Complexify removed intrinsic line here
!     Complexify removed intrinsic line here
...
! local variables.
    ...
    complex(kind=realtype) :: ac1d, ac2d
    complex(kind=realtype) :: r0, u0, v0, w0, qn0, vn0, c0, s0
    complex(kind=realtype) :: r0d, u0d, v0d, w0d, qn0d, c0d, s0d
    complex(kind=realtype) :: re, ue, ve, we, qne, ce
    complex(kind=realtype) :: red, ued, ved, wed, qned, ced
    complex(kind=realtype) :: qnf, cf, uf, vf, wf, sf, cc, qq
    complex(kind=realtype) :: qnfd, cfd, ufd, vfd, wfd, sfd, ccd
    intrinsic sqrt
    intrinsic mod
    complex(kind=realtype) :: tempd
    integer :: branch
...
! not matter too much in the far field.
      ...
      ufd = ww1d(i, j, ivx)
      ww1d(i, j, ivx) = 0.0_8
      if (sf*cc .le. 0.0_8 .and. ((ovgm1 .ceq. 0.0_8) .or. (ovgm1 .cne. int(&
&         ovgm1)))) then
        tempd0 = 0.0_8
      else
...
&           .cne. int(temp0))))) ww2d(i, j, irho) = ww2d(i, j, irho) + &
...
&       v0*qn0d
      if (gamma2(i, j)*(pp2(i, j)*re) .ceq. 0.0_8) then
        tempd = 0.0_8
      else
...
&       u0*qn0d
    ...
    if (gammainf*(pinfcorr*r0) .ceq. 0.0_8) then
      tempd = 0.0_8
    else
      tempd = gammainf*c0d/(2.0*sqrt(gammainf*(pinfcorr*r0)))
    end if
    if (.not.(winf(irho) .le. 0.0_8 .and. ((gammainf .ceq. 0.0_8) .or. &
(&       gammainf .cne. int(gammainf))))) winfd(irho) = winfd(irho) + &
...
! only implements the constant cpmodel
    ...
    implicit none
    complex(kind=realtype), dimension(:, :) :: pp
    complex(kind=realtype), dimension(:, :) :: ppd
    complex(kind=realtype), dimension(:, :, :) :: ww
    complex(kind=realtype), dimension(:, :, :) :: wwd
    logical :: correctfork
...
! are simply taken constant. their values do not matter.
    ...
    call popcontrol1b(branch)
    if (branch .ceq. 1) call popreal8array(ww0, size(ww0, 1)*size(ww0, 2)&
&                                   *size(ww0, 3))
!$bwd-of ii-loop 
    do ii=0,isize*jsize-1
      i = mod(ii, isize) + istart

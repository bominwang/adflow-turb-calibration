...
!
    ...
    complex(kind=realtype) :: plimd, clim2d
    complex(kind=realtype) :: uux, uuy, uuz, cc2, qsi, qsj, qsk, sx, sy, sz&
&   , rmu
    complex(kind=realtype) :: uuxd, uuyd, uuzd, cc2d, qsid, qsjd, qskd, sxd&
&   , syd, szd, rmud
    complex(kind=realtype) :: ri, rj, rk, rij, rjk, rki
    complex(kind=realtype) :: rid, rjd, rkd, rijd, rjkd, rkid
    complex(kind=realtype) :: vsi, vsj, vsk, rfl, dpi, dpj, dpk
    complex(kind=realtype) :: vsid, vsjd, vskd, rfld, dpid, dpjd, dpkd
...
&                 tempd1
                ...
                call popcontrol1b(branch)
                if (branch .ceq. 0) revd(i, j, k) = revd(i, j, k) + rmud
                call popreal8(rmu)
...
! is negligible.
              ...
              temp1 = rk/ri
              if (temp1 .le. 0.0_8 .and. ((adis .ceq. 0.0_8) .or. (adis .cne.&
&                 int(adis)))) then
                tempd0 = 0.0_8
              else
...
&                 int(adis)))) then
              ...
              call popcontrol1b(branch)
              if (branch .ceq. 0) rkd = 0.0_8
              call popcontrol1b(branch)
              if (branch .ceq. 0) rjd = 0.0_8
              call popcontrol1b(branch)
              if (branch .ceq. 0) rid = 0.0_8
            else
              rkd = radkd(i, j, k)
              radkd(i, j, k) = 0.0_8
              rjd = radjd(i, j, k)
...
!
    ...
    complex(kind=realtype) :: velxgridd, velygridd, velzgridd, ainfd
    complex(kind=realtype) :: velxgrid0, velygrid0, velzgrid0
    complex(kind=realtype) :: velxgrid0d, velygrid0d, velzgrid0d
    complex(kind=realtype), dimension(3) :: sc, xc, xxc
    complex(kind=realtype), dimension(3) :: scd, xcd, xxcd
    complex(kind=realtype), dimension(3) :: rotcenter, rotrate
    complex(kind=realtype), dimension(3) :: rotcenterd, rotrated
    complex(kind=realtype), dimension(3) :: rotationpoint
    complex(kind=realtype), dimension(3, 3) :: rotationmatrix, &
&   derivrotationmatrix
    complex(kind=realtype), dimension(3, 3) :: derivrotationmatrixd
    ...
    complex(kind=realtype) :: intervalmach, alphats, alphaincrement, betats&
&   , betaincrement
...
&     velxgrid0d
    if (gammainf*(pinf/rhoinf) .ceq. 0.0_8) then
      tempd = 0.0_8
    else
...
!      subroutine arguments.
!
    complex(kind=realtype), dimension(3), intent(in) :: xc, rotcenter, &
&   rotrate
    complex(kind=realtype), dimension(3) :: xcd, rotcenterd, rotrated
    complex(kind=realtype), intent(in) :: velxgrid, velygrid, velzgrid
    complex(kind=realtype) :: velxgridd, velygridd, velzgridd
    complex(kind=realtype), dimension(3, 3), intent(in) :: &
&   derivrotationmatrix
    complex(kind=realtype), dimension(3, 3) :: derivrotationmatrixd
    complex(kind=realtype), dimension(3) :: sc
    complex(kind=realtype), dimension(3) :: scd
!
!      local variables.
!
    complex(kind=realtype), dimension(3) :: rotationpoint, xxc
    complex(kind=realtype), dimension(3) :: xxcd
! determine the coordinates relative to the
! center of rotation.
    xxc(1) = xc(1) - rotcenter(1)
    xxc(2) = xc(2) - rotcenter(2)
...
!
    ...
    complex(kind=realtype), dimension(3) :: xcd, xxcd
    complex(kind=realtype), dimension(3) :: rotcenter, rotrate
    complex(kind=realtype), dimension(3) :: rotrated
    complex(kind=realtype), dimension(3) :: rotationpoint
    complex(kind=realtype), dimension(3, 3) :: rotationmatrix, &
&   derivrotationmatrix
    complex(kind=realtype), dimension(3, 3) :: derivrotationmatrixd
...
&                   (mm)%rface(i, j)
                  ...
                  temp1 = sk(i, j, kl, 2)
                  temp0 = sk(i, j, kl, 1)
                  if (temp0**2 + temp1**2 + temp2**2 .ceq. 0.0_8) then
                    tempd = 0.0_8
                  else
...
&               %rface(i, j)
              ...
              temp0 = si(il, i, j, 2)
              temp1 = si(il, i, j, 1)
              if (temp1**2 + temp0**2 + temp**2 .ceq. 0.0_8) then
                tempd0 = 0.0_8
              else
...
! the normal grid velocity to zero if allocated.
      ...
      do mm=nbocos,1,-1
        call popcontrol1b(branch)
        if (branch .cne. 0) bcdatad(mm)%rface = 0.0_8
      end do
    end if

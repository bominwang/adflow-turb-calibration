...
&   , sepsensorkssharpness, sepsensorksoffset
    ...
    implicit none
! input/output
    complex(kind=realtype), dimension(:, :), intent(in) :: globalvals
    complex(kind=realtype), dimension(:, :) :: globalvalsd
    complex(kind=realtype), dimension(:) :: funcvalues
    complex(kind=realtype), dimension(:) :: funcvaluesd
! working
    complex(kind=realtype) :: fact, factmoment, ovrnts
    complex(kind=realtype) :: factd
    complex(kind=realtype), dimension(3, ntimeintervalsspectral) :: force, &
&   forcep, forcev, forcem, moment, cforce, cforcep, cforcev, cforcem, &
&   cmoment, cofx, cofy, cofz
    complex(kind=realtype), dimension(3, ntimeintervalsspectral) :: forced&
...
&   cforcemd, cmomentd, cofxd, cofyd, cofzd
    ...
    complex(kind=realtype) :: mavgptot, mavgttot, mavgrho, mavgps, mflow, &
&   mavgmn, mavga, mavgvx, mavgvy, mavgvz, garea, mavgvi, fxlift, fylift&
&   , fzlift
    complex(kind=realtype) :: mavgptotd, mavgttotd, mavgrhod, mavgpsd, &
...
&   , fxliftd, fyliftd, fzliftd
    ...
    complex(kind=realtype) :: ks_compd
...
&         ovrnts*funcvaluesd(costfuncaxismoment)
        call popcontrol1b(branch)
        if (branch .ceq. 0) globalvalsd(icpmin, sps) = globalvalsd(icpmin&
...
! input/output variables
    complex(kind=realtype), dimension(nlocalvalues), intent(inout) :: &
&   localvalues
    complex(kind=realtype), dimension(nlocalvalues), intent(inout) :: &
&   localvaluesd
...
! local variables.
    ...
    complex(kind=realtype), dimension(3) :: fpd, fvd, mpd, mvd
    complex(kind=realtype), dimension(3) :: cofsumfx, cofsumfy, cofsumfz
    complex(kind=realtype), dimension(3) :: cofsumfxd, cofsumfyd, cofsumfzd
    complex(kind=realtype) :: yplusmax, sepsensorks, sepsensor, &
&   sepsensoravg(3), sepsensorarea, cavitation, cpmin_ks_sum, &
&   sepsensorksarea
    complex(kind=realtype) :: sepsensorksd, sepsensord, sepsensoravgd(3), &
&   sepsensoraread, cavitationd, cpmin_ks_sumd, sepsensorksaread
    integer(kind=inttype) :: i, j, ii, blk
    complex(kind=realtype) :: pm1, fx, fy, fz, fn
    complex(kind=realtype) :: pm1d, fxd, fyd, fzd
    complex(kind=realtype) :: vecttangential(3)
    complex(kind=realtype) :: vecttangentiald(3)
    complex(kind=realtype) :: vectdotproductfsnormal
    complex(kind=realtype) :: vectdotproductfsnormald
    complex(kind=realtype) :: xc, xco, yc, yco, zc, zco, qf(3), r(3), n(3)&
&   , l
    complex(kind=realtype) :: xcd, xcod, ycd, ycod, zcd, zcod, rd(3)
    complex(kind=realtype) :: fact, rho, mul, yplus, dwall
    complex(kind=realtype) :: v(3), sensor, sensor1, cp, tmp, plocal, &
&   ks_exponent
    complex(kind=realtype) :: vd(3), sensord, sensor1d, cpd, tmpd, plocald&
&   , ks_exponentd
    ...
    complex(kind=realtype), dimension(3) :: refpointd
    complex(kind=realtype), dimension(3, 2) :: axispoints
    complex(kind=realtype) :: mx, my, mz, cellarea, m0x, m0y, m0z, mvaxis, &
&   mpaxis
    complex(kind=realtype) :: mxd, myd, mzd, cellaread, m0xd, m0yd, m0zd, &
&   mvaxisd, mpaxisd
    complex(kind=realtype) :: cperror, cperror2
    complex(kind=realtype) :: cperrord, cperror2d
    intrinsic mod
!     Complexify removed intrinsic line here
    ...
    complex(kind=realtype) :: temp
    complex(kind=realtype) :: tempd
    complex(kind=realtype), dimension(3) :: tmp0
    complex(kind=realtype), dimension(3) :: tmpd0
    complex(kind=realtype) :: temp0
    complex(kind=realtype), dimension(3) :: tmp1
    complex(kind=realtype), dimension(3) :: tmpd1
    complex(kind=realtype) :: tempd0
    complex(kind=realtype) :: temp1
    complex(kind=realtype) :: tempd1
    integer :: branch
...
!
    if ((bctype(mm) .ceq. nswalladiabatic) .or. (bctype(mm) .ceq. &
...
! also do the ks-based cpmin computation
        ...
        temp0 = one + exp(temp1)
        if (sensor1 .le. 0.0_8 .and. ((cavexponent .ceq. 0.0_8) .or. &
(&           cavexponent .cne. int(cavexponent)))) then
...
&         + bcdata(mm)%norm(i, j, 1)*vectdotproductfsnormald
      ...
      if (ssi(i, j, 1)**2 + ssi(i, j, 2)**2 + ssi(i, j, 3)**2 .ceq. 0.0_8&
&     ) then
        tempd = 0.0_8
      else
...
!   with respect to varying inputs: g
  subroutine ksaggregationfunction_b(g, gd, max_g, g_rho, ks_g, ks_gd)
    ...
    complex(kind=realtype), intent(inout) :: ks_gd
    complex(kind=realtype), intent(in) :: g, max_g, g_rho
    complex(kind=realtype) :: gd
    intrinsic exp
...
! local variables
    complex(kind=realtype) :: massflowrate, mass_ptot, mass_ttot, mass_ps, &
&   mass_mn, mass_a, mass_rho, mass_vx, mass_vy, mass_vz, mass_nx, &
&   mass_ny, mass_nz, mass_vi
    complex(kind=realtype) :: massflowrated, mass_ptotd, mass_ttotd, &
...
&   , mass_nxd, mass_nyd, mass_nzd, mass_vid
    ...
    complex(kind=realtype) :: area_ptotd, area_psd
    complex(kind=realtype) :: govgm1, gm1ovg, viconst, vilocal, pratio
    complex(kind=realtype) :: vilocald, pratiod
    complex(kind=realtype) :: mredim
    complex(kind=realtype) :: mredimd
    integer(kind=inttype) :: i, j, ii, blk
    complex(kind=realtype) :: internalflowfact, inflowfact, fact, xc, xco, &
&   yc, yco, zc, zco, mx, my, mz
    complex(kind=realtype) :: xcd, xcod, ycd, ycod, zcd, zcod, mxd, myd, &
&   mzd
    complex(kind=realtype) :: sf, vmag, vnm, vnmfreestreamref, vxm, vym, &
&   vzm, fx, fy, fz, u, v, w
    complex(kind=realtype) :: vmagd, vnmd, vxmd, vymd, vzmd, fxd, fyd, fzd
    complex(kind=realtype) :: pm, ptot, ttot, rhom, gammam, am
    complex(kind=realtype) :: pmd, ptotd, ttotd, rhomd, amd
    complex(kind=realtype) :: area, cellarea, overcellarea
    complex(kind=realtype) :: aread, cellaread, overcellaread
    complex(kind=realtype), dimension(3) :: fp, mp, fmom, mmom, refpoint, &
&   sfacecoordref
    complex(kind=realtype), dimension(3) :: fpd, mpd, fmomd, mmomd, &
&   refpointd, sfacecoordrefd
    complex(kind=realtype), dimension(3) :: cofsumfx, cofsumfy, cofsumfz
    complex(kind=realtype), dimension(3) :: cofsumfxd, cofsumfyd, cofsumfzd
    complex(kind=realtype) :: mnm, massflowratelocal
    complex(kind=realtype) :: mnmd, massflowratelocald
    intrinsic sqrt
    intrinsic mod
!     Complexify removed intrinsic line here
!     Complexify removed intrinsic line here
...
! the sign of momentum forces are flipped for internal flows
    ...
    if (flowtype .ceq. internalflow) internalflowfact = -one
    inflowfact = one
...
&       vilocal*mass_vid
      ...
      if (viconst*(temp*(ttot*tref)) .ceq. 0.0_8) then
        tempd0 = 0.0_8
      else
        tempd0 = viconst*vilocald/(2.0*sqrt(viconst*(temp*(ttot*tref))))
      end if
      if (pratio .le. 0.0_8 .and. ((gm1ovg .ceq. 0.0_8) .or. (gm1ovg .cne. &
&         int(gm1ovg)))) then
        pratiod = 0.0_8
      else
      ...
      call popcontrol1b(branch)
      if (branch .ceq. 0) ptotd = ptotd - one*pratiod/ptot**2
...
&         i, j, 3)**2))
      ...
      vmagd = mnmd/am
      if (gammam*(pm/rhom) .ceq. 0.0_8) then
        tempd = 0.0_8
      else
      ...
      if (vxm**2 + vym**2 + vzm**2 .ceq. 0.0_8) then
        tempd = 0.0_8
      else

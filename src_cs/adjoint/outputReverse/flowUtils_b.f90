...
!
    ...
    complex(kind=realtype) :: rhod, pd, ud, vd, wd
    complex(kind=realtype) :: ttot
    complex(kind=realtype) :: ttotd
!
!      local variables.
!
    integer(kind=inttype) :: i
    complex(kind=realtype) :: govgm1, t, kin
    complex(kind=realtype) :: td, kind0
...
!
    ...
    complex(kind=realtype), intent(in) :: rho, p, u, v, w
    complex(kind=realtype) :: rhod, pd, ud, vd, wd
    complex(kind=realtype) :: ptot
    complex(kind=realtype) :: ptotd
!
!      local parameters.
!
...
!===============================================================
      temp = rho*kin/(govgm1*p)
      if (one + temp .le. 0.0_8 .and. ((govgm1 .ceq. 0.0_8) .or. (govgm1 &
&         .cne. int(govgm1)))) then
        tempd = 0.0_8
      else
...
!
    ...
    complex(kind=realtype) :: rhod, pd, kd
    complex(kind=realtype), intent(in) :: u, v, w
    complex(kind=realtype) :: ud, vd, wd
    complex(kind=realtype) :: etotal
    complex(kind=realtype) :: etotald
    logical, intent(in) :: correctfork
!
!      local variables.
!
...
!
    complex(kind=realtype), intent(in) :: rho, p, k
    complex(kind=realtype) :: rhod, pd, kd
    complex(kind=realtype) :: einternal
    complex(kind=realtype) :: einternald
    logical, intent(in) :: correctfork
!
!      local parameter.
!
...
! exponent == -1 into account.
                      if (cptempfit(nn)%exponents(ii) .ceq. -1_inttype) &
&                     then
...
!
    ...
    complex(kind=realtype) :: musuthd, tsuthd, ssuthd, td, ppd
    logical :: correctfork
    integer(kind=inttype) :: ibeg, iend, isize, jbeg, jend, jsize, kbeg&
&   , kend, ksize
    intrinsic mod
...
!
    ...
    complex(kind=realtype), dimension(3) :: winddirection
    complex(kind=realtype), dimension(3) :: winddirectiond
    integer(kind=inttype) :: liftindex
!
!     local variables.
!
    complex(kind=realtype) :: rnorm, x1, y1, z1, xbn, ybn, zbn, xw, yw, zw
    complex(kind=realtype) :: x1d, y1d, z1d, xbnd, ybnd, zbnd, xwd, ywd, &
&   zwd
    complex(kind=realtype) :: tmp
    complex(kind=realtype) :: tmpd
    intrinsic sqrt
...
!
    ...
    complex(kind=realtype) :: angled, xd, yd, zd
    complex(kind=realtype) :: xp, yp, zp
    complex(kind=realtype) :: xpd, ypd, zpd
    intrinsic cos
    intrinsic sin
...
!        local variables.
    ...
    complex(kind=realtype) :: oneovervd, ubard, vbard, wbard, a2d

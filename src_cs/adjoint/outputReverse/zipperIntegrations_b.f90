...
! input/output variables
    ...
    complex(kind=realtype), dimension(:, :) :: varsd
...
! working variables
    ...
    complex(kind=realtype) :: sf, vmag, vnm, vxm, vym, vzm, fx, fy, fz, u, &
&   v, w, vnmfreestreamref
    complex(kind=realtype) :: sfd, vmagd, vnmd, vxmd, vymd, vzmd, fxd, fyd&
&   , fzd
...
&   refpointd, ssd, x1d, x2d, x3d, normd, sfacecoordrefd
    complex(kind=realtype) :: pm, ptot, ttot, rhom, gammam, mnm, &
&   massflowratelocal, am
    complex(kind=realtype) :: pmd, ptotd, ttotd, rhomd, gammamd, mnmd, &
...
&   , mass_nxd, mass_nyd, mass_nzd, mass_vid
    ...
    complex(kind=realtype) :: internalflowfact, inflowfact, xc, xco, yc, &
&   yco, zc, zco, mx, my, mz
...
!     Complexify removed intrinsic line here
    complex(kind=realtype), dimension(3) :: arg1
    complex(kind=realtype), dimension(3) :: arg1d
    complex(kind=realtype), dimension(3) :: arg2
    complex(kind=realtype), dimension(3) :: arg2d
...
&           mass_nxd + vilocal*mass_vid
          ...
          if (viconst*(temp0*(ttot*tref)) .ceq. 0.0_8) then
            tempd = 0.0_8
          else
            tempd = viconst*vilocald/(2.0*sqrt(viconst*(temp0*(ttot*tref&
&             ))))
          end if
          if (pratio .le. 0.0_8 .and. ((gm1ovg .ceq. 0.0_8) .or. (gm1ovg &
&             .cne. int(gm1ovg)))) then
            pratiod = 0.0_8
          else
...
&           massflowratelocal*mass_ptotd + pm*pmd
          ...
          if (ss(1)**2 + ss(2)**2 + ss(3)**2 .ceq. 0.0_8) then
            tempd0 = 0.0_8
          else
...
! input/output
    ...
    complex(kind=realtype), intent(inout) :: localvaluesd(nlocalvalues)
    integer(kind=inttype), dimension(:), intent(in) :: famlist
    integer(kind=inttype), intent(in) :: sps
! working
    ...
    complex(kind=realtype), dimension(3) :: ssd, normd, refpointd
    complex(kind=realtype), dimension(3) :: p1, p2, p3, v1, v2, v3, x1, x2&
&   , x3
    complex(kind=realtype), dimension(3) :: p1d, p2d, p3d, v1d, v2d, v3d, &
&   x1d, x2d, x3d
    complex(kind=realtype) :: fact, triarea, fx, fy, fz, mx, my, mz, xc, &
&   xco, yc, yco, zc, zco
    complex(kind=realtype) :: triaread, fxd, fyd, fzd, mxd, myd, mzd, xcd, &

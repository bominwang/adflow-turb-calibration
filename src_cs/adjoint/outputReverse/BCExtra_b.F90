...
contains
  ...
  subroutine applyAllBC_block_b(secondHalo)
    ...
    ! ------------------------------------
    do mm=1,nBocos
       if (bcType(mm) .ceq. eulerWall) then
    ...
    ! ------------------------------------
    do mm=1,nBocos
       if (bcType(mm) .ceq. subsonicInflow) then
          call setBCPointers_d(mm, .False.)
    ...
    ! ------------------------------------
    do mm=1,nBocos
       if (bcType(mm) .ceq. subsonicOutflow) then
          call setBCPointers_d(mm, .False.)
    ...
    ! ------------------------------------
    DO mm=1,nviscbocos
       IF (bctype(mm) .ceq. nswalladiabatic) THEN
          CALL setBCPointers_d(mm, .false.)
    ...
    if (secondHalo) then
       do mm=1, nBocos
          if (BCType(mm) .ceq. symmPolar) then

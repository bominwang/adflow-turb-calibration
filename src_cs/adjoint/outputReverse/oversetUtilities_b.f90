...
! --------------------------------------------------
  subroutine fractoweights_b(frac, fracd, weights, weightsd)
    use constants
       use complexify 
    implicit none
    complex(kind=realtype), dimension(3), intent(in) :: frac
    complex(kind=realtype), dimension(3) :: fracd
    complex(kind=realtype), dimension(8) :: weights
    complex(kind=realtype), dimension(8) :: weightsd
...
! --------------------------------------------------
  subroutine fractoweights(frac, weights)
    ...
    implicit none
    complex(kind=realtype), dimension(3), intent(in) :: frac
    complex(kind=realtype), dimension(8), intent(out) :: weights
...
! input
    ...
    complex(kind=realtype), dimension(3) :: xcend
    complex(kind=realtype), dimension(3, 3, 3, 3), intent(in) :: blk
    complex(kind=realtype), dimension(3, 3, 3, 3) :: blkd
    complex(kind=realtype), dimension(3), intent(in) :: frac0
! output
    complex(kind=realtype), dimension(3) :: frac
    complex(kind=realtype), dimension(3) :: fracd
! working
    complex(kind=realtype), dimension(3, 8) :: xn
    complex(kind=realtype), dimension(3, 8) :: xnd
    complex(kind=realtype) :: u, v, w, uv, uw, vw, wvu, du, dv, dw
    complex(kind=realtype) :: ud, vd, wd, uvd, uwd, vwd, wvud, dud, dvd, &
&   dwd
    complex(kind=realtype) :: a11, a12, a13, a21, a22, a23, a31, a32, a33, &
&   val
    complex(kind=realtype) :: a11d, a12d, a13d, a21d, a22d, a23d, a31d, &
&   a32d, a33d, vald
    complex(kind=realtype) :: f(3), x(3)
    complex(kind=realtype) :: fd(3), xd(3)
...
!     Complexify removed intrinsic line here
    ...
    complex(kind=realtype), dimension(3) :: tmp
    complex(kind=realtype), dimension(3) :: tmpd

 ...
 contains
    subroutine master(useSpatial, famLists, funcValues, forces, &
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(nSections) :: time
        complex(kind=realType) :: dummyReal
...
#include <petsc/finclude/petsc.h>
        ...
        ! Input Arguments:
        complex(kind=realType), intent(in), dimension(:) :: wDot, xDot
        integer(kind=intType), optional, dimension(:, :), intent(in) :: famLists
        complex(kind=realType), optional, dimension(:, :), intent(out) :: funcValues, funcValuesd
        character, optional, dimension(:, :), intent(in) :: bcDataNames
        complex(kind=realType), optional, dimension(:), intent(in) :: bcDataValues, bcDataValuesd
        integer(kind=intType), optional, dimension(:, :) :: bcDataFamLists

        ! Output variables:
        complex(kind=realType), intent(out), dimension(:) :: dwDot
        complex(kind=realType), intent(out), dimension(:, :, :) :: forcesDot

        ! Working Variables
        complex(kind=realType), dimension(:, :, :), allocatable :: forces
        integer(kind=intType) :: ierr, nn, sps, mm, i, j, k, l, fSize, ii, jj, iRegion
        complex(kind=realType), dimension(nSections) :: time
        complex(kind=realType) :: dummyReal, dummyReald
        ...
        do sps = 1, nTimeIntervalsSpectral
            ! Update overset connectivity if necessary
            if (oversetPresent) then
                if (oversetUpdateMode .ceq. updateFast) then
                    call updateOversetConnectivity_d(1_intType, sps)
                else if (oversetUpdateMode .ceq. updateFull) then
                    if (myID .ceq. 0) then
...
#include <petsc/finclude/petsc.h>
        ...
        ! Input variables:
        complex(kind=realType), intent(in), dimension(:) :: dwBar
        complex(kind=realType), intent(in), dimension(:, :, :) :: forcesBar
        integer(kind=intType), intent(in) :: nState
        integer(kind=intType), optional, dimension(:, :), intent(in) :: famLists
        complex(kind=realType), optional, dimension(:, :) :: funcValues, funcValuesd
        character, optional, dimension(:, :), intent(in) :: bcDataNames
        complex(kind=realType), optional, dimension(:), intent(in) :: bcDataValues
        integer(kind=intType), optional, dimension(:, :) :: bcDataFamLists

        ! Output Arguments:
        complex(kind=realType), optional, intent(out), dimension(:) :: wBar, xBar, extraBar
        complex(kind=realType), optional, dimension(:), intent(out) :: bcDataValuesd

        ! Working Variables
        integer(kind=intType) :: ierr, nn, sps, mm, i, j, k, l, fSize, ii, jj, level, iRegion
        complex(kind=realType), dimension(:), allocatable :: extraLocalBar, bcDataValuesdLocal
        complex(kind=realType) :: dummyReal, dummyReald
        logical :: resetToRans
        complex(kind=realType), dimension(nSections) :: time
    ...
    subroutine master_state_b(wbar, dwBar, nState)
       ...
       use complexify 
        ...
        ! Input Arguments:
        complex(kind=realType), intent(out), dimension(:) :: wBar

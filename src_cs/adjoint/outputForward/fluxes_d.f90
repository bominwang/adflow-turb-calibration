        ...
        do k=0,kb
          do j=0,jb
            do i=0,ib
              ...
              temp1 = temp0**temp
              temp2 = p(i, j, k)/temp1
              if (temp0 .le. 0.0_8 .and. ((temp .ceq. 0.0_8) .or. (temp .cne.&
&                 int(temp)))) then
                tempd0 = 0.0_8
              else
...
&     du3d
      ...
      complex(kind=realtype), dimension(:), intent(out) :: leftd, rightd
...
!
      ...
      complex(kind=realtype), dimension(*), intent(in) :: leftd, rightd
      complex(kind=realtype), dimension(*), intent(out) :: flux
      complex(kind=realtype), dimension(*), intent(out) :: fluxd
!
!        local variables.
!
...
!     Complexify removed intrinsic line here
      ...
      complex(kind=realtype) :: arg1
      complex(kind=realtype) :: arg1d
      complex(kind=realtype) :: result1
      complex(kind=realtype) :: result1d
      complex(kind=realtype) :: arg2
      complex(kind=realtype) :: arg2d
      complex(kind=realtype) :: result2
      complex(kind=realtype) :: result2d
      complex(kind=realtype) :: temp
      complex(kind=realtype) :: temp0
      complex(kind=realtype) :: temp1
...
!     Complexify removed intrinsic line here
    ...
    complex(kind=realtype) :: temp3
    complex(kind=realtype) :: temp4
    complex(kind=realtype) :: temp5
    complex(kind=realtype) :: temp6
    complex(kind=realtype) :: temp7
! set qcr parameters
    ccr1 = 0.3_realtype
...
! and k == kl must be tested.
            if ((k .ceq. 1) .and. storewalltensor .and. visckminpointer(i, &
&               j) .gt. 0) then
...
! and the k == kl case.
            if ((k .ceq. kl) .and. storewalltensor .and. visckmaxpointer(i&
&               , j) .gt. 0) then
...
! and j == jl must be tested.
            if ((j .ceq. 1) .and. storewalltensor .and. viscjminpointer(i, &
&               k) .gt. 0) then
...
! and the j == jl case.
            if ((j .ceq. jl) .and. storewalltensor .and. viscjmaxpointer(i&
&               , k) .gt. 0) then
...
! and i == il must be tested.
            if ((i .ceq. 1) .and. storewalltensor .and. visciminpointer(j, &
&               k) .gt. 0) then
...
! and the i == il case.
            if ((i .ceq. il) .and. storewalltensor .and. viscimaxpointer(j&
&               , k) .gt. 0) then

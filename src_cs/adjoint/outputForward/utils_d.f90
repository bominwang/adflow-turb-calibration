...
!   with respect to varying inputs: timeref
  complex(kind=realtype) function derivativerigidrotangle_d(degreepolrot, &
...
!   with respect to varying inputs: x y
  complex(kind=realtype) function mydim_d(x, xd, y, yd, mydim)
    use constants
    implicit none
    complex(kind=realtype) :: x, y
    complex(kind=realtype) :: xd, yd
...
!input/output variables
    complex(kind=realtype), dimension(3), intent(in) :: cf, cm
    complex(kind=realtype), intent(out) :: bendingmoment
!subroutine variables
    complex(kind=realtype) :: elasticmomentx, elasticmomenty, &
&   elasticmomentz
    intrinsic sqrt
...
! working variables
    complex(kind=realtype), dimension(ntimeintervalsspectral, 8) :: &
&   basecoef
    complex(kind=realtype), dimension(8) :: coef0dot
    complex(kind=realtype), dimension(ntimeintervalsspectral, 8) :: &
&   resbasecoef
    complex(kind=realtype), dimension(ntimeintervalsspectral) :: &
...
! functions
    complex(kind=realtype), dimension(ntimeintervalsspectral) :: dphix, &
&   dphiy, dphiz
    complex(kind=realtype), dimension(ntimeintervalsspectral) :: dphixdot, &
&   dphiydot, dphizdot
    complex(kind=realtype) :: derivativerigidrotangle, &
...
&   , dcdbetadot, dcdmach, dcdmachdot
    complex(kind=realtype), dimension(8) :: dcdp, dcdpdot, dcdq, dcdqdot, &
&   dcdr, dcdrdot
...
!   with respect to varying inputs: x
  complex(kind=realtype) function mynorm2_d(x, xd, mynorm2)
    use constants
    implicit none
    complex(kind=realtype), dimension(3), intent(in) :: x
    complex(kind=realtype), dimension(3), intent(in) :: xd
  ...
  function iswalltype(btype)
    ...
    logical :: iswalltype
    iswalltype = .false.
    if (((btype .ceq. nswalladiabatic) .or. (btype .ceq. nswallisothermal)) &
&       .or. (btype .ceq. eulerwall)) iswalltype = .true.
...
! inputs
    ...
    complex(kind=realtype), dimension(3), intent(in) :: ad, bd
! outputs
    complex(kind=realtype), dimension(3), intent(out) :: c
    complex(kind=realtype), dimension(3), intent(out) :: cd
...
!
    ...
    complex(kind=realtype), intent(out) :: mult, trans
! determine the situation we are having here.
    if ((mass .ceq. kilogram) .and. (len .ceq. meter) .and. (time .ceq. second)) &
&   then

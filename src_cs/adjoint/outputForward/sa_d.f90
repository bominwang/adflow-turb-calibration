...
! local variables.
    ...
    complex(kind=realtype) :: fv1d, fv2d, ft2d
    complex(kind=realtype) :: ss, sst, nu, dist2inv, chi, chi2, chi3
    complex(kind=realtype) :: ssd, sstd, nud, dist2invd, chid, chi2d, chi3d
    complex(kind=realtype) :: rr, gg, gg6, termfw, fwsa, term1, term2
    complex(kind=realtype) :: rrd, ggd, gg6d, termfwd, fwsad, term1d, &
&   term2d
    complex(kind=realtype) :: dfv1, dfv2, dft2, drr, dgg, dfw
    complex(kind=realtype) :: uux, uuy, uuz, vvx, vvy, vvz, wwx, wwy, wwz
    complex(kind=realtype) :: uuxd, uuyd, uuzd, vvxd, vvyd, vvzd, wwxd, &
&   wwyd, wwzd
    complex(kind=realtype) :: div2, fact, sxx, syy, szz, sxy, sxz, syz
    complex(kind=realtype) :: div2d, factd, sxxd, syyd, szzd, sxyd, sxzd, &
&   syzd
    complex(kind=realtype) :: vortx, vorty, vortz
    complex(kind=realtype) :: vortxd, vortyd, vortzd
    complex(kind=realtype) :: omegax, omegay, omegaz
    complex(kind=realtype) :: omegaxd, omegayd, omegazd
    complex(kind=realtype) :: strainmag2, strainprod, vortprod
    complex(kind=realtype) :: strainmag2d, strainprodd, vortprodd
...
!     Complexify removed intrinsic line here
    ...
    complex(kind=realtype) :: temp8
    complex(kind=realtype) :: temp9
    complex(kind=realtype) :: temp10
! set model constants
    cv13 = rsacv1**3
    kar2inv = one/rsak**2
    cw36 = rsacw3**6
...
! add rotation term (userotationsa defined in inputparams.f90)
            if (userotationsa) then
              ...
              if (two*strainmag2 .ceq. 0.0_8) then
                y1d = 0.0_8
              else
...
&             temp10*sstd)/sst)
            ...
            if (temp10 .le. 0.0_8 .and. ((sixth .ceq. 0.0_8) .or. (sixth &
&               .cne. int(sixth)))) then
              termfwd = 0.0_8
            else
...
! local variables.
    ...
    complex(kind=realtype) :: nud
    complex(kind=realtype) :: fv1, fv2, ft2
    complex(kind=realtype) :: voli, volmi, volpi, xm, ym, zm, xp, yp, zp
    complex(kind=realtype) :: volid, volmid, volpid, xmd, ymd, zmd, xpd, &
&   ypd, zpd
    complex(kind=realtype) :: xa, ya, za, ttm, ttp, cnud, cam, cap
    complex(kind=realtype) :: xad, yad, zad, ttmd, ttpd, cnudd, camd, capd
    complex(kind=realtype) :: nutm, nutp, num, nup, cdm, cdp
    complex(kind=realtype) :: nutmd, nutpd, numd, nupd, cdmd, cdpd
    complex(kind=realtype) :: c1m, c1p, c10, b1, c1, d1, qs
    complex(kind=realtype) :: c1md, c1pd, c10d
!     Complexify removed intrinsic line here
    complex(kind=realtype) :: temp
    complex(kind=realtype) :: temp0
    complex(kind=realtype) :: temp1

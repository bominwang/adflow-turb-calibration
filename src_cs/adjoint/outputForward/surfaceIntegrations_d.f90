...
! input/output
    ...
    complex(kind=realtype), dimension(:, :), intent(in) :: globalvalsd
    complex(kind=realtype), dimension(:), intent(out) :: funcvalues
    complex(kind=realtype), dimension(:), intent(out) :: funcvaluesd
! working
...
&   , fxliftd, fyliftd, fzliftd
    ...
    complex(kind=realtype), dimension(3) :: temp1
...
!   with respect to varying inputs: g
  subroutine ksaggregationfunction_d(g, gd, max_g, g_rho, ks_g, ks_gd)
    ...
    complex(kind=realtype), intent(in) :: gd
    intrinsic exp

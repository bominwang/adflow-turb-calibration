...
!   with respect to varying inputs: nulam
  complex(kind=realtype) function sanuknowneddyratio_d(eddyratio, nulam, &
...
!
    ...
    complex(kind=realtype), intent(in) :: nulamd
!
!      local variables.
!
...
!
    ...
    complex(kind=realtype) :: chid, chi3d, fv1d, rnusad
...
!
    ...
    complex(kind=realtype) :: qsd, volid, xad, yad, zad
    complex(kind=realtype) :: uu, dwt, dwtm1, dwtp1, dwti, dwtj, dwtk
    complex(kind=realtype) :: uud, dwtd, dwtm1d, dwtp1d, dwtid, dwtjd, &
&   dwtkd
...
!     Complexify removed intrinsic line here
    ...
    complex(kind=realtype) :: abs13
    complex(kind=realtype) :: abs14
    complex(kind=realtype) :: abs15
    complex(kind=realtype) :: abs16
    complex(kind=realtype) :: abs17
    complex(kind=realtype) :: abs18
    complex(kind=realtype) :: abs19
    complex(kind=realtype) :: abs20
    complex(kind=realtype) :: abs21
    complex(kind=realtype) :: abs22
    complex(kind=realtype) :: abs23
    complex(kind=realtype) :: temp
    complex(kind=realtype) :: temp0
...
! advective terms must be used.
    secondord = .false.
    if ((groundlevel .ceq. 1_inttype) .and. (orderturb .ceq. secondorder)) &
&     secondord = .true.

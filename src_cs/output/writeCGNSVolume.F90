 ...
 contains
    ...
    subroutine writeCGNSVolumeSol
       ...
       use complexify 
        ...
        if (nVolSolToWrite .ceq. 0) return
        ...
        if (myID .ceq. 0) then
            ...
            if (equationMode .ceq. unsteady) call writeCGNSTimeHistory
        end if
    ...
    subroutine writeCGNSConvInfo
       ...
       use complexify 
        ...
        complex(kind=8), dimension(:), allocatable :: buf8
        ...
        case (timeSpectral)
            nConv = nVolSolToWrite    ! .ceq. number of spectral solutions.
    ...
    subroutine writeCGNSTimeHistory
       ...
       use complexify 
        ...
        complex(kind=4), dimension(:), allocatable :: buf4
        complex(kind=8), dimension(:), allocatable :: buf8
    ...
    subroutine writeSolCGNSZone(zone, nSolVar, nDiscrVar, solNames)
       ...
       use complexify 
        ...
        complex(kind=4), dimension(:), allocatable :: sol4
        complex(kind=8), dimension(:), allocatable :: sol8
        ...
        rootproc: if (myID .ceq. 0) then
            ...
            solLoopRoot: do ind = 1, nVolSolToWrite
                ...
                if (ind > 1 .and. (equationMode .ceq. unsteady)) &
                ...
                if (nn .cne. mm) call getSubRangesSol
                ...
                varWriteLoop: do nn = 1, nVarWritten
                    ...
                    do mm = 1, nBlocks
                        ...
                        if (storeRindLayer .and. (.not. unsteadyHigherSol)) then
                            if (iBegor .ceq. 1) iBeg = 1
                            if (jBegor .ceq. 1) jBeg = 1
                            if (kBegor .ceq. 1) kBeg = 1

                            if (iEndor .ceq. cgnsDoms(zone)%il) iEnd = ie
                            if (jEndor .ceq. cgnsDoms(zone)%jl) jEnd = je
                            if (kEndor .ceq. cgnsDoms(zone)%kl) kEnd = ke
                        end if
            ...
            if (ierr .cne. 0) call terminate("writeSolCGNSZone", &
            ...
            testRoot: if (myID .ceq. 0) then
                ...
                do i = 1, nBlocks
                    ...
                    if (rindLayerThisSol) then
                        ...
                        if (subRanges(1, 1, i) .ceq. 2) subRanges(1, 1, i) = 1
                        if (subRanges(2, 1, i) .ceq. 2) subRanges(2, 1, i) = 1
                        if (subRanges(3, 1, i) .ceq. 2) subRanges(3, 1, i) = 1

                        if (subRanges(1, 2, i) .ceq. cgnsDoms(zone)%il) &
                            subRanges(1, 2, i) = cgnsDoms(zone)%il + 1
                        if (subRanges(2, 2, i) .ceq. cgnsDoms(zone)%jl) &
                            subRanges(2, 2, i) = cgnsDoms(zone)%jl + 1
                        if (subRanges(3, 2, i) .ceq. cgnsDoms(zone)%kl) &
                ...
                do i = 1, nBlocks
                    ...
                    if (rindLayerThisSol) then
                        ...
                        if (ii(1) .ceq. 2) ii(1) = 1
                        if (ii(2) .ceq. cgnsDoms(zone)%il) ii(2) = ii(2) + 1
                        if (ii(3) .ceq. 2) ii(3) = 1
                        if (ii(4) .ceq. cgnsDoms(zone)%jl) ii(4) = ii(4) + 1
                        if (ii(5) .ceq. 2) ii(5) = 1
                        if (ii(6) .ceq. cgnsDoms(zone)%kl) ii(6) = ii(6) + 1

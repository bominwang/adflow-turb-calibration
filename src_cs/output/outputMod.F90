...
contains
    ...
    subroutine copyDataBufSinglePrecision(val, buffer, &
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(*), intent(in) :: buffer
        complex(kind=4), dimension(iBeg:iEnd, jBeg:jEnd, kBeg:kEnd), &
            intent(inout) :: val
        !
        !      Local variables.
        !
    ...
    subroutine copyDataBufDoublePrecision(val, buffer, &
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(*), intent(in) :: buffer
        complex(kind=8), dimension(iBeg:iEnd, jBeg:jEnd, kBeg:kEnd), &
            intent(inout) :: val
        !
        !      Local variables.
        !
    ...
    subroutine storeSolInBuffer(buffer, copyInBuffer, solName, &
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(*), intent(out) :: buffer
        ...
        complex(kind=realType) :: vortx, vorty, vortz, a2, ptotInf, ptot
        complex(kind=realType) :: a, UovA(3), gradP(3)

        complex(kind=realType), dimension(:, :, :, :), pointer :: wIO
    ...
    subroutine storeSurfsolInBuffer(sps, buffer, nn, blockID, &
       ...
       use complexify 
        ...
        complex(kind=realType) :: fact, gm1, ptotInf, ptot, psurf, rsurf
        complex(kind=realType) :: usurf, vsurf, wsurf, m2surf, musurf
        complex(kind=realType) :: fx, fy, fz, fn, a2Tot, a2, qw
        complex(kind=realType) :: tauxx, tauyy, tauzz
        complex(kind=realType) :: tauxy, tauxz, tauyz
        complex(kind=realType) :: pm1, a, sensor, plocal, sensor1
        complex(kind=realType) :: vectTangential(3)
        complex(kind=realType) :: vectDotProductFsNormal
        complex(kind=realType), dimension(3) :: norm, V
        complex(kind=realType) :: coeffPressure

        complex(kind=realType), dimension(:, :, :), pointer :: ww1, ww2
        complex(kind=realType), dimension(:, :, :), pointer :: ss1, ss2, ss
        complex(kind=realType), dimension(:, :), pointer :: pp1, pp2

        complex(kind=realType), dimension(:, :), pointer :: gamma1, gamma2
        complex(kind=realType), dimension(:, :), pointer :: rlv1, rlv2
        complex(kind=realType), dimension(:, :), pointer :: dd2Wall

        complex(kind=realType) :: uInfDim2 ! MachCoeff-derived (Uinf*Uref)**2
        complex(kind=realType) :: rot_speed2 ! norm of wCrossR squared
        complex(kind=realType), Dimension(3) :: r_ ! spanwise position for given point
        complex(kind=realType), Dimension(3) :: rrate_  ! the rotational rate of the WT
        complex(kind=realType), Dimension(3) :: wCrossR ! rotationrate cross radius
        complex(kind=realType), dimension(:, :, :), pointer :: xx1, xx2 ! for the coords
        ! The original i,j beging of the local block in the entire cgns block.
        complex(kind=realType) :: subface_jBegOr, subface_jEndOr, subface_iBegOr, subface_iEndOr
        ...
        case (iMin)
            ...
            if (equations .ceq. RANSEquations) dd2Wall => d2Wall(2, :, :)
        ...
        case (iMax)
            ...
            if (equations .ceq. RANSEquations) dd2Wall => d2Wall(il, :, :)
        ...
        case (jMax)
            ...
            if (equations .ceq. RANSEquations) dd2Wall => d2Wall(:, jl, :)
        ...
        case (kMax)
            ...
            if (equations .ceq. RANSEquations) dd2Wall => d2Wall(:, :, kl)
        ...
        do j = rangeFace(2, 1), rangeFace(2, 2)
            ...
            if (present(jBeg) .and. present(jEnd) .and. (useRindLayer)) then
                ...
                if (jor .ceq. jBeg) then
                    jj = j + 1
                else if (jor .ceq. jEnd + 1) then
                    jj = j - 1
                else
                    jj = j
                end if
            ...
            do i = rangeFace(1, 1), rangeFace(1, 2)
                if (present(iBeg) .and. present(iEnd) .and. (useRindLayer)) then
                    ...
                    if (ior .ceq. iBeg) then
                        ii = i + 1
                    else if (ior .ceq. iEnd + 1) then
                        ii = i - 1
                    else
                        ii = i
                    end if
    ...
    subroutine describeScheme(string)
       ...
       use complexify 
        ...
        if (spaceDiscr .ceq. dissScalar) then
            if (dirScaling) then
        ...
        if (equations .ceq. EulerEquations) then
    ...
    subroutine writeCGNSHeader(cgnsInd, base)
       ...
       use complexify 
        ...
        viscousTest: if (viscous) then
            ...
            turbulentTest: if (equations .ceq. RANSEquations) then

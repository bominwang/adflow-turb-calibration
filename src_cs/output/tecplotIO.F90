module tecplotIO
    ...
    type slice
        ...
        complex(kind=realType), dimension(:, :), allocatable :: w, vars
        integer(kind=intType) :: nNodes
        complex(kind=realType) :: pL, vL, pD, vD, pM, vM, CLp, CLv, CDp, CDv, CMp, CMv
        complex(kind=realType) :: chord, twist, thickness
        complex(kind=realType), dimension(3) :: le, te
        complex(kind=realType), dimension(3) :: pt, normal, dir_vec
        logical :: use_dir
        integer(kind=intType), allocatable, dimension(:) :: famList
        ! here we declare that 'slice'-types also now have fx,fy and fz:
        complex(kind=realType) :: fx, fy, fz
    end type slice
    ...
    type liftDist
        ...
        complex(kind=realType) :: normal(3)
        complex(kind=realType) :: delta
        complex(kind=realType), dimension(:, :), allocatable :: slicePts
    end type liftDist
...
contains
    subroutine addParaSlice(sliceName, pt, normal, dir_vec, use_dir, famList, n)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(3), intent(in) :: pt, normal, dir_vec
        ...
        complex(kind=realType), dimension(:, :), allocatable :: pts
    ...
    subroutine addLiftDistribution(nSegments, normal, normal_ind, distName, famList, n)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(3) :: normal
    ...
    subroutine writeTecplot(sliceFile, writeSlices, liftFile, writeLift, &
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(:, :, :), allocatable :: nodalValues
        ! Working
    ...
    subroutine writeSlicesFile(fileName, nodalValues)
       ...
       use complexify 
        ...
        complex(kind=realType), intent(inout), dimension(:, :, :) :: nodalValues
        ...
        testwriteSlices: if (nParaSlices + nAbsSlices > 0) then
            ...
            if ((myID .ceq. 0) .and. printIterations) then
                print "(a)", "#"
    ...
    subroutine writeLiftDistributions(sps, fileID, nodalValues)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(:, :), intent(in) :: nodalValues

        complex(kind=realType), dimension(3) :: xmin, xmax, xmin_local, xmax_local
        complex(kind=realType), parameter :: tol = 1e-8
        type(liftDist), pointer :: d
        integer(kind=intType) :: i, j, ii, jj, iDist, ierr
        complex(kind=realType), dimension(:, :), allocatable :: values
        character(len=maxCGNSNameLen), dimension(:), allocatable :: liftDistNames
        complex(kind=realType) :: dmin, dmax, sumL, sumD, sumM, span, delta, xCur(3)
        ...
        do iDist = 1, nLiftDists
            ...
            do i = 1, d%nSegments
                ...
                else if (i .ceq. d%nSegments) then
            ...
            if (d%normal_ind .ceq. 1) then! X slices
                values(:, 2) = d%slicePts(1, :)
            else if (d%normal_ind .ceq. 2) then ! Y slices
                values(:, 3) = d%slicePts(2, :)
            else if (d%normal_ind .ceq. 3) then ! Z slices
...
#include <petsc/finclude/petsc.h>
        ...
        complex(kind=realType), dimension(:, :), allocatable :: vars
        ...
        spectralLoop: do sps = 1, nTimeIntervalsSpectral
            ...
            masterBCLoop1: do iBCGroup = 1, nFamExchange
                ...
                rootProc: if ((myid .ceq. 0) .and. nCells > 0) then
                    do iFam = 1, size(exch%famList)
                        ...
                        famInclude: if (famInList(exch%famList(iFam), famList)) then
                            ...
                            do i = 1, nCells
                                ...
                                if (elemFam(i) .ceq. exch%famList(iFam)) then
            ...
            masterBCLoop: do iBCGroup = 1, nFamExchange
                ...
                iZone = 0
                rootProc2: if ((myid .ceq. 0) .and. nCells > 0) then
                    ...
                    do iFam = 1, size(exch%famList)
                        ...
                        famInclude2: if (famInList(exch%famList(iFam), famList)) then
                            ...
                            actualWrite2: if (nCellsToWrite > 0) then
                                ...
                                do i = 1, 3
                                    if (precisionSurfGrid .ceq. precisionSingle) then
                                        call writeInteger(1)
                                    else if (precisionSurfGrid .ceq. precisionDouble) then
                                        call writeInteger(2)
                                    end if
                                end do

                                do i = 1, nSolVar
                                    if (precisionSurfSol .ceq. precisionSingle) then
                                        call writeInteger(1)
                                    else if (precisionSurfSol .ceq. precisionDouble) then
                                        call writeInteger(2)
                                    end if
                                end do
    ...
    contains
       ...
       use complexify 
            implicit none
            complex(kind=realType) :: adflowRealVal
            complex(kind=real32) :: float
            float = adflowRealval
            write (fileID) float
       ...
       use complexify 
            implicit none
            complex(kind=realType) :: adflowRealVals(:)
            complex(kind=real32) :: floats(size(adflowRealVals))
            integer :: i
       ...
       use complexify 
            implicit none
            complex(kind=realType) :: adflowRealVals(:)
            complex(kind=real64) :: dbles(size(adflowrealvals))
            integer :: i
...
#include <petsc/finclude/petsc.h>
        ...
        complex(kind=realType), dimension(:, :), intent(inout) :: nodalValues
        ...
        character(len=maxCGNSNameLen), dimension(:), allocatable :: solNames
        complex(kind=realType), dimension(:), allocatable :: buffer
        complex(kind=realType), dimension(:), pointer :: weightPtr, localPtr
    ...
    subroutine createSlice(pts, conn, elemFam, slc, pt, normal, dir_vec, use_dir, sliceName, famList)
       ...
       use complexify 
        ...
        complex(kind=realType) :: f(4), d, ovrdnom, tol, len_vec
        logical :: logic1, foundFam
        complex(kind=realType), dimension(:, :), pointer :: tmpWeight, dummy, tmpNodes
        integer(kind=intType), dimension(:, :), pointer :: tmpInd
        integer(kind=intType), dimension(:), allocatable :: link
        complex(kind=realType), dimension(:), allocatable :: fc
        complex(kind=realType), dimension(3) :: elemc, vec
    ...
    subroutine integrateSlice(lSlc, gSlc, nodalValues, nFields, doConnectivity)
       ...
       use complexify 
        ...
        complex(kind=realType), dimension(3) :: x1, x2, pT1, pT2, vT1, vT2, pF, vF, pF_elem, vF_elem
        complex(kind=realType) :: len, dmax, dmin, dist, fact, M(3, 3), tmp(6)
        complex(kind=realType) :: r(3), r_new(3), hyp, te(3), le(3), theta, w1, w2
        integer(kind=intType) :: bestPair(2), normal_ind, iProc, ierr, iSize
        complex(kind=realtype), dimension(:, :), allocatable :: tempCoords
        complex(kind=realtype), dimension(:, :), allocatable :: localVals
        ...
        complex(kind=realType), dimension(3) :: refPoint, pM, vM
        complex(kind=realType) :: xc, yc, zc
        ! Need another tmp to reduce the fx, fy and fz values
        complex(kind=realType) :: tmp_(3)
    ...
    subroutine writeSlice(slc, fileID, nFields)
       ...
       use complexify 
        ...
        complex(kind=realType) :: tmp, tx, ty, tz

module inputDiscretization
    ...
    complex(kind=realType) :: vis2, vis4, vis2Coarse, adis
    complex(kind=realType) :: acousticScaleFactor
    complex(kind=realType) :: kappaCoef
    logical :: lumpedDiss
    logical :: approxSA
    complex(kind=realType) :: sigma
    logical :: useBlockettes

#ifndef USE_TAPENADE
    complex(kind=realType) :: vis2b, vis4b, vis2Coarseb, adisb
    complex(kind=realType) :: kappaCoefb
    complex(kind=realType) :: sigmab
#endif
...
module inputIteration
    ...
    complex(kind=realType) :: CFLLimit
    ...
    complex(kind=realType) :: timeLimit
    integer(kind=intType), allocatable, dimension(:) :: cycleStrategy
    integer(kind=intType) :: miniterNum
    complex(kind=realType) :: cfl, cflCoarse, fcoll, smoop
    complex(kind=realType) :: alfaTurb, betaTurb
    complex(kind=realType) :: L2Conv, L2ConvCoarse
    complex(kind=realType) :: L2ConvRel
    complex(kind=realType) :: maxL2DeviationFactor
    complex(kind=realType) :: relaxBleeds
    complex(kind=realtype) :: epscoefconv
    integer(kind=inttype) :: convcheckwindowsize
    complex(kind=realType), allocatable, dimension(:) :: etaRK, cdisRK
    ...
    complex(kind=realType), dimension(4) :: turbResScale
    complex(kind=realType) :: meshMaxSkewness
    logical :: useSkewnessCheck
    logical :: useDissContinuation
    complex(kind=realType) :: dissContMagnitude, dissContMidpoint, dissContSharpness
...
module inputCostFunctions
    ...
    complex(kind=realtype) :: sepSensorOffset
    complex(kind=realtype) :: sepSensorKsOffset
    complex(kind=realtype) :: sepSensorKsPhi
    complex(kind=realtype) :: sepSensorSharpness
    complex(kind=realtype) :: sepSensorKsSharpness
    complex(kind=realtype) :: cavSensorOffset
    complex(kind=realtype) :: cavSensorSharpness
...
module inputMotion
    ...
    complex(kind=realType), dimension(3) :: rotPoint
    complex(kind=realType), dimension(3) :: rotPointd
    ...
    complex(kind=realType), dimension(:), allocatable :: coefPolXRot
    complex(kind=realType), dimension(:), allocatable :: coefPolYRot
    complex(kind=realType), dimension(:), allocatable :: coefPolZRot
    ...
    complex(kind=realType) :: omegaFourXRot, omegaFourXRotb
    complex(kind=realType) :: omegaFourYRot, omegaFourYRotb
    complex(kind=realType) :: omegaFourZRot, omegaFourZRotb
    ...
    complex(kind=realType), dimension(:), allocatable :: cosCoefFourXRot
    complex(kind=realType), dimension(:), allocatable :: cosCoefFourYRot
    complex(kind=realType), dimension(:), allocatable :: cosCoefFourZRot
    ...
    complex(kind=realType), dimension(:), allocatable :: sinCoefFourXRot
    complex(kind=realType), dimension(:), allocatable :: sinCoefFourYRot
    complex(kind=realType), dimension(:), allocatable :: sinCoefFourZRot
    ...
    complex(kind=realType), dimension(:), allocatable :: coefPolAlpha
    complex(kind=realType), dimension(:), allocatable :: coefPolAlphab
    ...
    complex(kind=realType) :: omegaFourAlpha, omegafouralphab

    ! cosCoefFourAlpha(0:): cosine coefficients of the
    !                      x-rotation fourier series.

    complex(kind=realType), dimension(:), allocatable :: cosCoefFourAlpha
    complex(kind=realType), dimension(:), allocatable :: cosCoefFourAlphab

    ! sinCoefFourAlpha(1:): sine coefficients of the
    !                      Alpha fourier series.

    complex(kind=realType), dimension(:), allocatable :: sinCoefFourAlpha
    complex(kind=realType), dimension(:), allocatable :: sinCoefFourAlphab
    ...
    complex(kind=realType), dimension(:), allocatable :: coefPolBeta
    complex(kind=realType), dimension(:), allocatable :: coefPolBetab
    ...
    complex(kind=realType) :: omegaFourBeta, omegafourbetab

    ! cosCoefFourBeta(0:): cosine coefficients of the
    !                      Beta fourier series.

    complex(kind=realType), dimension(:), allocatable :: cosCoefFourBeta
    complex(kind=realType), dimension(:), allocatable :: cosCoefFourBetab

    ! sinCoefFourBeta(1:): sine coefficients of the
    !                      Beta fourier series.

    complex(kind=realType), dimension(:), allocatable :: sinCoefFourBeta
    complex(kind=realType), dimension(:), allocatable :: sinCoefFourBetab
    ...
    complex(kind=realType), dimension(:), allocatable :: coefPolMach
    complex(kind=realType), dimension(:), allocatable :: coefPolMachb
    ...
    complex(kind=realType) :: omegaFourMach, omegafourmachb

    ! cosCoefFourMach(0:): cosine coefficients of the
    !                      Mach Number fourier series.

    complex(kind=realType), dimension(:), allocatable :: cosCoefFourMach
    complex(kind=realType), dimension(:), allocatable :: cosCoefFourMachb

    ! sinCoefFourMach(1:): sine coefficients of the
    !                      Mach Number fourier series.

    complex(kind=realType), dimension(:), allocatable :: sinCoefFourMach
    complex(kind=realType), dimension(:), allocatable :: sinCoefFourMachb
...
module inputParallel
    ...
    complex(realType) :: loadImbalance
    logical :: splitBlocks
...
module inputPhysics
    ...
    complex(kind=realType) :: alpha, beta
    integer(kind=intType) :: liftIndex
    complex(kind=realType) :: Mach, MachCoef, MachGrid
    complex(kind=realType) :: Reynolds, ReynoldsLength
    complex(kind=realType) :: gammaConstant, RGasDim
    complex(kind=realType) :: Prandtl, PrandtlTurb, pklim, wallOffset, wallDistCutoff
    complex(kind=realType) :: eddyVisInfRatio, turbIntensityInf
    complex(kind=realType) :: surfaceRef, lengthRef
    complex(kind=realType), dimension(3) :: velDirFreestream
    complex(kind=realType), dimension(3) :: liftDirection
    complex(kind=realType), dimension(3) :: dragDirection
    complex(kind=realType), dimension(3) :: pointRef
    complex(kind=realType), dimension(3, 2) :: momentAxis
    complex(kind=realType) :: SSuthDim, muSuthDim, TSuthDim
    complex(kind=realType) :: cavitationnumber
    complex(kind=realType) :: cpmin_rho
    complex(kind=realType), dimension(:), allocatable :: cpmin_family
    complex(kind=realType) :: sepSenMaxRho
    complex(kind=realType), dimension(:), allocatable :: sepSenMaxFamily

#ifndef USE_TAPENADE
    complex(kind=realType) :: alphad, betad
    complex(kind=realType), dimension(3) :: velDirFreestreamd, velDirFreeStreamb
    complex(kind=realType), dimension(3) :: liftDirectiond, liftDirectionb
    complex(kind=realType), dimension(3) :: dragDirectiond, dragDirectionb
    complex(kind=realType), dimension(3) :: pointRefd, pointRefb
    complex(kind=realType), dimension(3, 2) :: momentAxisd, momentAxisb
    complex(kind=realType) :: Machd, MachCoefd, MachGridd
    complex(kind=realType) :: reynoldsd, reynoldslengthd
    complex(kind=realType) :: gammaconstantd
    complex(kind=realType) :: surfaceRefd, lengthRefd
    complex(kind=realType) :: rgasdimd
    complex(kind=realType) :: Prandtlb, PrandtlTurbb
#endif

    complex(kind=realType), dimension(3) :: pointRefEC
...
module inputTimeSpectral
    ...
    complex(kind=realType), dimension(:, :, :), allocatable :: dscalar
    complex(kind=realType), dimension(:, :, :), allocatable :: dvector
    ...
    complex(kind=realType) :: dtUnsteadyRestartSpectral
    ...
    complex(kind=realType), dimension(:, :, :), allocatable :: &
        rotMatrixSpectral
    logical :: useTSInterpolatedGridVelocity

    complex(kind=realType) :: omegaFourier
...
module inputUnsteady
    ...
    complex(kind=realType), dimension(:, :), allocatable :: betaRKUnsteady
    complex(kind=realType), dimension(:), allocatable :: gammaRKUnsteady
...
module inputADjoint
    ...
    complex(kind=realType) :: adjMaxL2Dev
...
module inputOverset
    ...
    complex(kind=realType) :: overlapFactor = 0.9
    complex(kind=realType) :: nearWallDist = 0.1
    complex(kind=realType) :: oversetProjTol = 1e-12
    complex(kind=realType) :: backgroundVolScale = 1.0
    logical :: debugZipper = .False.
    integer(kind=intType) :: oversetUpdateMode
    complex(kind=realType) :: selfZipCutoff

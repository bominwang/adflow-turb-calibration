...
#include <petsc/finclude/petsc.h>
    ...
    type CSRMatrix
        ...
        complex(kind=realType), dimension(:), pointer :: data
    ...
    type oversetBlock
        ...
        complex(kind=realType), dimension(:), allocatable :: rBuffer
    ...
    type oversetFringe
        ...
        complex(kind=realType), dimension(:, :), allocatable :: x

        ! These are the coordinate of its wall surface pt if applicable
        complex(kind=realType), dimension(:, :), allocatable :: xSeed
        ...
        complex(kind=realType), dimension(:, :), pointer :: fringeRealBuffer => null()
    ...
    type oversetWall
        ...
        complex(kind=realType), dimension(:, :), pointer :: x => null()

        ! Only primal mesh cell centers for dual mesh. Only allocated as
        ! needed.
        complex(kind=realType), dimension(:, :), pointer :: xPrimalCen => null()

        ! Surface nonal used for determining if point is "underneath" the
        ! surface.
        complex(kind=realType), dimension(:, :), pointer :: norm => null()

        ! Local estimate of surface error
        complex(kind=realType), dimension(:), pointer :: delta => null()
    ...
    type oversetString
        ...
        complex(kind=realType), dimension(:, :), pointer :: nodeData => null()
        ...
        complex(kind=realType), dimension(:, :), pointer :: perpNorm => null()

        ! Pointer for nodal element size. Points to nodeData
        complex(kind=realType), dimension(:), pointer :: h => null()
    ...
    complex(kind=realType), dimension(:), allocatable :: clusterAreas
    complex(kind=realType), dimension(:), allocatable :: clusterMarchDist
    ...
    complex(kind=realType), dimension(iTotal) :: tStart
    complex(kind=realType), dimension(iTotal) :: oversetTimes

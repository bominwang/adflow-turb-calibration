...
contains
    ...
    subroutine heapify(a, i_in)
       use complexify 
        ...
        complex(kind=realType) :: pri_i, pri_l, pri_r, pri_largest
    ...
    complex(kind=realType) function pq_maxpri(a)
        implicit none
        type(pq), pointer :: a
    ...
    complex(kind=realType) function pq_insert(a, dis, idx)
        implicit none
        !
    ...
    subroutine pq_adjust_heap(a, i)
       use complexify 
        ...
        complex(kind=realType) :: prichild
    ...
    complex(kind=realType) function pq_replace_max(a, dis, idx)
        implicit none
        !
        ...
        integer(kind=intType) :: parent, child, N
        complex(kind=realType) :: prichild, prichildp1
...
module kdtree2_module
    ...
    type interval
        complex(kind=realType) :: lower, upper
    end type interval
    ...
    type :: tree_node
        ...
        complex(kind=realType) :: cut_val
        ! where to cut the dimension
        complex(kind=realType) :: cut_val_left, cut_val_right
    ...
    type :: kdtree2
        ...
        complex(kind=realType), pointer :: the_data(:, :) => null()
        ...
        complex(kind=realType), pointer :: rearranged_data(:, :) => null()
    ...
    type :: tree_search_record
        ...
        complex(kind=realType) :: ballsize
        ...
        logical :: overflow
        complex(kind=realType), pointer :: qv(:)  ! query vector
        type(kdtree2_result), pointer :: results(:) ! results
        type(pq) :: pq
        complex(kind=realType), pointer :: data(:, :)  ! temp pointer to data
...
contains
    ...
    function kdtree2_create(input_data, dim, sort, rearrange) result(mr)
       use complexify 
        ...
        ! ..
        ! .. Array Arguments ..
        complex(kind=realType), target :: input_data(:, :)
        !
        integer(kind=intType) :: i
        ! ..
        allocate (mr)
    ...
    recursive function build_tree_for_range(tp, l, u, parent) result(res)
       ...
       use complexify 
        ...
        logical :: recompute
        complex(kind=realType) :: average, left, right, coorspread(tp%dimen), tmp
...
!!$
    ...
    integer(kind=intType) function select_on_coordinate_value(v, ind, c, alpha, li, ui) &
        ...
        complex(kind=realType), intent(in) :: alpha
        ! ..
        complex(kind=realType) :: v(1:, 1:)
    ...
    subroutine spread_in_coordinate(tp, c, l, u, interv)
       ...
       use complexify 
        ...
        ! ..
        ! .. Local Scalars ..
        complex(kind=realType) :: last, lmax, lmin, t, smin, smax
        integer(kind=intType) :: i, ulocal
        ! ..
        ! .. Local Arrays ..
        complex(kind=realType), pointer :: v(:, :)
    ...
    subroutine kdtree2_n_nearest(tp, qv, nn, results)
       use complexify 
        ...
        type(kdtree2), pointer :: tp
        complex(kind=realType), target, intent(In) :: qv(:)
    ...
    subroutine kdtree2_r_nearest(tp, qv, r2, nfound, nalloc, results)
       use complexify 
        ...
        type(kdtree2), pointer :: tp
        complex(kind=realType), target, intent(In) :: qv(:)
        complex(kind=realType), intent(in) :: r2
    ...
    recursive subroutine search(node)
       use complexify 
        ...
        ! ..
        complex(kind=realType) :: qval, dis
        complex(kind=realType) :: ballsize
        complex(kind=realType), pointer :: qv(:)
    ...
    complex(kind=realType) function dis2_from_bnd(x, amin, amax) result(res)
        implicit none
        complex(kind=realType), intent(in) :: x, amin, amax
    ...
    logical function box_in_search_range(node, sr) result(res)
       use complexify 
        ...
        complex(kind=realType) :: dis, ballsize
        complex(kind=realType) :: l, u
    ...
    subroutine process_terminal_node(node)
       use complexify 
        ...
        complex(kind=realType), pointer :: qv(:)
        integer(kind=intType), pointer :: ind(:)
        complex(kind=realType), pointer :: data(:, :)
        !
        integer(kind=intType) :: dimen, i, indexofi, k, centeridx, correltime
        complex(kind=realType) :: ballsize, sd, newpri
        logical :: rearrange
        type(pq), pointer :: pqp
        !
        ...
        mainloop: do i = node%l, node%u
            ...
            if (sr%nfound .lt. sr%nn) then
                ...
                if (sr%nfound .ceq. sr%nn) ballsize = newpri
    ...
    subroutine process_terminal_node_fixedball(node)
       use complexify 
        ...
        complex(kind=realType) :: ballsize, sd
        logical :: rearrange
    ...
    subroutine kdtree2_n_nearest_brute_force(tp, qv, nn, results)
       use complexify 
        ...
        ! does.
        type(kdtree2), pointer :: tp
        complex(kind=realType), intent(In) :: qv(:)
        integer(kind=intType), intent(In) :: nn
        type(kdtree2_result) :: results(:)

        integer(kind=intType) :: i, j, k
        complex(kind=realType), allocatable :: all_distances(:)
        ! ..
    ...
    subroutine heapsort(a, ind, n)
       use complexify 
        ...
        !
        integer(kind=intType), intent(in) :: n
        complex(kind=realType), intent(inout) :: a(:)
        integer(kind=intType), intent(inout) :: ind(:)

        !
        !
        complex(kind=realType) :: value   ! temporary for a value from a()

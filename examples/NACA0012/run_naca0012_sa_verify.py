"""
NACA0012 SA 系数修改验证: 1 组默认 + 3 组随机采样 (7 个参数), 提取 Cp 并对比。
...
用法 (Docker 内):
    mpirun --oversubscribe -np 2 python run_naca0012_sa_verify.py
"""
import os
import re
import numpy as np
...
OUTPUT_DIR = "/workspace/repo/examples/NACA0012/output_sa"
...
if rank == 0:
    print("=" * 70)
    print("  NACA0012 SA Coefficient Verification")
...
for case_idx, (label, coeffs) in enumerate(all_sets):
    ...
    # 设置 SA 系数 (7 个校准参数, ct3/ct4 保持默认)
    pt.setsaconstants(
        coeffs["cb1"],
        coeffs["cb2"],
        coeffs["sigma"],   # 注意: sigma 存在 rsaCb3 中
        coeffs["kappa"],
        coeffs["cv1"],
        coeffs["cw2"],
        coeffs["cw3"],
    ...
    # 验证
    if rank == 0:
        cw1 = coeffs["cb1"] / coeffs["kappa"]**2 + (1 + coeffs["cb2"]) / coeffs["sigma"]
        print(f"  Set: cb1={pt.rsacb1:.6f}, cb2={pt.rsacb2:.6f}, "
              f"sigma={pt.rsacb3:.6f}, kappa={pt.rsak:.6f}")
        print(f"  Set: cv1={pt.rsacv1:.6f}, cw2={pt.rsacw2:.6f}, cw3={pt.rsacw3:.6f}")
        print(f"  Derived cw1={pt.rsacw1:.6f} (expected={cw1:.6f})")
    ...
    # 切片文件由 solver(ap) 自动写入
    if rank == 0:
        slice_file = f"{OUTPUT_DIR}/n0012_sa_{label}_slices.dat"
        if not os.path.exists(slice_file):
            for fn in os.listdir(OUTPUT_DIR):
                if label in fn and "slice" in fn and fn.endswith(".dat"):
                    slice_file = f"{OUTPUT_DIR}/{fn}"
                    break
        ...
        if os.path.exists(slice_file):
            ...
            if icp is None:
                print(f"  WARNING: No Cp column!")
                continue

            x = data[:, ix] if ix is not None else None
            y = data[:, iy] if iy is not None else None
            cp = data[:, icp]

            if x is None:
                print(f"  WARNING: No x column!")
                continue
...
if rank == 0:
    ...
    npz_path = f"{OUTPUT_DIR}/naca0012_sa_verify.npz"
